 ...
 * 6. Ensures all package.json files use catalog references where available
...
async function main() {
  ...
  console.log('\n4️⃣  Checking package.json files for catalog references');
    ...
    for (let i = 0; i < packageJsonFiles.length; i++) {
      ...
      try {
        ...
        const checkAndUpdateSection = (section) => {
          ...
          for (const [pkg, version] of Object.entries(packageJson[section])) {
            // Skip special protocols and invalid versions
