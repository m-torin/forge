...
[Lemon Squeezy](https://www.lemonsqueezy.com) is an all-in-one platform for
running your SaaS business. It handles payments, subscriptions, global tax
compliance, fraud prevention, multi-currency, and more. Here's how to switch the
default payment processor from Stripe to Lemon Squeezy.
...
Additionally, replace all instances of `STRIPE_SECRET_KEY` with
`LEMON_SQUEEZY_API_KEY` in the `packages/env/index.ts` file.

## 3. Update the payments client

Initialize the payments client in the `packages/payments/index.ts` file with the
new API key. Then, export the `lemonSqueezySetup` function from the file.
...
There's quite a lot you can do with Lemon Squeezy, so check out the following
resources for more information:

<Card
  title="Webhooks Overview"
  icon="webhook"
  href="https://docs.lemonsqueezy.com/guides/developer-guide/webhooks"
  horizontal
>
...
<Card
  ...
  icon="badge-check"
  href="https://docs.lemonsqueezy.com/help/webhooks/signing-requests"
  horizontal
>
