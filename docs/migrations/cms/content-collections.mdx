...
[Content Collections](https://www.content-collections.dev/) to generate
type-safe data collections from MDX files. This approach provides a structured
way to manage blog posts while maintaining full type safety throughout your
application.
...
Now we need to modify the CMS package scripts to replace the `basehub` commands
...
<Note>
  We're using the Content Collections CLI directly to generate the collections
  prior to Next.js processes. The files are cached and not rebuilt in the
  Next.js build process. This is a workaround for [this
  issue](https://github.com/sdorra/content-collections/issues/214).
</Note>
...
Create a new content collections file in the `cms` package, then import the
collections in the `web` package.

<Note>
  We're remapping the `title` field to `_title` and the `_meta.path` field to
  `_slug` to match the default next-forge CMS.
</Note>
...
directory. The file name will be used as the slug for the blog post and the
frontmatter will be used to generate the blog post page. For example:
...
The same concept applies to the `legal` collection, which is used to generate
the legal policy pages. Also, the `image` field is the path relative to the
app's root `public` directory.
...
You can use the [Fumadocs](/migrations/documentation/fumadocs) MDX plugins to
enhance your MDX content.
...
You can calculate reading time for your collection by adding a transform
function.
...
You can generate a low-quality image placeholder for your collection by adding a
transform function.
