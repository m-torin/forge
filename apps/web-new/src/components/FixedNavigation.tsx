...
import CollectionCard3 from "@repo/design-system/ciseco/components/CollectionCard3";
import { type TCollection } from "@repo/design-system/ciseco/data/data";
import { type TNavigationItem } from "@repo/design-system/ciseco/data/navigation";
...
const MegaMenu = ({
  ...
  const renderNavlink = (item: TNavigationItem) => {
    return (
      <li
        key={item.id}
        className={clsx("menu-item", item.isNew && "menuIsNew")}
      >
        <Link
...
const FixedNavigation: FC<Props> = ({
  className,
  featuredCollection,
  menu,
}) => {
