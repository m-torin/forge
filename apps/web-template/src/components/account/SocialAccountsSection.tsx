...
import { Card, Title, Stack, Group, Button, Text, Badge } from '@mantine/core';
import { IconBrandGoogle, IconBrandFacebook, IconBrandTwitter, IconLink, IconUnlink } from '@tabler/icons-react';
...
export function SocialAccountsSection() {
  ...
  const unlinkSocialAccount = async (provider: keyof typeof providerConfig) => {
    ...
    const connectedAccounts = user.accounts?.filter(a => 
      Object.keys(providerConfig).includes(a.provider)
    ).length || 0;
  ...
  return (
    <Card shadow="sm" radius="md" withBorder>
      <Title order={3} mb="md">Connected Accounts</Title>
      ...
      {!user.hasPassword && (
        <Card bg="yellow.1" mt="md" p="md">
          <Group gap="xs">
            <Text size="sm" fw={500}>Security tip:</Text>
            <Text size="sm">
