'use client';

import { Avatar, Button, Group, Menu, Text, UnstyledButton, useMantineTheme } from '@mantine/core';
import {
  IconChevronDown,
  IconCreditCard,
  IconHeart,
  IconKey,
  IconLogin,
  IconLogout,
  IconPackage,
  IconSettings,
  IconUser,
} from '@tabler/icons-react';
import Link from 'next/link';
import { type FC } from 'react';

import classes from './GuestNavMenu.module.css';

interface GuestNavMenuProps {
  dict?: {
    account?: string;
    help?: string;
    login?: string;
    logout?: string;
    orders?: string;
    wishlist?: string;
  };
  locale?: string;
  user?: null | {
    avatar?: string;
    email?: string;
    name?: string;
  };
}

const GuestNavMenu: FC<GuestNavMenuProps> = ({ dict = {}, locale = 'en', user }) => {
  const theme = useMantineTheme();

  if (!user) {
    return (
      <Button
        component={Link}
        href={`/${locale}/login`}
        leftSection={<IconLogin size={16} />}
        radius="sm"
        size="md"
        variant="subtle"
      >
        {dict.login || 'Login'}
      </Button>
    );
  }

  return (
    <Menu
      closeDelay={300}
      openDelay={100}
      position="bottom-end"
      transitionProps={{ transition: 'pop-top-right' }}
      trigger="hover"
      width={260}
      withinPortal
    >
      <Menu.Target>
        <UnstyledButton className={classes.user}>
          <Group gap={7}>
            <Avatar alt={user.name} radius="xl" size={20} src={user.avatar} />
            <Text fw={500} lh={1} mr={3} size="md">
              {user.name}
            </Text>
            <IconChevronDown size={12} stroke={1.5} />
          </Group>
        </UnstyledButton>
      </Menu.Target>
      <Menu.Dropdown>
        <Menu.Label>My Account</Menu.Label>

        <Menu.Item
          component={Link}
          href={`/${locale}/account`}
          leftSection={<IconUser size={16} stroke={1.5} />}
        >
          Account Settings
        </Menu.Item>

        <Menu.Item
          component={Link}
          href={`/${locale}/account-wishlists`}
          leftSection={<IconHeart color={theme.colors.red[6]} size={16} stroke={1.5} />}
        >
          My Favorites
        </Menu.Item>

        <Menu.Item
          component={Link}
          href={`/${locale}/orders`}
          leftSection={<IconPackage size={16} stroke={1.5} />}
        >
          Order History
        </Menu.Item>

        <Menu.Item
          component={Link}
          href={`/${locale}/account-password`}
          leftSection={<IconKey size={16} stroke={1.5} />}
        >
          Change Password
        </Menu.Item>

        <Menu.Item
          component={Link}
          href={`/${locale}/account-billing`}
          leftSection={<IconCreditCard size={16} stroke={1.5} />}
        >
          Billing & Payment
        </Menu.Item>

        <Menu.Divider />

        <Menu.Item leftSection={<IconLogout size={16} stroke={1.5} />}>
          {dict.logout || 'Logout'}
        </Menu.Item>
      </Menu.Dropdown>
    </Menu>
  );
};

export default GuestNavMenu;
