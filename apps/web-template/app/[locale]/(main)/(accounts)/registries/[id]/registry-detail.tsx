...
export function RegistryDetail({
  ...
  const handleRemoveUser = async (userId: string) => {
    try {
      const result = await removeRegistryUser(registry.id, userId);
      if (result.success) {
        notifications.show({
          title: 'Success',
  ...
  const handleUpdateItemPriority = async (itemId: string, newPriority: number) => {
    try {
      const result = await updateRegistryItem({
        registryId: registry.id,
        itemId,
  ...
  const handleSendThankYou = async (values: z.infer<typeof thankYouFormSchema>) => {
    ...
    try {
      const result = await sendThankYouMessage(selectedPurchase.id, values.message);
