...
const ProductSizeOption = ({
  ...
  const renderSizeOptions = () => {
    const sizeOptionValues = (options as any)?.find(
    ...
    return (
      <div>
        <div className="flex justify-between text-sm font-medium">
          ...
          <div className="text-primary-600 hover:text-primary-500 cursor-pointer">
            See sizing chart
          </div>
        </div>
        <div className="mt-2.5 grid grid-cols-5 gap-2 sm:grid-cols-7">
          {sizeOptionValues.map((size: any, index: number) => {
            // for demo purpose
