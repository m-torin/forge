...
export async function getQStashConfig(): Promise<QStashConfig> {
  ...
  if (useLocalQStash) {
    ...
    return {
      ...
      currentSigningKey:
        env.QSTASH_CURRENT_SIGNING_KEY || 'sig_7RvLjqfZBvP5KEUimQCE1pvpLuou',
      nextSigningKey:
        env.QSTASH_NEXT_SIGNING_KEY || 'sig_7W3ZNbfKWk5NWwEs3U4ixuQ7fxwE',
      mode: 'local',
    }
  } else {
    ...
    if (
      ...
      throw new Error(
        'Production QStash environment variables are required when not using local mode',
      )
    }
