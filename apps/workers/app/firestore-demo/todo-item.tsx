...
export default function TodoItem({ removeTodo, todo, toggleStatus }: TodoItemProps) {
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  // Format the date for display
  const formattedDate = todo.createdAt ? new Date(todo.createdAt).toLocaleString() : 'Unknown date';
  ...
  return (
    <li className={`p-4 ${isLoading ? 'opacity-50' : ''}`}>
      <div className="flex items-center">
        ...
        <span className={`ml-3 flex-grow ${todo.completed ? 'line-through text-gray-500' : ''}`}>
          {todo.title}
        </span>

        <span className="text-xs text-gray-500 mx-2">{formattedDate}</span>
