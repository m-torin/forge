...
describe('WorkflowService', () => {
  ...
  describe('workflow execution', () => {
    it('should execute workflow asynchronously', async () => {
      ...
      const result = await service.executeWorkflow(
        'test-workflow',
        input,
        { type: 'manual', payload: {}, triggeredBy: 'test' },
        options,
      );
    ...
    it('should use workflow defaults when options not provided', async () => {
      ...
      await service.executeWorkflow('test-workflow', input, {
        type: 'manual',
        payload: {},
        triggeredBy: 'test',
      });
  ...
  describe('synchronous workflow execution', () => {
    ...
    it('should handle synchronous execution failure', async () => {
      ...
      await expect(service.executeWorkflowSync('test-workflow', input)).rejects.toThrow(
    ...
    it('should throw error for non-existent workflow in sync execution', async () => {
      ...
      await expect(service.executeWorkflowSync('non-existent', {})).rejects.toThrow(
  ...
  describe('execution cancellation', () => {
    ...
    it('should throw error when trying to cancel completed execution', async () => {
      ...
      await expect(service.cancelExecution('exec-123')).rejects.toThrow(
    ...
    it('should throw error when trying to cancel failed execution', async () => {
      ...
      await expect(service.cancelExecution('exec-123')).rejects.toThrow(
        'Cannot cancel execution in status: failed',
      );
    });
    ...
    it('should handle cancellation errors', async () => {
      ...
      await expect(service.cancelExecution('exec-123')).rejects.toThrow('Cancellation failed');
    });
  });
  ...
  describe('event handling', () => {
    it('should register event handler', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        'Registered handler for event type: workflow-started',
      );
    ...
    it('should unregister event handler', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        'Unregistered handler for event type: workflow-started',
      );
    });
  });
});
