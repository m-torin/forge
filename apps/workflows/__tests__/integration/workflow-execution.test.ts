...
// Mock the memory store singleton
vi.mock('@/lib/storage/memory-store', async () => {
  const actual = await vi.importActual<typeof import('@/lib/storage/memory-store')>(
    '@/lib/storage/memory-store',
  );
  const store = new actual.MemoryStore(1000, 100);
  return {
    memoryStore: store,
    MemoryStore: vi.fn().mockImplementation(() => store),
  };
});
...
// Import MemoryStore here after mocks are set up
...
describe('Workflow Integration Tests', () => {
  ...
  // Mock workflow registry methods directly
  const mockWorkflow = {
    id: 'test-workflow',
  ...
  beforeEach(async () => {
    ...
    const { memoryStore: mockedStore } = await import('@/lib/storage/memory-store');
    registry = new WorkflowRegistry('./test-workflows');
    memoryStore = mockedStore as MemoryStore;
    memoryStore.clear(); // Clear between tests
    wsServer = new WorkflowWebSocketServer(3102);
    qstashClient = new QStashClient();

    // Register the test workflow directly
    await registry.registerWorkflow(mockWorkflow);

    // Mock the executeWorkflow method to use our handler
    vi.spyOn(registry, 'executeWorkflow').mockImplementation(async (id, input) => {
      if (id === 'test-workflow') {
        return mockWorkflowHandler(input);
      }
  ...
  describe('memory store and execution tracking integration', () => {
    it('should track and manage workflow executions', async () => {
      ...
      expect(byWorkflow).toContainEqual(execution);

      const byStatus = memoryStore.getExecutionsByStatus('running');
      expect(byStatus).toContainEqual(execution);
      ...
      expect(searchResult.executions).toContainEqual(execution);
      ...
      expect(completedExecutions).toContainEqual(completedExecution);
    });
    ...
    it('should handle memory constraints properly', async () => {
      // Use the actual MemoryStore implementation for this test
      const { MemoryStore: ActualMemoryStore } = await vi.importActual<
        typeof import('@/lib/storage/memory-store')
      >('@/lib/storage/memory-store');
      const smallStore = new ActualMemoryStore(3, 3); // Small capacity
      ...
      for (let i = 0; i < 5; i++) {
        const execution: WorkflowExecution = {
          ...
          status: 'completed',
          input: { iteration: i },
          startedAt: new Date(Date.now() - (5 - i) * 1000), // Oldest first (exec-0 is oldest)
          steps: [],
          triggeredBy: 'manual',
          environment: 'test',
          version: '1.0.0',
      ...
      // Should keep most recent (exec-2, exec-3, exec-4)
  ...
  describe('cross-component integration scenarios', () => {
    ...
    it('should handle error scenarios across components', async () => {
      ...
      await expect(registry.executeWorkflow('test-workflow', execution.input)).rejects.toThrow(
    ...
    it('should maintain data consistency across operations', async () => {
      ...
      // 1. Register multiple workflows directly
      const testWorkflow2 = {
        ...mockWorkflow,
        id: 'email-workflow',
        name: 'Email Workflow',
        filePath: '/test/workflows/email-workflow.ts',
      };

      await registry.registerWorkflow(testWorkflow2);
