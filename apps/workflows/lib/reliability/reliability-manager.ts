...
export class ReliabilityManager extends EventEmitter {
      ...
      if (this.bulkheads.has(patternName)) {
        ...
        result = await bulkhead.execute(async () => {
          if (this.circuitBreakers.has(patternName)) {
            ...
            if (config.retry) {
              const retryResult = await RetryManager.execute(() => circuitBreaker.execute(timedOperation), {
                ...config.retry,
  ...
  private async performHealthCheck(patternName: string): Promise<void> {
    ...
    if (!allHealthy) {
      console.warn(`Health check failed for pattern ${patternName}:`, 
  ...
  private getPatternStats(patternName: string): ReliabilityStats {
    ...
    const uptime = stats.totalExecutions > 0
  ...
  async shutdown(): Promise<void> {
    ...
    // Shutdown bulkheads
    const bulkheadShutdowns = Array.from(this.bulkheads.values())
      .map(bulkhead => bulkhead.shutdown())
    
    // Shutdown circuit breakers
    const circuitBreakerShutdowns = Array.from(this.circuitBreakers.values())
      .map(cb => cb.shutdown())
...
export function Reliable(patternName: string, options?: { timeout?: number; context?: Record<string, any> }) {
