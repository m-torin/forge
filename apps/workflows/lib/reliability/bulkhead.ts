...
export class Bulkhead extends EventEmitter {
  ...
  getStats(): BulkheadStats {
    const averageExecutionTime = this.stats.completedExecutions > 0
    ...
    const averageWaitTime = totalWaited > 0
      ? this.stats.totalWaitTime / totalWaited
      : 0
  ...
  isHealthy(): boolean {
    const rejectionRate = this.stats.totalExecutions > 0
  ...
  getQueueUtilization(): number {
    return this.config.maxWaiting > 0
      ? (this.queue.length / this.config.maxWaiting) * 100
      : 0
  }
  ...
  async shutdown(timeoutMs = 30000): Promise<void> {
    ...
    while (this.queue.length > 0) {
      ...
      if (request) {
        ...
        request.reject(new BulkheadError(
    ...
    if (this.currentExecutions > 0) {
      console.warn(`Bulkhead ${this.config.name} shutdown timeout: ${this.currentExecutions} executions still running`)
    }
