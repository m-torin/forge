...
export function Retryable(config?: Partial<RetryConfig>) {
  return function (target: any, propertyName: string, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = async function (...args: any[]) {
      const result = await RetryManager.execute(() => method.apply(this, args), config);
      return result.result;
    };
  ...
  static async query<T>(queryFn: () => Promise<T>): Promise<T> {
    const result = await RetryManager.execute(queryFn, {
      ...
      onRetry: (attempt, error, delay) => {
        console.warn(
          `Database query failed (attempt ${attempt}), retrying in ${delay}ms:`,
          error.message,
        );
      },
...
// Server-only utilities - not available in browser
export class FileSystemRetryManager {
  static async readFile(filePath: string): Promise<string> {
    const fs = await import('fs/promises');

    const result = await RetryManager.execute(() => fs.readFile(filePath, 'utf-8'), {
  ...
  static async writeFile(filePath: string, data: string): Promise<void> {
    ...
    const result = await RetryManager.execute(() => fs.writeFile(filePath, data, 'utf-8'), {
      ...RetryManager.strategies.quick,
      retryCondition: RetryManager.fileSystemRetryCondition,
    });
    return result.result;
