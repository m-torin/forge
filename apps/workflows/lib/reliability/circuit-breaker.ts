...
export class CircuitBreaker<T = any> extends EventEmitter {
  ...
  private validateConfig(): void {
    const { failureThreshold, halfOpenMaxAttempts, monitoringPeriod, resetTimeout, successThreshold, timeout } = this.config
...
export class CircuitBreakerRegistry {
  ...
  async shutdown(): Promise<void> {
    const shutdownPromises = Array.from(this.breakers.values())
      .map(breaker => breaker.shutdown())
