...
export class WorkflowProviderManager {
  ...
  async cancelWorkflow(executionId: string, providerHint?: ProviderType): Promise<void> {
    ...
    throw new Error(
      `Failed to cancel execution ${executionId}. Errors: ${errors.map((e) => e.message).join(', ')}`,
    );
  }
  ...
  private validateProviderCapabilities(
    ...
    if (options) {
      try {
        ...
        throw new Error(
          `Provider ${provider.name} validation failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
        );
      }
    }
  }
