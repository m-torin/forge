...
export class StepFactory {
  ...
  createStep(config: StepFactoryConfig): StepDefinition {
    ...
    if (template.configSchema && config.config) {
      ...
      if (!parseResult.success) {
        throw new Error(`Invalid configuration for template ${templateId}: ${parseResult.error.message}`)
      }
    }
  ...
  cloneStep(stepId: string, modifications: Partial<StepFactoryConfig>): StepDefinition {
    ...
    const templateId = Array.from(this.templates.entries())
      .find(([, template]) => template.category === originalStep.category)?.[0]
    ...
    const compositeStep: StepDefinition = {
      ...
      handler: async (context: StepContext): Promise<StepResult> => {
        ...
        return {
          ...
          output: options.aggregateResults ? outputs : results[results.length - 1]?.output,
          success: allSuccessful || (options.continueOnError && results.some(r => r.success))
        }
      }
    }
  ...
  createConditionalStep(
    ...
    const conditionalStep: StepDefinition = {
      ...
      name,
      category: 'control',
      description: options.description || `Conditional step: ${ifTrueStepId} or ${ifFalseStepId || 'skip'}`,
  ...
  createLoopStep(
    ...
    const loopStep: StepDefinition = {
      ...
      handler: async (context: StepContext): Promise<StepResult> => {
        ...
        const success = errors.length === 0 || (options.continueOnError && results.length > 0)
