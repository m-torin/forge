...
export class WorkflowFactory {
  ...
  createWorkflow(config: WorkflowFactoryConfig): WorkflowDefinition {
    ...
    if (template.configSchema && config.config) {
      ...
      if (!parseResult.success) {
        throw new Error(`Invalid configuration for workflow template ${templateId}: ${parseResult.error.message}`)
      }
    }
  ...
  getWorkflowHandler(workflowId: string): ((input: Record<string, any>) => Promise<any>) | undefined {
  ...
  cloneWorkflow(workflowId: string, modifications: Partial<WorkflowFactoryConfig>): WorkflowDefinition {
    ...
    const templateId = Array.from(this.templates.entries())
      .find(([, template]) => template.category === originalWorkflow.category)?.[0]
