...
export class ExecutionContextManager {
  ...
  private updateExecutionMetrics(context: ExecutionContext): void {
    if (context.status === 'completed' || context.status === 'failed') {
      ...
      if (stepDurations.length > 0) {
        context.metrics.latency = stepDurations.reduce((sum, d) => sum + d, 0) / stepDurations.length
      }
    }
  }
  ...
  getExecutionStats(): {
    ...
    const averageDuration = completedContexts.length > 0
      ? completedContexts.reduce((sum, c) => sum + (c.metrics.duration || 0), 0) / completedContexts.length
      : 0

    const averageStepCount = allContexts.length > 0
      ? allContexts.reduce((sum, c) => sum + Object.keys(c.stepResults).length, 0) / allContexts.length
      : 0
  ...
  getPerformanceReport(executionId: string): {
    ...
    const memoryPeak = Math.max(
      ...Object.values(context.stepMetrics)
        .map(m => m.memoryUsage?.heapUsed || 0)
    )
