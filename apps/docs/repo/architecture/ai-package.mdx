...
<Tabs>
  <Tab title="Model Selection">
    ...
    // Check model availability based on API keys
    if (isModelAvailable('claude-4-opus-20250514')) {
      // Model is available with configured API key
    }
    ```
...
<Warning>
  **Model Configuration Guidelines:** - Never hardcode model IDs in applications
  - use selection functions - Always check model availability before use with
  `isModelAvailable()` - Use task-based selection (`getBestModelForTask()`) for
  automatic optimization - Respect user tiers when selecting models with
