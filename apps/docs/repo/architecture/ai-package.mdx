...
The AI package includes a centralized model registry system that serves as the
single source of truth for all model configurations across providers.

### Model Registry Features

<CardGroup cols={2}>
  <Card title="Unified Configuration" icon="database">
    Single location for all model metadata, capabilities, and provider
    configurations
  </Card>
  <Card title="Smart Selection" icon="brain">
    Automatic model recommendations based on capabilities and user requirements
  </Card>
  <Card title="Type Safety" icon="shield">
    Comprehensive TypeScript interfaces for model metadata and capabilities
  </Card>
  <Card title="Dynamic Updates" icon="refresh">
    Easy model addition/updates without code changes across consuming
    applications
  </Card>
</CardGroup>
...
The centralized model system consists of three main components:

1. **Model Registry** - Central source of truth for all model metadata
2. **Selection Functions** - Intelligent model selection based on use cases
3. **Provider Integration** - Automatic configuration with proper settings

```mermaid
graph TB
    A[Model Registry<br/>metadata.ts] --> B[Selection Functions<br/>utils.ts]
    B --> C[Provider Registry<br/>registry.ts]
    C --> D[Configured Models<br/>Ready to use]

    E[User Request] --> B
    F[Environment Vars] --> C
    G[User Tier] --> B
    ...
    subgraph "Model Metadata"
        ...
        I[Capabilities] --> A
        J[Context Windows] --> A
        K[Reasoning Config] --> A
    end

    subgraph "Smart Selection"
        L[Strategy] --> B
        M[User Access] --> B
        N[Capabilities] --> B
    end
...
<Tabs>
  <Tab title="Model Selection">
    ...
    import { hasAnthropicConfig, hasOpenAIConfig, hasGoogleAIConfig } from '@repo/ai/env';
    ...
    // Strategic model selection with user tiers
    ...
    const recommendations = getModelRecommendations('coding', 'enterprise');

    // Check provider availability based on API keys
    if (hasAnthropicConfig()) {
      // Anthropic provider is available with configured API key
      const claudeModel = registry.languageModel('anthropic:claude-4-opus-20250514');
    }
    ```

  </Tab>

  <Tab title="Model Metadata">
    ```typescript
    import {
      getModelConfig,
    ...
    const config = getModelConfig('claude-4-opus-20250514');
    ...
    const hasReasoning = modelSupportsReasoning('claude-4-opus-20250514');
    const visionModels = getModelsByCapability('vision');
    ```

  </Tab>

  <Tab title="Provider Integration">
    ```typescript
    // The centralized registry automatically configures providers
    import { registry, models } from '@repo/ai/server';

    // Models are automatically configured with proper settings
    const claude4 = registry.languageModel('anthropic:claude-4-opus-20250514');
    // Automatically includes reasoning headers and budget tokens

    const gpt4 = registry.languageModel('openai:gpt-4o');
    // Standard configuration without reasoning
    ...
    const result = await generateText({
      ...
      // Reasoning is automatically enabled based on model configuration
    });
    ```

  </Tab>

  <Tab title="Dynamic Selection">
    ```typescript
    import {
      getModelsByCapability,
...
The registry tracks comprehensive capabilities for intelligent model selection:
...
type ModelCapability =
  ...
  | "computer-use" // Computer automation tools
  | "pdf-support" // PDF document processing
  | "search" // Web search integration
  | "code" // Code generation and analysis
  | "multimodal"; // Multiple input types

type ModelCostTier = "low" | "medium" | "high" | "very-high";
...
#### 1. Automatic Reasoning Configuration

Models with reasoning capabilities are automatically configured with proper
headers and settings:

```typescript
// Anthropic models automatically get reasoning headers
const claude4 = registry.languageModel("anthropic:claude-4-opus-20250514");
// Automatically includes:
// - headers: { 'anthropic-beta': 'interleaved-thinking-2025-05-14' }
// - budgetTokens: 15000

// O1 models get their specific configuration
const o1 = registry.languageModel("openai:o1-preview");
// Automatically configured for reasoning with lower temperature
```

#### 2. Provider-Agnostic Model Selection

```typescript
// Select best model regardless of provider
const bestReasoning = getBestModelForTask("reasoning");
// Returns: 'claude-4-opus-20250514' (or best available)

const bestVision = getBestModelForTask("vision");
// Returns: 'claude-4-sonnet-20250514' (or best available)
```
...
// Free users get access to appropriate models only
```
...
if (hasAnthropicConfig()) {
  const claude = registry.languageModel("anthropic:claude-4-opus-20250514");
}
...
<CardGroup cols={1}>
  <Card title="Consistency Across Applications">
    All applications using the AI package automatically stay in sync with the
    latest model configurations and capabilities.
  </Card>
  <Card title="Easy Model Updates">
    Adding new models or updating existing ones requires changes in only one
    location, automatically propagating to all consumers.
  </Card>
  <Card title="Intelligent Selection">
    Applications can automatically select the best model for specific tasks
    without hardcoding model names.
  </Card>
  <Card title="Reasoning Integration">
    Models with reasoning capabilities are automatically configured with proper
    headers and settings.
  </Card>
  <Card title="Cost Control">
    User tier integration ensures appropriate model access based on subscription
    levels and cost constraints.
  </Card>
  <Card title="Provider Flexibility">
    Switch between providers seamlessly without changing application code, just
...
The system supports multiple selection strategies for different use cases:
...
Track model usage for cost monitoring and optimization:

```typescript
import { trackModelUsage, getModelUsageStats } from "@repo/ai/models";
...
<Warning>
  **Model Configuration Guidelines:** - Never hardcode model IDs in applications
  - use selection functions - Always check provider availability before use with
  provider availability functions (`hasAnthropicConfig()`, `hasOpenAIConfig()`,
  etc.) - Use task-based selection (`getBestModelForTask()`) for automatic
  optimization - Respect user tiers when selecting models with
  `canUserAccessModel()` - Let the registry handle reasoning configuration
  automatically - Track model usage for cost monitoring with `trackModelUsage()`
</Warning>

### Adding New Models

To add a new model to the centralized registry:
...
2. **Update Provider Registry** if needed in
   `packages/ai/src/server/providers/registry.ts`
3. **Test** with `pnpm test` to ensure all functions work with the new model
4. **Deploy** - All applications automatically get access to the new model
