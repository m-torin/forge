...
if (isAvailable && modelConfig?.reasoning?.supported) {
  ...
  setBudgetTokens(modelConfig.reasoning.budgetTokens || 10000);
}
...
- `isModelAvailable(modelId)` - Check if API key is configured
...
- `models.language.reasoning()` - Best reasoning model
- `models.language.claude()` - Best Claude model
- `models.language.openai()` - Best OpenAI model
