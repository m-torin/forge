...
| `@repo/db-prisma`            | Prisma ORM + generated types   | 3     | ❌           | `import { db }`                           |
| `@repo/analytics`            | PostHog/GA with feature flags  | 4     | ✅           | `import { analytics }`                    |
| `@repo/observability`        | Sentry error tracking          | 2     | ✅           | `createObservability` from `/server/next` |
| `@repo/email`                | Email templates and sending    | 4     | ❌           | `import { sendEmail }`                    |
| `@repo/storage`              | Cloudflare R2/Images           | 5.5   | ✅           | `import { storage }`                      |
| `@repo/payments`             | Stripe integration             | 5     | ❌           | `import { stripe }`                       |
| `@repo/ai`                   | AI/ML functionality            | 5.5   | ✅           | `import { ai }`                           |
...
| `@repo/seo`                  | SEO utilities                  | 5     | ✅           | `import { seo }`                          |
| `@repo/links`                | Link management                | 5     | ✅           | `import { links }`                        |
...
├─ UI Component
│  ├─ Reusable across apps? → /packages/uix-system
...
└─ New Feature
   ├─ Needs database? → Start with schema in /packages/pkgs-databases/prisma
...
# Edit packages/db-prisma/src/prisma/schemas/*.prisma files, then:
pnpm migrate
...
pnpm --filter @repo/db-prisma db:status

# 2. If needed, reset database (CAUTION: Data loss!)
pnpm --filter @repo/db-prisma db:push --force-reset
...
# Open Prisma Studio
cd packages/db-prisma && pnpm dev
```
...
- [ ] **Client Regenerated**: `pnpm --filter @repo/db-prisma generate` completed
...
export async function handleWebhook(payload: unknown) {
  try {
    await logInfo("webhook.received", { kind: "payment", payload });
    // ...process payload
  } catch (error) {
    await logError("webhook.failed", { error });
    throw error;
  }
}
```
...
export function TrackButtonClicks() {
  ...
  return (
    <button
      onClick={() => {
        observability.logInfo('ui.button.clicked', {
          component: 'TrackButtonClicks',
        });
      }}
    >
      Track Action
