...
The documentation website is now available at `http://localhost:3800`.

### Custom Ports

In this project, we run Mintlify on port 3800. The default Mintlify port is 3000, but you can use
the `--port` flag to customize it. For example:

```bash
mintlify dev --port 3800
```
...
Each CLI is linked to a specific version of Mintlify. Please update the CLI if your local website
looks different than production.
