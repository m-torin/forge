...
async getLatestVersion(templateName: string) { return await
prisma.templateVersion.findFirst({ where: { templateName, status: 'approved', },
orderBy: { createdAt: 'desc', }, }); }

async compareVersions(templateName: string, version1: string, version2: string)
{ const [v1, v2] = await Promise.all([ this.getVersion(templateName, version1),
