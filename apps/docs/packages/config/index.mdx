...
<Tabs>
  <Tab title="Next.js Apps">
    ```typescript // eslint.config.ts import nextConfig from
    '@repo/config-eslint/next'; export default nextConfig; ```
  </Tab>
...
<Tab title="React Packages">
  ...
  <Tab title="Node.js Services">
    ```typescript // eslint.config.ts import serverConfig from
    '@repo/config-eslint/server'; export default serverConfig; ```
  </Tab>
</Tabs>
