...
// With expiration
await redis.set('temp:token', 'abc123', 'EX', 3600); // 1 hour

// Lists and queues
await redis.lpush(
  'notifications',
  JSON.stringify({ type: 'email', recipient: 'user@example.com' })
);
...
async function publishProductUpdate(productId: string, update: any) {
  ...
  await redis.publish('product:updates', JSON.stringify(message));
  
  // Also publish to organization-specific channel
  ...
  if (product) {
    await redis.publish(
      `org:${product.organizationId}:products`,
