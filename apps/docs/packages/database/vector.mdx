...
// Store product embedding await vector.upsert([{ id: 'product-123', vector:
...
// Find similar products const results = await vector.query({ vector:
...
import type {
  VectorMetadata,
  QueryOptions
} from "@repo/database/upstash/client";
import { logError, logInfo } from "@repo/observability/server/next";
