...
title: "Feature Flags"
description:
  "Comprehensive feature flag system with Flags SDK v4, PostHog, and Edge Config
  integration"
icon: "flag"
---
...
Comprehensive feature flag management using Flags SDK v4 with support for static
precomputation, edge middleware evaluation, PostHog integration, and Vercel Edge
Config for ultra-low latency.
...
<CardGroup cols={2}>
  <Card title="Flags SDK v4" icon="vercel">
    Full Flags SDK v4 implementation with static precomputation and edge
    middleware support
  </Card>
...
The feature flags package is included in the monorepo workspace:

```bash
# Already available as @repo/feature-flags
# No additional installation needed
```

For environment setup, see the [Environment Setup](#environment-setup) section.

## Quick Start

<CodeGroup>
```typescript Basic Flag Definition
import { flag } from '@repo/feature-flags/server/next';
...
# For flag evaluation (server-side)
...
The package provides helper functions for checking environment configuration:
...
if (isEncryptionEnabled()) {
  // Encryption is enabled in production with FLAGS_SECRET
}
```
...
import { precompute } from "@repo/feature-flags/server/next";
...
<Tabs>
  ...
  <Tab title="Build-time Generation">
    ```typescript
    import { generatePermutations } from '@repo/feature-flags/server/next';
    import { marketingFlags } from '../flags';
...
```typescript
import { dedupe } from "@repo/feature-flags/server/next";
