...
import { ObservabilityBuilder } from '@repo/observability';

// Host app const hostObservability = ObservabilityBuilder.create()
.withPlugin(createSentryMicroFrontendPlugin({ isHost: true, zones: [ { name:
'admin', dsn: process.env.SENTRY_DSN_ADMIN }, { name: 'dashboard', dsn:
process.env.SENTRY_DSN_DASHBOARD }, ], dsn: process.env.SENTRY_DSN,
useMultiplexedTransport: true, addZoneContext: true, })) .build();
...
// Child app
const childObservability = ObservabilityBuilder.create()
  .withPlugin(createSentryMicroFrontendPlugin({
    zone: 'admin',
...
// sentry.config.ts import { createSentryMicroFrontendPlugin } from
'@repo/observability/plugins/sentry-microfrontend'; import {
ObservabilityBuilder } from '@repo/observability';
...
<Steps>
  <Step title="Create centralized config">
    ...
    export const observability = ObservabilityBuilder.create()
      .withPlugin(createSentryMicroFrontendPlugin({
        zone: 'your-zone',
...
<ResponseField name="ObservabilityBuilder" type="class">
  Builder class for creating observability instances ```typescript
  ObservabilityBuilder.create(): ObservabilityBuilder
  builder.withPlugin(plugin): ObservabilityBuilder builder.build():
