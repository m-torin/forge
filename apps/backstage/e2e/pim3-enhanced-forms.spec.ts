...
/**
 * Tests specifically for the enhanced form features
 * Validates tabs, multi-selects, and complex relationship management
 */

const timestamp = Date.now();

test.describe('PIM3 Enhanced Form Features', () => {
  ...
  test('Multi-select functionality - search and selection', async ({ page }) => {
    ...
    if (chipCount > 0) {
      ...
      const newCount = await selectedChips.count();
  ...
  test('Dynamic attribute management', async ({ page }) => {
    ...
    // Verify Size is gone
    await expect(page.getByDisplayValue('Size')).not.toBeVisible();
