...
import { brandFormSchema, type BrandFormValues } from '@/schemas/pim3/brand';
...
export function BrandForm({ onClose, onSuccess, opened, brandId }: BrandFormProps) {
  ...
  const form = useForm<BrandFormValues>({
    validate: zodResolver(brandFormSchema),
    initialValues: {
      // Basic information
      name: '',
      slug: '',
  ...
  const loadOptions = async () => {
    try {
      const [brandsRes, productsRes, collectionsRes] = await Promise.all([
        ...
        findManyProductsAction({ take: 100 }),
  ...
  const handleSubmit = async (values: typeof form.values) => {
    ...
    try {
      ...
      let brandResult;
      if (isEditing && brandId) {
        brandResult = await updateBrandAction({
          where: { id: brandId },
          data: data,
        });
        ...
        brandResult = await createBrandAction({
          data: data,
        });
