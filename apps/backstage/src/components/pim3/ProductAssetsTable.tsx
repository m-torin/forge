...
import { createAsset, deleteAsset, getAssets } from '@/actions/pim3/actions';

import type { MediaType, Media } from '@repo/database/prisma';

interface ProductAssetWithProduct extends Media {
  product: {
    id: string;
    name: string;
    sku: string;
...
export function ProductAssetsTable() {
  ...
  const loadAssets = useCallback(async () => {
    ...
    try {
      const result = await getAssets({
        type: (typeFilter as MediaType) || undefined,
        limit: 10,
        page,
        search,
      });
      ...
      if (result.success && result.data) {
        // Map Media to ProductAssetWithProduct format
        const mappedAssets = (result.data as any[]).map((media: any) => ({
          ...media,
          product: media.product || { id: media.productId, name: 'Unknown', sku: 'N/A' },
        }));
  ...
  const getAssetIcon = (type: MediaType) => {
    switch (type) {
      case 'IMAGE':
    ...
    <Stack>
      <Group justify="space-between">
        <Group>
          ...
          />
          <Select
            onChange={(value) => setTypeFilter(value as MediaType | '')}
      ...
      <Modal
        ...
        size="lg"
        title={previewAsset?.url?.split('/').pop() || 'Asset'}
      >
        {previewAsset && (
          <Stack>
            {previewAsset.type === 'IMAGE' && (
              <Image
                alt={previewAsset.altText || 'Asset'}
                fit="contain"
                mah={400}
            ...
            {(previewAsset.copy as any)?.description && (
              <Text size="sm">{(previewAsset.copy as any).description}</Text>
            )}
