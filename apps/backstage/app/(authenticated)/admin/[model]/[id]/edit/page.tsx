...
export default async function EditModelPage({ params }: PageProps) {
  ...
  const fieldsWithOptions = await Promise.all(
    config.fields.map(async (field, index) => {
      ...
      if (field.type === 'select' && field.name.endsWith('Id')) {
        try {
          const currentValue = record[field.name] as string | undefined;
  ...
  // Transform record data for form
  const initialValues = { ...record };

  // Handle JSON fields
  config.fields.forEach((field) => {
    if (field.type === 'json' && record[field.name]) {
      initialValues[field.name] = JSON.stringify(record[field.name], null, 2);
    }
  });
  ...
  async function handleSubmit(values: Record<string, any>) {
    ...
    await updateRecord(params.model, params.id, data);
