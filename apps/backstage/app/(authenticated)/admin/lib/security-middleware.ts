...
class AuditLogger {
  ...
  static async log(entry: AuditEntry): Promise<void> {
    ...
    console.log(`[AUDIT] ${entry.severity.toUpperCase()}: ${entry.action} on ${entry.modelName}${entry.fieldName ? `.${entry.fieldName}` : ''} by user ${entry.userId}`);
  }
  ...
  static getRecentLogs(limit: number = 100): AuditEntry[] {
    ...
    modelConfig.rules.forEach(rule => {
      if (filtered[rule.fieldName] !== undefined) {
        ...
        if (!hasPermission) {
          ...
          filtered[rule.fieldName] = maskSensitiveData(filtered[rule.fieldName], rule.maskingStrategy);
        }
      }
    });
...
export async function auditFieldAccess(
  ...
  const rule = getFieldSecurityRule(modelName, fieldName);
  const severity = rule?.sensitivity === 'high' ? 'high' : 
                  rule?.sensitivity === 'medium' ? 'medium' : 'low';
  ...
  await AuditLogger.log({
    ...
    recordId,
    fieldName,
    oldValue: oldValue ? SecurityHelpers.sanitizeForLogging({ [fieldName]: oldValue }, modelName)[fieldName] : undefined,
    newValue: newValue ? SecurityHelpers.sanitizeForLogging({ [fieldName]: newValue }, modelName)[fieldName] : undefined,
...
export function createSecurityMiddleware(modelName: string) {
  return async function securityMiddleware(
    ...
    if (modelConfig?.requiresPermission && !context.permissions.includes(modelConfig.requiresPermission)) {
      await AuditLogger.log({
        timestamp: new Date().toISOString(),
        userId: context.userId,
