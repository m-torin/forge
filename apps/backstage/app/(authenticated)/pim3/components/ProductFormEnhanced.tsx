...
import { Product, ProductStatus, ProductType, BarcodeType } from '@repo/database/prisma';
...
export function ProductFormEnhanced({
  ...
  const loadProduct = async (id: string) => {
    setLoading(true);
    try {
      const productData = await getProductAction(id);
      if (productData) {
        ...
        if (relationships.success && relationships.data) {
          ...
          form.setValues({
            ...
            // Relationships
            collectionIds: relationships.data.collections.map((c) => c.id),
            taxonomyIds: relationships.data.taxonomies.map((t) => t.id),
            brandIds: relationships.data.brands.map((b) => b.id),
