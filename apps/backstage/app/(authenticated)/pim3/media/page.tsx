...
function MediaStatsCard({ stats }: { stats: MediaStats }) {
  ...
  const imagePercentage = totalFiles > 0 ? ((stats.byType.IMAGE || 0) / totalFiles) * 100 : 0;
  const videoPercentage = totalFiles > 0 ? ((stats.byType.VIDEO || 0) / totalFiles) * 100 : 0;
  const documentPercentage = totalFiles > 0 ? ((stats.byType.DOCUMENT || 0) / totalFiles) * 100 : 0;
  const audioPercentage = totalFiles > 0 ? ((stats.byType.AUDIO || 0) / totalFiles) * 100 : 0;
