...
export async function getRegistryAnalytics(): Promise<{
  ...
  try {
    const [registryCounts, registriesByType, monthlyStats, topProducts, userEngagement] =
      await Promise.all([
        // Basic counts
    ...
    const itemStats = await prisma.registryItem.count({
      where: { deletedAt: null },
    });

    const purchasedItems = await prisma.registryItem.count({
      where: { deletedAt: null, purchased: true },
    });

    const activeRegistries = await prisma.registry.count({
      where: {
        deletedAt: null,
    ...
    const analytics: RegistryAnalytics = {
      ...
      purchasedItems,
      registriesByType: (() => {
        const initialCounts: Record<RegistryType, number> = {
          BABY: 0,
          BIRTHDAY: 0,
          GIFT: 0,
          HOLIDAY: 0,
        ...
        return (registriesByType as { type: RegistryType; _count: number }[]).reduce(
          (acc, item) => {
...
export async function updateRegistryUserRole(data: z.infer<typeof registryUserRoleSchema>) {
  try {
    ...
    await prisma.registryUserJoin.upsert({
      create: {
...
export async function removeRegistryUser(registryId: string, userId: string) {
  try {
    await prisma.registryUserJoin.delete({
      where: {
        userId_registryId: {
          registryId,
          userId,
...
export async function bulkAddRegistryItems(
  ...
  try {
    ...
    const createdItems = await prisma.registryItem.createMany({
      data: itemsToCreate,
    });
...
export async function recordItemPurchase(data: {
  ...
  try {
    const purchase = await prisma.registryPurchaseJoin.create({
      data: {
...
export async function updatePurchaseStatus(
  ...
  try {
    const purchase = await prisma.registryPurchaseJoin.update({
      data: {
        status,
        ...trackingInfo,
      },
...
export async function getRegistryItemAnalytics(registryId?: string) {
  try {
    ...
    const [
      ...
      prisma.$queryRaw`
        ...
        WHERE ri."deletedAt" IS NULL 
          ...
          ${registryId ? `AND ri."registryId" = '${registryId}'` : ''}
    ...
    const analytics = {
      ...
      itemsByPriority: (itemsByPriority as { priority: number; _count: number }[]).map((item) => ({
        count: item._count,
        priority: item.priority,
      })),
      ...
      purchasedItems,
      topCollections: (topCollections as { collectionId: string; _count: number }[]).map((item) => {
        const collection = collectionDetails.find((c) => c.id === item.collectionId);
        return {
          collectionId: item.collectionId,
          collectionName: collection?.name || 'Unknown Collection',
          collectionType: collection?.type,
          timesAdded: item._count,
        };
      }),
      topProducts: (topProducts as { productId: string; _count: number }[]).map((item) => {
