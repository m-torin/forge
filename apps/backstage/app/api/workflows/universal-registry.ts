...
export const fetchProductsStep = compose(
  ...
  (step) => withStepMonitoring(step, {
    ...
    customMetrics: ['productCount', 'sourceCount'],
  })
);
...
export const deduplicateProductsStep = createStep(
  ...
  async (data: any) => {
    ...
    return {
      ...
      deduplicationStats: {
        ...
        deduplicationRate: 
          ((allProducts.length - deduplicationResults.unique.length - deduplicationResults.mergedProducts.length) / 
...
function mergeProducts(products: any[]): any {
  ...
  const sorted = products.sort((a, b) => 
    new Date(b.source.lastUpdated).getTime() - new Date(a.source.lastUpdated).getTime()
  );
...
export const enrichProductsStep = StepTemplates.conditional(
  ...
  {
    condition: (data: any) => data.options.enrichmentProviders?.length > 0,
...
export const validateProductsStep = createStep(
  'validate-products',
  async (data: any) => {
    const { enrichedProducts, deduplicationResults, options } = data;
    const products = enrichedProducts || [...deduplicationResults.unique, ...deduplicationResults.mergedProducts];
    ...
    const rules = options.validationRules || [
      ...
      { field: 'price.amount', rule: 'range', params: { min: 0, max: 1000000 } },
...
export const updateIndexesStep = createStep(
  'update-indexes',
  async (data: any) => {
...
export const generateRegistryReportStep = createStep(
  'generate-report',
  async (data: any) => {
    const { operation, sources, validationStats, deduplicationStats, indexes } = data;
