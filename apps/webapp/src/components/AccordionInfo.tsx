...
const AccordionInfo: FC<Props> = ({
  ...
  data = DEMO_DATA,
  'data-testid': testId = 'accordion-info',
}) => {
  return (
    <div className="w-full space-y-2.5 rounded-2xl" data-testid={testId}>
      {/* ============ */}
      {data.map((item, index) => {
        return (
          <Disclosure key={`accordion-${item.name}`} defaultOpen={index < 2}>
            {({ open }) => (
              <div>
