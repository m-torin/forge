...
export default async function Page({ params }: { params: Promise<{ handle: string }> }) {
  const { handle } = await params;
  const blogPost = await getBlogPostsByHandle(handle);
  ...
  const renderContent = () => {
    ...
    return (
      <div
        ...
        className="prose prose-sm sm:prose lg:prose-lg dark:prose-invert mx-auto max-w-(--breakpoint-md)!"
      >
  ...
  return (
    <div className="pt-8 lg:pt-16">
      ...
      <div className="container flex flex-col gap-y-10">
        {renderContent()}
        {renderTags()}
        <div className="mx-auto w-full max-w-(--breakpoint-md) border-t border-b border-neutral-100 dark:border-neutral-700" />
        {renderAuthor()}
        {renderCommentForm()}
      </div>
