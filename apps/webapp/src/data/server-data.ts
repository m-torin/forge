...
import { ContentStatus, ProductStatus, actions } from '@repo/database/prisma/server/next';
...
export const getProducts = unstable_cache(
  async (): Promise<WebappProduct[]> => {
    try {
      const products = await actions.findManyProductsAction({
        where: {
...
export const getProductByHandle = cache(async (handle: string): Promise<WebappProduct | null> => {
  try {
    const product = await actions.findUniqueProductAction({
      where: { slug: handle },
      include: {
        media: {
          orderBy: { sortOrder: 'asc' },
...
export const getCollections = unstable_cache(
  async (): Promise<WebappCollection[]> => {
    try {
      const collections = await actions.findManyCollectionsAction({
        where: {
...
export const getGroupCollections = unstable_cache(
  async (): Promise<Array<{ name: string; collections: WebappCollection[] }>> => {
    try {
      const groupCollections = await actions.findManyCollectionsAction({
        where: {
          status: ContentStatus.PUBLISHED,
          deletedAt: null,
...
export const getCollectionByHandle = cache(
  async (handle: string): Promise<WebappCollection | null> => {
    try {
      const collection = await actions.findUniqueCollectionAction({
        where: { slug: handle },
        include: {
          media: {
            orderBy: { sortOrder: 'asc' },
...
export const getProductReviews = cache(async (handle: string): Promise<WebappReview[]> => {
  try {
    ...
    const reviewsResponse = await actions.getReviewsAction({
...
export const getBlogPosts = unstable_cache(
  async (): Promise<WebappBlogPost[]> => {
    try {
      const articles = await actions.findManyArticlesAction({
        where: {
          status: ContentStatus.PUBLISHED,
          deletedAt: null,
        },
...
export const getBlogPostByHandle = cache(async (handle: string): Promise<WebappBlogPost | null> => {
  try {
    const article = await actions.findUniqueArticleAction({
      where: { slug: handle },
      include: {
        media: {
          orderBy: { sortOrder: 'asc' },
