...
test.describe('Chat activity', () => {
  ...
  test('Hide suggested actions after sending message', async () => {
    ...
    // Assert that suggested actions are hidden
    const suggestedActions = chatPage.getElementByTestId('suggested-actions');
    await expect(suggestedActions).toBeHidden();
  });
  ...
  test('Upvote message', async () => {
    ...
    // Assert that the upvote action was completed successfully
    const upvoteButton = assistantMessage.element.getByTestId('message-upvote');
  ...
  test('Downvote message', async () => {
    ...
    // Assert that the downvote action was completed successfully
    const downvoteButton = assistantMessage.element.getByTestId('message-downvote');
  ...
  test('Update vote', async () => {
    ...
    // Assert that the vote update was completed successfully
    const downvoteButton = assistantMessage.element.getByTestId('message-downvote');
    await expect(downvoteButton).toBeVisible();
  });
  ...
  test('auto-scrolls to bottom after submitting new messages', async () => {
    ...
    // Assert that we are scrolled to the bottom
  ...
  test('scroll button appears when user scrolls up, hides on click', async () => {
    await chatPage.sendMultipleMessages(5, i => `filling message #${i}`);
    await expect(chatPage.scrollToBottomButton).toBeHidden();
