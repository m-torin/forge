import { CrossSmallIcon, LoaderIcon, TerminalWindowIcon } from '#/components/icons';
...
export function Console({ consoleOutputs, setConsoleOutputs }: ConsoleProps) {
  ...
  const [isResizing, toggleResizing] = useToggle([false, true]);
  ...
  // Use conditional event listeners for better performance
  React.useEffect(() => {
  ...
  return consoleOutputs.length > 0 ? (
    <>
      <div
        className={`fixed z-[${Z_INDEX.PANEL}] h-2 w-full cursor-ns-resize`}
      ...
      >
        <div
          className={`sticky top-0 z-[${Z_INDEX.STICKY}] flex h-fit w-full flex-row items-center justify-between border-b border-zinc-200 bg-muted px-2 py-1 dark:border-zinc-700`}
        >
            ...
            >
              ...
              {['in_progress', 'loading_packages'].includes(consoleOutput.status) ? (
                ...
                <div className="flex w-full flex-col gap-2 overflow-x-scroll text-zinc-900 dark:text-zinc-50">
                  {consoleOutput.contents.map((content, _index) =>
                    content.type === 'image' ? (
                      <div key={`${consoleOutput.id}-image-${content.value.slice(-10)}`}>
                        <Image
                          src={content.value}
                          alt="Console output"
                          width={400}
                      ...
                      <div
                        key={`${consoleOutput.id}-text-${content.value.slice(0, 10)}`}
