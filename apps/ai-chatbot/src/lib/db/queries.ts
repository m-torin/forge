...
/**
 * Creates a temporary guest user with generated credentials
 ...
 * @param userId - ID of the chat owner
 * @param title - Chat title
 * @param visibility - Chat visibility setting
 * @returns Promise resolving to database insert result
 */
 ...
 * @returns Promise resolving to deleted chat data
 */
...
/**
 * Retrieves paginated chats for a specific user
 * @param id - User ID
 * @param limit - Maximum number of chats to return
 * @param startingAfter - Cursor for pagination (after this chat)
 * @param endingBefore - Cursor for pagination (before this chat)
 * @returns Promise resolving to paginated chat results
 */
...
export async function getMessagesByChatIdPaginated({
  ...
  try {
    ...
    throw new ChatSDKError('bad_request:database', 'Failed to get paginated messages');
  }
}

export async function getMessageCountByChatId({ chatId }: { chatId: string }) {
  try {
