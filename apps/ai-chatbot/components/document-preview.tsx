...
import { Image, Loader2, Maximize } from 'lucide-react';
...
// Local document type with missing properties from database schema
type DocumentWithKind = Document & {
...
export function DocumentPreview({ isReadonly, result, args }: DocumentPreviewProps) {
  ...
  const document: DocumentWithKind | null = previewDocument
    ? {
        ...previewDocument,
        kind: 'text' as const, // Default kind since column is missing
        contentHtml: null,
...
const DocumentContent = ({ document }: { document: DocumentWithKind }) => {
