...
export const Context = ({
  ...
  const safeMax = Math.max(0, Number.isFinite(maxOutputTokens) ? maxOutputTokens : 0);
  const safeUsed = Math.min(Math.max(0, Number.isFinite(usedTokens) ? usedTokens : 0), safeMax);
  const usedPercent =
    safeMax > 0 ? Math.min(PERCENT_MAX, Math.max(0, (safeUsed / safeMax) * PERCENT_MAX)) : 0;
