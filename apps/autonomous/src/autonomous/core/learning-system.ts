...
import { WorkflowSpecification, ErrorAnalysis, RepairAttempt, RepairStrategy, LearningEvent, ErrorCategory } from '../types';
...
export class LearningSystem {
  ...
  private loadPatterns(): void {
    if (existsSync(this.patternsPath)) {
      try {
        ...
        this.patterns = new Map(Object.entries(data).map(([k, v]: [string, any]) => [
          k,
  ...
  async recordSuccess(spec: WorkflowSpecification, iterations: number, attempts: RepairAttempt[]): Promise<void> {
  ...
  async getPredictedStrategy(context: { workflowType: string; complexity: string }): Promise<RepairStrategy> {
  ...
  async getRepairStrategy(errorAnalysis: ErrorAnalysis): Promise<RepairStrategy> {
    ...
    const strategyData = this.strategies.get(bestStrategy) || this.strategies.get('incremental-fix')!;
  ...
  async generateInsights(spec: WorkflowSpecification, attempts: RepairAttempt[]): Promise<string> {
    ...
    insights.push(`- System learning rate: ${(learningRate * 100).toFixed(1)}% improvement over last 100 workflows`);
  ...
  private getStrategiesForCategory(category: ErrorCategory): Array<{ strategy: string; score: number }> {
    const categoryStrategies: Record<ErrorCategory, Array<{ strategy: string; score: number }>> = {
      ...
      'type-error': [{ strategy: 'type-focused', score: 0.9 }, { strategy: 'contract-alignment', score: 0.7 }],
      ...
      'logic-error': [{ strategy: 'incremental-fix', score: 0.8 }, { strategy: 'test-driven', score: 0.6 }],
    ...
    for (const [group, patterns] of patternGroups.entries()) {
      if (patterns.length >= this.PATTERN_DETECTION_THRESHOLD) {
        ...
        if (avgConfidence >= this.MIN_CONFIDENCE_THRESHOLD) {
          console.log(`🔍 New pattern detected: ${group} (confidence: ${(avgConfidence * 100).toFixed(1)}%)`);
  ...
  private generateRecommendations(spec: WorkflowSpecification, attempts: RepairAttempt[]): string[] {
  ...
  getSystemMetrics(): any {
    ...
    return {
      ...
      averageIterations: this.events.reduce((sum, e) => sum + e.iterations, 0) / Math.max(totalEvents, 1),
