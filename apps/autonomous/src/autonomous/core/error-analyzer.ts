...
export class ErrorAnalyzer {
  ...
  private errorPatterns = new Map<RegExp, { category: ErrorCategory; priority: number; confidence: number }>([
    // TypeScript errors
    [/TypeError.*is not a function/, { category: 'type-error', priority: 1, confidence: 0.95 }],
    [/ReferenceError.*is not defined/, { category: 'reference-error', priority: 1, confidence: 0.98 }],
    [/SyntaxError/, { category: 'syntax-error', priority: 1, confidence: 1.0 }],
    [/Type '.*' is not assignable to type/, { category: 'type-error', priority: 2, confidence: 0.95 }],
    [/Property '.*' does not exist on type/, { category: 'type-error', priority: 2, confidence: 0.95 }],
    ...
    // Contract violations
    [/Expected.*but got|Expected.*Received/, { category: 'contract-violation', priority: 2, confidence: 0.9 }],
    [/Schema validation failed/, { category: 'contract-violation', priority: 1, confidence: 0.95 }],
    [/Invalid input|Invalid output/, { category: 'contract-violation', priority: 1, confidence: 0.9 }],
    ...
    // Performance issues
    [/Timeout|TimedOut|exceeded.*timeout/, { category: 'performance-issue', priority: 4, confidence: 0.95 }],
    [/Performance budget exceeded/, { category: 'performance-issue', priority: 4, confidence: 0.9 }],
    [/Slow test detected/, { category: 'performance-issue', priority: 5, confidence: 0.85 }],
    
    // Network/Infrastructure
    [/Network|fetch|ECONNREFUSED|ETIMEDOUT/, { category: 'network-error', priority: 3, confidence: 0.9 }],
    [/Connection refused|Connection timeout/, { category: 'network-error', priority: 3, confidence: 0.95 }],
  ...
  private repairStrategies = new Map<ErrorCategory, string>([
    ['syntax-error', 'Fix syntax issues: Check for missing brackets, semicolons, or invalid syntax'],
    ['type-error', 'Update TypeScript types: Ensure type definitions match usage and add type assertions where needed'],
    ['reference-error', 'Add missing declarations: Import required modules or define missing variables'],
    ['import-error', 'Fix imports: Check module paths and ensure dependencies are installed'],
    ['contract-violation', 'Align with contracts: Update implementation to match input/output specifications'],
    ['logic-error', 'Fix business logic: Review step execution order and conditional logic'],
    ['performance-issue', 'Optimize performance: Add caching, reduce complexity, or increase timeouts'],
    ['network-error', 'Add error handling: Implement retry logic and proper error boundaries'],
    ['async-error', 'Fix async/await: Ensure proper promise handling and async function declarations']
  ]);
  ...
  async analyzeFailures(testResult: TestResult): Promise<ErrorAnalysis> {
    ...
    const allFailures = [
      ...testResult.vitest.failures,
      ...testResult.playwright.failures
    ];
  ...
  private async performRootCauseAnalysis(errors: any[], failures: TestFailure[]): Promise<string[]> {
  ...
  private async generateRepairStrategy(categories: ErrorCategory[], rootCauses: string[]): Promise<string> {
  ...
  private prioritizeStrategies(strategies: string[], categories: ErrorCategory[]): string[] {
    ...
    if (minPriority === 1) {
      ...
      return strategies.filter(s => 
        s.includes('syntax') || s.includes('import') || s.includes('reference')
      ).concat(strategies.filter(s => 
        !s.includes('syntax') && !s.includes('import') && !s.includes('reference')
      ));
    }
  ...
  private estimateRepairComplexity(categories: ErrorCategory[], rootCauses: string[]): 'low' | 'medium' | 'high' {
