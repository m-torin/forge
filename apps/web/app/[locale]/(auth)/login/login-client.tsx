...
import { authClient, signIn, sendMagicLink } from '@repo/auth/client/next';
...
export default function LoginClient({ dict, locale }: LoginClientProps) {
  ...
  const handleSubmit = async (values: LoginFormData) => {
    try {
      ...
      // Login error handled by notification above
      notifications.show({
        title: 'Error',
  ...
  const handleSocialLogin = async (provider: 'google' | 'facebook' | 'twitter') => {
    try {
      ...
      // Social login error handled by notification above
      notifications.show({
  ...
  const handleMagicLink = async (values: MagicLinkFormData) => {
    try {
      ...
      // Magic link error handled by notification above
      notifications.show({
        title: 'Error',
        ...
        <div className="mx-auto flex max-w-md flex-col gap-y-6">
          ...
          <Tabs defaultValue="password" variant="outline">
            ...
            <Tabs.Panel value="magic-link" pt="md">
              {!magicLinkSent ? (
                <form
                  onSubmit={magicLinkForm.onSubmit(handleMagicLink)}
