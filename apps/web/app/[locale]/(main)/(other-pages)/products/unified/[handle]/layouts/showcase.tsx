    ...
    <div className="showcase-layout">
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="absolute inset-0 z-0">
          {hasVideo && isPlaying ? (
            <video autoPlay loop className="w-full h-full object-cover" muted src={product.video} />
          ) : (
            images[0] && (
              <Image
        ...
        <div className="relative z-10 container text-center text-white px-4">
          <div className="max-w-3xl mx-auto space-y-6">
            {/* Category */}
            {product.category && (
              <p className="text-sm uppercase tracking-widest opacity-80">{product.category}</p>
            )}
            ...
            {/* Tagline */}
            {product.tagline && (
              <p className="text-xl md:text-2xl font-light opacity-90">{product.tagline}</p>
            )}
            ...
            {hasVideo && !isPlaying && (
              ...
              >
                <IconPlayerPlay className="w-12 h-12 border border-white/50 rounded-full p-3" />
            ...
            <div className="absolute bottom-8 left-1/2 -translate-x-1/2 animate-bounce">
              <IconChevronDown className="w-6 h-6 text-white/60" />
            </div>
          </div>
        </div>
      </section>
            ...
            {product.keyFeatures && (
              <div className="grid md:grid-cols-3 gap-8 mt-12">
                {product.keyFeatures.map((feature: any) => (
                  <div key={feature.title || feature} className="text-center space-y-4">
                ...
                <div className="mt-6 space-y-6">
                  ...
                  {product.materials && (
                    <div>
                      ...
                      <p className="text-neutral-600 dark:text-neutral-400">{product.materials}</p>
                    </div>
                  )}
                  ...
                  {product.dimensions && (
                    <div>
                      ...
                      <p className="text-neutral-600 dark:text-neutral-400">{product.dimensions}</p>
                    </div>
                  )}
                  ...
                  {product.care && (
                    <div>
                      ...
                      <p className="text-neutral-600 dark:text-neutral-400">{product.care}</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              {product.sustainability && (
                <div className="border-t pt-8">
                  <div className="flex items-start gap-4">
                    <IconStar className="w-6 h-6 text-green-600 mt-1" />
                    <div>
                      <h3 className="text-lg font-medium mb-2">
            ...
            <div className="lg:sticky lg:top-24 lg:h-fit">
              <div className="bg-neutral-50 dark:bg-neutral-800 rounded-2xl p-8 space-y-6">
                ...
                <div>
                  ...
                  {product.status && <ProductStatus className="mt-2" status={product.status} />}
                </div>
                ...
                {product.rating && (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-neutral-500">
                      {product.rating} ({product.reviewCount} {dict.product?.reviews || 'reviews'})
                    </span>
                  </div>
                )}
                ...
                {product.variants && product.variants.length > 1 && (
                  <div>
                    <Label className="mb-3">{dict.product?.selectOption || 'Select Option'}</Label>
                    <Select
                      onChange={(value) => {
                        const variant = product.variants.find((v: any) => v.id === value);
                        setSelectedVariant(variant);
                      }}
                      placeholder={dict.product?.chooseVariant || 'Choose variant'}
                ...
                {/* Quantity */}
                <div>
                  <Label className="mb-3">{dict.product?.quantity || 'Quantity'}</Label>
                  <NcInputNumber defaultValue={1} max={product.stock || 99} min={1} />
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex gap-3">
                    <LikeSaveBtnsWithFavorites productId={product.id} productName={product.title} />
                ...
                <div className="border-t pt-6 space-y-3 text-sm">
                  <div className="flex items-center gap-3">
                    <IconTruck className="w-5 h-5 text-neutral-500" />
                    <span>{dict.product?.freeShipping || 'Free shipping worldwide'}</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <IconShieldCheck className="w-5 h-5 text-neutral-500" />
                    <span>{dict.product?.warranty || '2-year warranty'}</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <IconRepeat className="w-5 h-5 text-neutral-500" />
        ...
        <section className="py-20">
          <div className="container">
            <div className="text-center mb-12">
              <Heading>{dict.product?.completeTheLook || 'Complete the Look'}</Heading>
              <p className="mt-4 text-neutral-600 dark:text-neutral-400">
                {dict.product?.curatedSelection || 'Curated selection to complement your style'}
              </p>
            </div>
