...
import { useAuth, revokeUserSession } from '@repo/auth/client/next';
...
function SessionManagerInner() {
  ...
  const loadSessions = async () => {
    try {
      setLoading(true);
      // TODO: Session management requires admin API or a dedicated session list endpoint
  ...
  return (
    <Card shadow="sm" radius="md" withBorder>
      <Group justify="space-between" mb="md">
        ...
        <Button variant="subtle" size="sm" onClick={loadSessions} loading={loading}>
          Refresh
        </Button>
      </Group>
      ...
      <Card bg="blue.1" mt="md" p="md">
        <Group gap="xs">
          <IconClock size={16} />
          <Text size="sm">Sessions expire after 30 days of inactivity</Text>
        </Group>
      </Card>
    </Card>
  );
      ...
      fallback={
        <Card shadow="sm" radius="md" withBorder p="md">
          <Stack gap="md" ta="center">
            ...
            <Title order={3}>Session Manager Unavailable</Title>
            <Text c="dimmed">
              We're unable to load your session information right now. Please try refreshing the
              page.
            </Text>
