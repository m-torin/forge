...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  // Product pages
  const productPages: MetadataRoute.Sitemap = products.products.flatMap(
    (product: any) =>
      languages.map((lang) => ({
  ...
  // Brand pages
  const brandPages: MetadataRoute.Sitemap = brands.flatMap((brand: any) =>
    languages.map((lang) => ({
  ...
  // Event pages
  const eventPages: MetadataRoute.Sitemap = events.flatMap((event: any) =>
    languages.map((lang) => ({
  ...
  // Blog post pages
  const blogPages: MetadataRoute.Sitemap = blogPosts.flatMap((post: any) =>
    languages.map((lang) => ({
