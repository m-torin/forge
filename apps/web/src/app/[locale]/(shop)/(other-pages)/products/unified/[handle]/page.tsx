...
export async function generateMetadata({
  params,
}: LocalizedUnifiedPDPProps): Promise<Metadata> {
  ...
  return {
    ...
    other: {
      ...
      "product:availability":
        status === dict.product.inStock ? "in stock" : "out of stock",
...
export default async function UnifiedPDPPage({
  params,
}: LocalizedUnifiedPDPProps) {
  const { handle, locale } = await params;
  const dict = await getDictionary(locale);
  const product = await getProductDetailByHandle(handle);
  ...
  return (
    <div>
      {/* Pinterest Rich Pin Structured Data */}
      <JsonLd id={`product-${handle}`} data={productStructuredData} />
