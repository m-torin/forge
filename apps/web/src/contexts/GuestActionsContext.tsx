...
export function GuestActionsProvider({
  ...
  const [cartItems, setCartItems] = useLocalStorage<string[]>({
    defaultValue: [],
    key: listConfigs.cart,
  });
  const [optimisticCartItems, updateOptimisticCartItems] = useOptimistic(
    cartItems,
    (
      state: string[],
      { action, itemId }: { action: "add" | "remove" | "clear"; itemId?: string },
    ) => {
      switch (action) {
        case "add":
          return itemId && !state.includes(itemId) ? [...state, itemId] : state;
        case "remove":
  ...
  // Comparison hooks
  const [comparisonItems, setComparisonItems] = useLocalStorage<string[]>({
    defaultValue: [],
    key: listConfigs.comparison,
  });
  const [optimisticComparisonItems, updateOptimisticComparisonItems] = useOptimistic(
    comparisonItems,
    (
      state: string[],
      { action, itemId }: { action: "add" | "remove" | "clear"; itemId?: string },
  ...
  // Favorites hooks
  const [favoritesItems, setFavoritesItems] = useLocalStorage<string[]>({
    defaultValue: [],
    key: listConfigs.favorites,
  });
  const [optimisticFavoritesItems, updateOptimisticFavoritesItems] = useOptimistic(
    favoritesItems,
    (
      state: string[],
      { action, itemId }: { action: "add" | "remove" | "clear"; itemId?: string },
  ...
  // Later hooks
  const [laterItems, setLaterItems] = useLocalStorage<string[]>({
    defaultValue: [],
    key: listConfigs.later,
  });
  const [optimisticLaterItems, updateOptimisticLaterItems] = useOptimistic(
    laterItems,
    (
      state: string[],
      { action, itemId }: { action: "add" | "remove" | "clear"; itemId?: string },
  ...
  // Registry hooks
  const [registryItems, setRegistryItems] = useLocalStorage<string[]>({
    defaultValue: [],
    key: listConfigs.registry,
  });
  const [optimisticRegistryItems, updateOptimisticRegistryItems] = useOptimistic(
    registryItems,
    (
      state: string[],
      { action, itemId }: { action: "add" | "remove" | "clear"; itemId?: string },
  ...
  // Shopping hooks
  const [shoppingItems, setShoppingItems] = useLocalStorage<string[]>({
    defaultValue: [],
    key: listConfigs.shopping,
  });
  const [optimisticShoppingItems, updateOptimisticShoppingItems] = useOptimistic(
    shoppingItems,
    (
      state: string[],
      { action, itemId }: { action: "add" | "remove" | "clear"; itemId?: string },
  ...
  // Create listHooks object
  const listHooks = useMemo(() => ({
    cart: {
      items: cartItems,
      optimisticItems: optimisticCartItems,
      setter: setCartItems,
      updateOptimisticItems: updateOptimisticCartItems,
    },
    comparison: {
      items: comparisonItems,
      optimisticItems: optimisticComparisonItems,
      setter: setComparisonItems,
      updateOptimisticItems: updateOptimisticComparisonItems,
    },
    favorites: {
      items: favoritesItems,
      optimisticItems: optimisticFavoritesItems,
      setter: setFavoritesItems,
      updateOptimisticItems: updateOptimisticFavoritesItems,
    },
    later: {
      items: laterItems,
      optimisticItems: optimisticLaterItems,
      setter: setLaterItems,
      updateOptimisticItems: updateOptimisticLaterItems,
    },
    registry: {
      items: registryItems,
      optimisticItems: optimisticRegistryItems,
      setter: setRegistryItems,
      updateOptimisticItems: updateOptimisticRegistryItems,
    },
    shopping: {
      items: shoppingItems,
      optimisticItems: optimisticShoppingItems,
      setter: setShoppingItems,
      updateOptimisticItems: updateOptimisticShoppingItems,
    },
  }), [
    cartItems, optimisticCartItems, setCartItems, updateOptimisticCartItems,
    comparisonItems, optimisticComparisonItems, setComparisonItems, updateOptimisticComparisonItems,
    favoritesItems, optimisticFavoritesItems, setFavoritesItems, updateOptimisticFavoritesItems,
    laterItems, optimisticLaterItems, setLaterItems, updateOptimisticLaterItems,
    registryItems, optimisticRegistryItems, setRegistryItems, updateOptimisticRegistryItems,
    shoppingItems, optimisticShoppingItems, setShoppingItems, updateOptimisticShoppingItems,
  ]);
...
[listHooks, guestId, startTransition, isMobile, isTablet],
  );
...
[
  ...
  const analyticsOperations = useMemo(
    ...
    [trackAnalyticsEvent, guestId, pathname],
  );
