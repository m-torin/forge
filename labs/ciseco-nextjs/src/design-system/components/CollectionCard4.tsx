...
const CollectionCard4: FC<CollectionCard4Props> = ({ bgSvgUrl, className, collection }) => {
  ...
  return (
    <div
      className={clsx(
        'hover:nc-shadow-lg group relative overflow-hidden rounded-3xl bg-white p-5 transition-shadow sm:p-8 dark:bg-neutral-900',
        className
      )}
    >
      {bgSvgUrl && (
        <div className="absolute bottom-0 end-0 size-52 sm:size-64 xl:size-72">
          <Image className="object-bottom-right object-contain opacity-60" alt="bgSVG" fill src={bgSvgUrl} />
        </div>
      )}
          ...
          {collection.image?.src && (
            <div className={clsx('relative size-20 overflow-hidden rounded-full', collection.color)}>
              <div className="absolute inset-4">
                <Image className="object-cover" alt={collection.image?.alt} fill sizes="80px" src={collection.image} />
              </div>
            </div>
          )}
          <ArrowUpRightIcon className="size-6 text-neutral-500 transition-transform group-hover:-translate-y-1 group-hover:translate-x-1 group-hover:scale-110" />
        </div>
