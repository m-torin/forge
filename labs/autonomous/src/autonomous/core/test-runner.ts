      ...
      vitest.on('close', (code) => {
        ...
        try {
          ...
          if (jsonMatch) {
            ...
            resolve(this.parseVitestResults(results, code ?? 1, duration));
          } else {
            // Fallback parsing if JSON extraction fails
            resolve(this.parseVitestFallback(output, errorOutput, code ?? 1, duration));
          }
        } catch (error) {
    ...
    return new Promise((resolve) => {
      ...
      playwright.on('close', (code) => {
        ...
        try {
          ...
          if (jsonMatch) {
            const results = JSON.parse(jsonMatch[0]);
            resolve(this.parsePlaywrightResults(results, code ?? 1, duration));
          } else {
            resolve(this.parsePlaywrightFallback(output, errorOutput, code ?? 1, duration));
          }
        } catch (error) {
