...
import { ProductForm } from '@/components/pim3/ProductForm';
import { ProductRelationshipManager } from '@/components/pim3/ProductRelationshipManager';
import { getProductsWithPIMFilters, deleteProduct } from '@/actions/pim3/actions';
import {
  formatCurrency,
  getStatusColor,
...
import { ProductStatus } from '@repo/database/prisma';
      ...
      },
      {
        ...
        Cell: ({ row }) => (
          <Stack gap={4}>
            ...
            {row.original.count.soldBy > 0 && (
              <Text size="xs">
                <strong>Sellers:</strong> {row.original.count.soldBy}
              </Text>
            )}
          </Stack>
        ),
      },
      {
        ...
        Cell: ({ row }) => (
          <Stack gap={4}>
            ...
            {row.original.count.children > 0 && (
              <Text size="xs">{row.original.count.children} variants</Text>
            )}
          </Stack>
        ),
      },
