...
export async function getMediaUsage(id: string) {
  ...
  const media = await findUniqueMediaOrm({
    include: {
      article: { select: { id: true, title: true } },
      brand: { select: { id: true, name: true } },
      category: { select: { id: true, name: true } },
...
export async function getEntityOptions(entityType: MediaEntityType) {
  ...
  switch (entityType) {
    case 'PRODUCT':
      return await findManyProductsOrm({
        orderBy: { name: 'asc' },
    ...
    case 'BRAND':
      return await findManyBrandsOrm({
        orderBy: { name: 'asc' },
    ...
    case 'CATEGORY':
      return await findManyProductCategoriesOrm({
        orderBy: { name: 'asc' },
        select: { id: true, name: true },
        take: 100,
        where: { deletedAt: null },
    ...
    case 'COLLECTION':
      return await findManyCollectionsOrm({
        orderBy: { name: 'asc' },
        select: { id: true, name: true },
        take: 100,
        where: { deletedAt: null },
    ...
    case 'ARTICLE':
      return await findManyArticlesOrm({
        orderBy: { title: 'asc' },
    ...
    case 'TAXONOMY':
      return await findManyTaxonomiesOrm({
        orderBy: { name: 'asc' },
        select: { id: true, name: true },
        take: 100,
        where: { deletedAt: null },
    ...
    case 'REVIEW':
      return await findManyReviewsOrm({
...
export async function getMediaStats() {
  ...
  const [total, byType, byEntityType, recentUploads] = await Promise.all([
    // Total count
    countMediaOrm({ where: { deletedAt: null } }),
    ...
    Promise.all([
      countMediaOrm({ where: { deletedAt: null, productId: { not: null } } }),
      countMediaOrm({ where: { brandId: { not: null }, deletedAt: null } }),
      countMediaOrm({ where: { categoryId: { not: null }, deletedAt: null } }),
      countMediaOrm({ where: { collectionId: { not: null }, deletedAt: null } }),
      countMediaOrm({ where: { articleId: { not: null }, deletedAt: null } }),
      countMediaOrm({ where: { deletedAt: null, taxonomyId: { not: null } } }),
      countMediaOrm({ where: { deletedAt: null, reviewId: { not: null } } }),
      countMediaOrm({
        where: {
          articleId: null,
          brandId: null,
...
export async function getFolders() {
  ...
  const media = await findManyMediaOrm({
    select: { url: true },
    where: { deletedAt: null },
  });
