...
import {
  ...
  updateReviewVoteJoinOrm,
  deleteReviewVoteJoinOrm,
  findUniqueReviewVoteJoinOrm,
  findManyProductsOrm,
  countSuspiciousReviews,
...
export async function getReviewById(id: string) {
  ...
  return await findUniqueReviewOrm({
    include: {
      _count: { select: { votes: true } },
      deletedBy: {
        select: {
...
export async function bulkUpdateReviews(data: BulkUpdateData) {
  ...
  return await updateManyReviewsOrm({
    data: updateData,
    where: {
...
export async function getReviewAnalytics(): Promise<ReviewAnalytics> {
  ...
  const [
    ...
    // Total reviews
    countReviewsOrm({ where: { deletedAt: null } }),
  ...
  const topProductsByReviews = topProducts.map((tp) => {
    ...
    return {
      ...
      reviewCount: (tp._count as any)?.id || 0,
    };
  });
  ...
  return {
    ...
    ratingDistribution: ratingDistribution.map((rd) => ({
      count: (rd._count as any)?.rating || 0,
      rating: rd.rating,
    })),
    recentTrends: {
      ...
      totalLastMonth: (lastMonthStats._count as any)?.id || 0,
      totalThisMonth: (thisMonthStats._count as any)?.id || 0,
    },
    ...
    statusDistribution: statusDistribution.map((sd) => ({
      count: (sd._count as any)?.status || 0,
      status: sd.status,
    })),
    topProductsByReviews,
    totalReviews,
...
export async function voteOnReview(reviewId: string, voteType: VoteType) {
  ...
  const existingVote = await findUniqueReviewVoteJoinOrm({
    where: {
      userId_reviewId: {
        reviewId,
        userId,
  ...
  if (existingVote) {
    if (existingVote.voteType === voteType) {
      ...
      await deleteReviewVoteJoinOrm({
        where: { id: existingVote.id },
      });
    } else {
      // Update vote type
      await updateReviewVoteJoinOrm({
        data: { voteType },
        where: { id: existingVote.id },
      });
    }
...
export async function detectSuspiciousReviews() {
  ...
  const allReviews = await findManyReviewsOrm({
    include: {
