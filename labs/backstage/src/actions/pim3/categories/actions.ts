...
export async function createCategory(formData: FormData) {
  try {
    ...
    const category = await executeTransaction(async (tx) => {
      // Create the category
      const newCategory = await tx.productCategory.create({
        data: {
          ...
          copy: categoryFields.copy as any,
        },
      });
  ...
  try {
    ...
    const category = await executeTransaction(async (tx) => {
      ...
      const updatedCategory = await tx.productCategory.update({
        where: { id },
        data: {
          ...(categoryFields.name && { name: categoryFields.name }),
          ...(categoryFields.slug && { slug: categoryFields.slug }),
          ...(categoryFields.status && { status: categoryFields.status }),
          ...(categoryFields.parentId !== undefined && { parentId: categoryFields.parentId }),
          ...(categoryFields.displayOrder !== undefined && {
            displayOrder: categoryFields.displayOrder,
          }),
          ...(categoryFields.copy && { copy: categoryFields.copy as any }),
        },
      });
...
export async function duplicateCategory(id: string) {
  try {
    ...
    const originalCategory = await findUniqueProductCategoryOrm({
      where: { id, deletedAt: null },
      include: {
        products: {
          select: { id: true },
    ...
    const duplicatedCategory = await executeTransaction(async (tx) => {
      const {
        id: id,
        createdAt,
        updatedAt,
      ...
      const products = (originalCategory as any).products || [];
      const collections = (originalCategory as any).collections || [];
      ...
      if (products.length > 0) {
        await tx.productCategory.update({
          ...
          data: {
            products: {
              connect: products.map((product: { id: string }) => ({ id: product.id })),
            },
          },
        });
      }
      ...
      if (collections.length > 0) {
        await tx.productCategory.update({
          ...
          data: {
            collections: {
              connect: collections.map((collection: { id: string }) => ({ id: collection.id })),
            },
          },
        });
      }
