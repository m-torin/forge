...
import {
  getBrands,
  createProductBrandAssociation,
  removeProductBrandAssociation,
...
export function ProductRelationshipManager({
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (relResult.success && relResult.data) {
        ...
        setSelectedCollections(relResult.data.collections.map((c: any) => c.id));
        setSelectedTaxonomies(relResult.data.taxonomies.map((t: any) => t.id));
        setSelectedBrands(relResult.data.brands.map((b: any) => b.id));
      }

      // Load available options
      const [collections, taxonomies, brandsResult] = await Promise.all([
        getCollectionsAction({}),
        getTaxonomiesAction({}),
        getBrands({}),
      ]);

      const brands = (brandsResult as any).data?.data || (brandsResult as any).brands || [];
  ...
  const handleBrandToggle = async (brandId: string, checked: boolean) => {
    ...
    try {
      if (checked) {
        await createProductBrandAssociation(productId, brandId);
      } else {
        await removeProductBrandAssociation(productId, brandId);
      }
