import { z } from 'zod';
import { SecretScalarWhereInputObjectSchema } from './SecretScalarWhereInput.schema';
import { SecretUpdateManyMutationInputObjectSchema } from './SecretUpdateManyMutationInput.schema';
import { SecretUncheckedUpdateManyWithoutFlowInputObjectSchema } from './SecretUncheckedUpdateManyWithoutFlowInput.schema';

import type { Prisma } from '@prisma/client';

const Schema: z.ZodType<any> = z
  .object({
    where: z.lazy(() => SecretScalarWhereInputObjectSchema),
    data: z.union([
      z.lazy(() => SecretUpdateManyMutationInputObjectSchema),
      z.lazy(() => SecretUncheckedUpdateManyWithoutFlowInputObjectSchema),
    ]),
  })
  .strict();

export const SecretUpdateManyWithWhereWithoutFlowInputObjectSchema = Schema;
