...
describe("next.config.js test", () => {
  describe("rewrites", () => {
    ...
    it("non blog pages are not rewritten", () => {
      expect(getRewrittenUrl("/")).toStrictEqual(undefined);
      expect(getRewrittenUrl("/blog-not")).toStrictEqual(undefined);
      expect(getRewrittenUrl("/blog2")).toStrictEqual(undefined);
    });

    it("/blog is rewritten to child zone", () => {
      expect(getRewrittenUrl("/blog")).toStrictEqual(`${BLOG_URL}/blog`);
      expect(getRewrittenUrl("/blog/post/1")).toStrictEqual(
    ...
    it("/blog static resources are rewritten to child zone", () => {
      ...
      ).toStrictEqual(`${BLOG_URL}/blog-static/_next/static/chunks/chunk.css`);
    });
  });
});
