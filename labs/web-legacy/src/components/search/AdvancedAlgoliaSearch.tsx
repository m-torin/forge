  ...
  const debouncedSearch = useDebouncedCallback(async (searchQuery: string) => {
    ...
    if (searchQuery.length > 2) {
      ...
      try {
        ...
        console.warn('Query suggestions unavailable: ', _error);
      } finally {
        setIsLoading(false);
      }
    }
...
function AdvancedAlgoliaSearchInner({ params }: { params: { locale: string } }) {
  const { locale } = params;
      ...
      >
        ...
        <Stack gap="xl">
          ...
          <Grid>
            ...
            <Grid.Col span={{ base: 12, md: 9 }}>
              <Stack gap="lg">
                <ErrorBoundary fallback={<div>Sort options unavailable</div>}>
                  <Group justify="space-between" ta="center">
                    <SmartSortBy />
                  </Group>
                </ErrorBoundary>
                ...
                <ErrorBoundary
                  fallback={
                    <Text ta="center" py="xl">
                      Search results unavailable. Please try again.
                    </Text>
                  }
                >
...
export default function AdvancedAlgoliaSearch(props: { params: { locale: string } }) {
  return (
    <ErrorBoundary
      fallback={
        <Container size="xl" py="xl">
          <Stack gap="xl" ta="center">
            ...
            <Text c="dimmed">
              We're experiencing technical difficulties with search. Please try again later.
            </Text>
    ...
    >
      <AdvancedAlgoliaSearchInner {...props} />
    </ErrorBoundary>
  );
}
