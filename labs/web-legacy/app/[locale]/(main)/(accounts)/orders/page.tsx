...
import { Link, Prices } from '@/components/ui';
  ...
  return (
    <div className="z-0 overflow-hidden rounded-lg border border-neutral-200 dark:border-neutral-700">
      <div className="flex flex-col bg-neutral-50 p-4 sm:flex-row sm:items-center sm:justify-between sm:p-8 dark:bg-neutral-500/5">
        <div>
          <Link href={`/orders/${order.orderNumber}`} className="text-lg font-semibold">
            #{order.orderNumber}
          </Link>
            ...
            <div className="ml-4 flex flex-1 flex-col">
              <div>
                <div className="flex justify-between">
                  <div>
                    <h3 className="line-clamp-1 text-base font-medium">{item.productName}</h3>
                    {item.variantName && (
                  ...
                  <Prices className="ml-2 mt-0.5" price={item.price} />
                </div>
              </div>
  ...
  return (
    <div className="flex flex-col gap-y-10 sm:gap-y-12">
      ...
      {orders.length === 0 ? (
        ...
        orders.map((order: any) => <Order key={order.orderNumber || order.id} order={order} />)
      )}
    </div>
  );
};
