...
export default function TestDrawersPage() {
    ...
    try {
      ...
      if (cartButton) {
        ...
        setTimeout(() => {
          ...
          const cartContent = drawer?.querySelector('h2')?.textContent?.includes('Shopping Cart');
          const hasProducts = (drawer?.querySelectorAll('[role="list"] > div')?.length || 0) > 0;
          ...
          setTestResults((prev) => ({
            ...
            cartDrawer: {
              details: {
                ...
                productCount: drawer?.querySelectorAll('[role="list"] > div').length || 0,
          ...
          // Close the drawer
          const closeButton = drawer?.querySelector('button[aria-label*="Close"]');
  ...
  const testNavigationDrawer = () => {
    try {
      ...
      const menuButton = document.querySelector('[data-testid="hamburger-menu"]');
      ...
      if (menuButton) {
        ...
        setTimeout(() => {
          ...
          const drawer = document.querySelector('[role="dialog"]');
          const hasLogo =
            drawer?.querySelector('img[alt*="logo" i]') || drawer?.querySelector('svg');
    ...
    try {
        ...
        setTimeout(() => {
          ...
          if (quickViewButton) {
            ...
            setTimeout(() => {
              ...
              const hasAddToCart = drawer?.querySelector('button')?.textContent?.includes('Add to');
              ...
              setTestResults((prev) => ({
                ...
                productQuickView: {
                  ...
                  success: Boolean(drawer && (hasTitle || hasProductInfo) && hasAddToCart),
                },
              }));
      ...
      setTestResults((prev) => ({
        ...
        productQuickView: {
          ...
          message: `Error: ${error instanceof Error ? (error as Error)?.message || 'Unknown error' : String(error)}`,
          success: false,
        },
      }));
    }
      ...
      <Stack gap="lg">
        <Card shadow="sm" withBorder={true} padding="lg" radius="sm">
          ...
          <Group>
            ...
            <Button onClick={runAllTests} variant="light">
              Run All Tests
            </Button>
          </Group>
        </Card>
        ...
        <Card shadow="sm" withBorder={true} padding="lg" radius="sm">
          ...
          <Stack gap="md">
            {Object.entries(testResults).map(([key, result]) => (
              <Card key={key} withBorder={true} padding="sm">
                ...
                {result && (
                  <>
                    <Text color="dimmed" className="mb-2" size="md">
                      {result.message}
                    </Text>
                    {result.details && (
                      <Card withBorder={true} bg="gray.0" padding="xs">
        ...
        <Card shadow="sm" withBorder={true} padding="lg" radius="sm">
          ...
          <Stack gap="sm">
            <Text>
              1. <strong>Cart Drawer:</strong> Click the cart icon in the header. The drawer should
              slide in from the right showing cart items.
            </Text>
            <Text>
              2. <strong>Navigation Drawer:</strong> Click the menu icon in the header. The drawer
              should slide in from the right showing navigation menu.
            </Text>
            <Text>
              3. <strong>Product Quick View:</strong> Hover over any product card above and click
              the "Quick view" button. A drawer should open with product details.
            </Text>
          </Stack>
        </Card>
      </Stack>
