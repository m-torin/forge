/**
 * Common test patterns and utilities to reduce duplication across test files
 *
...
export const databaseAssertions = {
   ...
   */
  expectCrudCalls: (mockClient: any, models: string[]) => {
   ...
   */
  expectModelCreation: (mockClient: any, model: string, expectedFields: string[]) => {
  ...
  expectRelationships: (
    ...
    relationships.forEach(({ child, parent, foreignKey }) => {
      const childCall = mockClient[child].create.mock.calls[0][0];
      const parentCall = mockClient[parent].create.mock.calls[0][0];
  ...
  /**
   * Assert that mock calls were made correctly
   */
  expectMockCalls: (mockClient: any, expectedCalls: string[], operation: string) => {
    expectedCalls.forEach(call => {
      if (mockClient[call] && mockClient[call][operation]) {
        expect(mockClient[call][operation]).toHaveBeenCalledWith();
      }
    });
  },
};
...
/**
 * Common test data generators for different data types
 */
export const testDataGenerators = {
  /**
   * Generate test data with common fields
   */
  generateCommonData: (type: string, overrides: Record<string, any> = {}) => ({
    id: `${type}-test-id`,
...
/**
 * Common mock scenarios for different test cases
 */
export const mockScenarios = {
  /**
   * Setup successful operation scenario
   */
  success: (mockClient: any, models: string[] = []) => {
    models.forEach(model => {
      if (mockClient[model]) {
   ...
   */
  error: (mockClient: any, models: string[], errorMessage = 'Test error') => {
    models.forEach(model => {
      if (mockClient[model]) {
        Object.keys(mockClient[model]).forEach(method => {
          if (typeof mockClient[model][method] === 'function') {
            mockClient[model][method].mockRejectedValue(new Error(errorMessage));
          }
        });
      }
    });
   ...
   */
  existingData: (mockClient: any, existingData: Record<string, any>) => {
  ...
  /**
   * Test entity errors for multiple models
   */
  testEntityErrors: async (
    ...
    entities.forEach(entity => {
      if (mockClient[entity]) {
        Object.keys(mockClient[entity]).forEach(method => {
          if (typeof mockClient[entity][method] === 'function') {
            mockClient[entity][method].mockRejectedValue(new Error(`${entity} error`));
          }
        });
      }
    });
   ...
   */
  testConcurrentExecution: async (testFunction: () => Promise<any>, concurrency: number = 5) => {
