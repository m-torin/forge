...
class MockRedisStorage {
  ...
  scan(cursor: number, options?: { match?: string; count?: number }): [number, string[]] {
    ...
    if (match) {
      ...
      // eslint-disable-next-line security/detect-non-literal-regexp -- Safe: converting Redis glob patterns to regex
...
// Create a vitest-compatible Redis mock for testing
export const createVitestCompatibleRedisClient = () => {
  // Mock storage for testing - single source of truth
  ...
  const client = {
    ...
    // Internal testing helpers - same storage reference
    _getStorage: () => ({
...
export const resetUpstashMocks = (mocks?: any) => {
  vi.clearAllMocks();
  resetMockRedisStorage();
