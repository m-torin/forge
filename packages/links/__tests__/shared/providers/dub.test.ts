...
describe("dub Provider", () => {
  ...
  describe("provider Initialization", () => {
    test("should initialize with minimal configuration", async () => {
      const { DubProvider } = await import(
  ...
  describe("link Creation", () => {
    ...
    test("should handle create errors", async () => {
      ...
      await expect(
        provider.createLink({ url: "https://example.com" }),
  ...
  describe("link Retrieval", () => {
    ...
    test("should handle non-404 errors", async () => {
      ...
      await expect(provider.getLink("test-link-id")).rejects.toThrow(
        "500: Server error",
      );
    });
  });
  ...
  describe("link Updates", () => {
    ...
    test("should handle update errors", async () => {
      ...
      mockDubClient.links.update.mockRejectedValueOnce(
        new Error("Update failed"),
      );

      await expect(
        provider.updateLink("test-link-id", { title: "New Title" }),
  ...
  describe("link Deletion", () => {
    ...
    test("should handle delete errors", async () => {
      ...
      mockDubClient.links.delete.mockRejectedValueOnce(
        new Error("Delete failed"),
      );

      await expect(provider.deleteLink("test-link-id")).rejects.toThrow(
        "Delete failed",
      );
    });
  });
  ...
  describe("bulk Operations", () => {
    ...
    test("should handle bulk create errors", async () => {
      ...
      mockDubClient.links.createMany.mockRejectedValueOnce(
  ...
  describe("error Handling", () => {
    test("should handle authentication errors", async () => {
      ...
      const provider = new DubProvider(
        { enabled: true, apiKey: "invalid-key" },
        mockDubClient,
      );

      mockDubClient.links.create.mockRejectedValueOnce(
    ...
    test("should handle validation errors", async () => {
      ...
      mockDubClient.links.create.mockRejectedValueOnce(
        new Error("400: Invalid URL"),
      );

      await expect(provider.createLink({ url: "invalid-url" })).rejects.toThrow(
        "400: Invalid URL",
      );
    });
