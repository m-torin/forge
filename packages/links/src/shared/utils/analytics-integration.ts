...
export class LinkAnalyticsIntegration implements AnalyticsIntegration {
  ...
  private async processEvent(eventData: LinkAnalyticsEventData): Promise<void> {
    try {
      ...
      if (this.config.debugMode) {
        console.debug('[LinkAnalytics] Event tracked: ', analyticsEvent);
      }
    } catch (error: any) {
      if (this.config.debugMode) {
        console.debug('[LinkAnalytics] Event tracking failed (non-critical): ', error);
      }
  ...
  async identify(userId: string, traits?: Record<string, any>): Promise<void> {
    ...
    try {
      ...
      if (this.config.debugMode) {
        console.debug('[LinkAnalytics] User identification failed (non-critical): ', error);
      }
    }
  }
