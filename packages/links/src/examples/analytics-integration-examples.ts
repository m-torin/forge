...
export async function serverSideWithAnalytics() {
  ...
  console.log('Link created with analytics:', link.shortLink);
  ...
  console.log(`Bulk created ${bulkResult.created.length} links with analytics`);
...
export async function testingWithMockAnalytics() {
  ...
  const mockAnalytics = {
    ...
    track: async (event: string, properties?: Record<string, any>) => {
      ...
      console.log('[MockAnalytics]', event, properties);
    },
  };
  ...
  console.log('Analytics events captured:', mockAnalytics.events);
