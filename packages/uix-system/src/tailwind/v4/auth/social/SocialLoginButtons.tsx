/**
 * Tailwind v4 RSC Social Login Buttons
...
interface SocialLoginButtonsProps extends BaseProps {
  providers: Array<'google' | 'github' | 'discord' | 'facebook' | 'microsoft'>;
  ...
  return (
    <div className={`space-y-3 ${className}`}>
      {providers.map(provider => {
        ...
        return (
          <form key={provider} action={action} className="w-full">
            <input type="hidden" name="provider" value={provider} />
            <Button
              type="submit"
              variant="outline"
              className={`w-full ${config.bgColor} ${config.textColor} border transition-shadow hover:shadow-sm`}
            >
