...
export function AdminBulkUserActions({
      ...
      switch (action) {
        case "ban":
          results = await Promise.allSettled(
            userIds.map((userId) => banUserAction(userId)),
          );
          break;

        case "unban":
          results = await Promise.allSettled(
            userIds.map((userId) => unbanUserAction(userId)),
          );
          break;
        ...
        case "delete":
          ...
          results = await Promise.allSettled(
            userIds.map((userId) => adminDeleteUserAction(userId)),
          );
          break;
  ...
  const canExecuteAction = () => {
    ...
    if (selectedAction === "delete" && confirmationText !== "DELETE")
      return false;
    if (selectedAction === "send-emails" && (!emailSubject || !emailMessage))
      return false;
    return true;
  };
      ...
      <Card className={className}>
        <CardContent className="p-8 text-center">
          ...
          <p className="text-gray-600">
            Select users from the list to perform bulk actions
          </p>
        </CardContent>
      </Card>
    );
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-2 gap-4 md:grid-cols-3">
            ...
            <Button
              variant={
                selectedAction === "revoke-sessions" ? "destructive" : "outline"
              }
      ...
      {actionState.success && (
        <Alert variant="default">{actionState.message}</Alert>
      )}
