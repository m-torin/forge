...
export function SecurityEventLog({
  ...
  const [filterBy, setFilterBy] = useState<'all' | 'new' | 'high' | 'critical' | 'action_required'>(
    'all',
  );
  ...
  const [dismissState, dismissAction] = useFormState(dismissEventAction, initialFormState);
  const [refreshState, refreshAction] = useFormState(refreshEventsAction, initialFormState);
  ...
  const handleSelectAll = () => {
    ...
    setSelectedEvents(
      selectedEvents.size === filteredEventIds.length ? new Set() : new Set(filteredEventIds),
    );
  };
  ...
  const criticalEvents = events.filter(e => e.severity === 'critical' && e.status === 'new');
  const newEvents = events.filter(e => e.status === 'new');
  const actionRequiredEvents = events.filter(e => e.actionRequired && e.status === 'new');
  ...
  return (
    <div className={`space-y-6 ${className}`}>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-sm">
                ...
                {criticalEvents.length !== 1 ? 's' : ''} require immediate attention
              </p>
            </div>
            <Button
              size="sm"
      ...
      {(acknowledgeState.success || dismissState.success || refreshState.success) && (
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center">
              <span className="mr-3 text-2xl">🆕</span>
              <div>
                <div className="text-2xl font-bold text-blue-600">{newEvents.length}</div>
                      ...
                      <div className="mb-3 grid grid-cols-1 gap-4 text-sm text-gray-600 md:grid-cols-3">
                        ...
                        <div>
                          ...
                          {event.source.location && (
                            <div className="text-xs">
                              {event.source.location.city}, {event.source.location.country}
                            </div>
                          )}
                        </div>
                      </div>
                          ...
                          {expandedEvent === event.id && (
                            <div className="mt-3 rounded-lg bg-gray-50 p-4">
                              <div className="space-y-2 text-sm">
                                <div>
                                  <strong>User Agent:</strong> {event.source.userAgent}
                                </div>
                                {event.source.device && (
                                  <div>
                                    <strong>Device:</strong> {event.source.device.name} (
          ...
          <div className="flex items-start">
            ...
            <div className="text-sm text-blue-800">
              ...
              <ul className="list-inside list-disc space-y-1">
                ...
                <li>Contact security team for critical or high-severity events</li>
