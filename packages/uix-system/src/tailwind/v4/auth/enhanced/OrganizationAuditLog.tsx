...
interface AuditLogEntry {
  ...
  timestamp: string;
  action: string;
  category: 'user' | 'team' | 'project' | 'security' | 'settings' | 'billing' | 'integration';
  actor: {
    id: string;
    email: string;
    name?: string;
...
export function OrganizationAuditLog({
  ...
  const [exportState, exportAction] = useFormState(exportAuditLogAction, initialFormState);
  const [refreshState, refreshAction] = useFormState(refreshAuditLogAction, initialFormState);
  ...
  const filteredEntries = entries.filter(entry => {
    ...
    // Date range filter
    if (dateRange.start && new Date(entry.timestamp) < new Date(dateRange.start)) return false;
    if (dateRange.end && new Date(entry.timestamp) > new Date(dateRange.end)) return false;
    ...
    <div className={`space-y-6 ${className}`}>
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-600">Activity trail for {organizationName}</p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-6">
        ...
        <Card>
          <CardContent className="p-4">
            <div className="text-center">
              <div className="text-2xl font-bold text-green-600">{outcomeCounts.success || 0}</div>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="text-center">
              <div className="text-2xl font-bold text-red-600">{outcomeCounts.failure || 0}</div>
      ...
      {showExportModal && (
        <Card className="border-blue-200 bg-blue-50">
          <CardHeader>
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-medium text-blue-900">Export Audit Log</h3>
          ...
          <CardContent>
            <form onSubmit={handleExport} className="space-y-4">
              ...
              <div className="rounded-lg border border-blue-200 bg-white p-4">
                <h4 className="mb-2 font-medium text-gray-900">Export Summary</h4>
                <div className="space-y-1 text-sm text-gray-600">
                  ...
                  <p>
                    ...
                    {[
                      ...
                      selectedSeverity !== 'all' ? `Severity: ${selectedSeverity}` : null,
                      selectedOutcome !== 'all' ? `Outcome: ${selectedOutcome}` : null,
              ...
              <Button
                ...
                className="w-full"
              >
                {isPending ? 'Preparing Export...' : `Export ${filteredEntries.length} Records`}
              </Button>
            </form>
          </CardContent>
        </Card>
        ...
        {filteredEntries.length === 0 ? (
          <Card>
            <CardContent className="p-8 text-center">
              ...
              <h3 className="mb-2 text-lg font-medium text-gray-900">No Audit Entries Found</h3>
                          ...
                          {expandedEntry === entry.id && (
                            <div className="mt-3 rounded-lg bg-gray-50 p-4">
                              <div className="space-y-2 text-sm">
                                {entry.context.ip && (
                                  <div>
                                    <strong>IP Address:</strong> {entry.context.ip}
                                  </div>
                                )}
                                {entry.context.userAgent && (
                                  <div>
                                    <strong>User Agent:</strong> {entry.context.userAgent}
                                  </div>
                                )}
                                {entry.context.location && (
                                  <div>
                                    <strong>Location:</strong> {entry.context.location.city},{' '}
        ...
        <CardContent className="p-4">
          <div className="flex items-start">
            ...
            <div className="text-sm text-green-800">
              ...
              <p>
                This audit log provides a complete trail of all organizational activities for
                compliance, security monitoring, and operational transparency. All entries are
                immutable and automatically retained according to your data retention policy.
              </p>
            </div>
          </div>
        </CardContent>
