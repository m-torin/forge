import { createResponse, extractPayload, validatePayload, workflowError } from '../context';
...
async function processEnhancedBasicWorkflow(
  ...
  const results = await Promise.all(
    sortedTasks.map((task: any) =>
      context.run(`enhanced-process-${task.enhancedId}`, async () => {
  ...
  const enhancedSummary = await context.run('generate-enhanced-summary', async () => {
    ...
    return {
      ...
      results,
      successfulTasks,
      successRate: (((successfulTasks + recoveredTasks) / tasks.length) * 100).toFixed(2) + '%',
      taskId,
