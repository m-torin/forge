  ...
  return {
    ...
    notify:
      mockFn().mockImplementation?.(async (options: any) => {
        console.log(`[MOCK] Notifying:`, options);
        return [{ status: 'success' }];
      }) || (async () => [{ status: 'success' }]),

    cancel:
      mockFn().mockImplementation?.(async (reason: string) => {
        console.log(`[MOCK] Cancelling: ${reason}`);
      }) || (async () => {}),
