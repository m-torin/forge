import {
  ...
  moderateContent,
} from '../ai/anthropic-integration';
import { createOptimalBatchConfig, processBatches } from '../qstash/batch-processing';
...
import { createSigningConfigFromEnv, withSignatureVerification } from '../qstash/request-signing';
...
export async function processAIContentPipeline(
  ...
  console.log(
    `[AI-PIPELINE] Starting content moderation job ${jobId} with ${content.length} items`,
  );
  ...
  if (options.verifySignatures) {
    ...
    if (signingConfig) {
      await withSignatureVerification(context, signingConfig, async () => {
        console.log('[AI-PIPELINE] Request signature verified');
      });
    }
  }
  ...
  if (!anthropicConfig) {
    throw new Error('Anthropic API configuration not found');
  }
  ...
  const notificationResults = await context.run('send-notifications', async () => {
    ...
    const notificationPromises = flaggedItems.map(async (item) => {
      ...
      return fanOutToURLGroup(context, `notify-flagged-${item.contentId}`, {
  ...
  const summary = await context.run('generate-pipeline-summary', async () => {
    ...
    const successRate = ((processedContent / totalContent) * 100).toFixed(2);
    const flaggedRate = ((processedResults.flaggedContent / processedContent) * 100).toFixed(2);

    console.log(
      `[AI-PIPELINE] Completed job ${jobId}: ${processedContent}/${totalContent} processed, ${processedResults.flaggedContent} flagged`,
    );
...
export async function processAdvancedContentAnalysis(
  ...
  console.log(
    `[AI-ANALYSIS] Starting advanced analysis job ${jobId} with ${analysisTypes.length} analysis types`,
  );
  ...
  const summary = await context.run('generate-analysis-summary', async () => {
    ...
    return {
      ...
      successRate: `${((Object.keys(analysisResults).length / analysisCount) * 100).toFixed(2)}%`,
      totalItems,
    };
  });
