...
export async function processSaaSTenantOperation(
  ...
  const operationId = generateUniqueId(`${tenantId}-${operationType}`);
  ...
  const deduplicationOptions: DeduplicationOptions = {
    ...
    // Uses dedupId by default, with fallback to deduplicationId for compatibility
  };
  ...
  return withDeduplication(
    ...
    async () => {
      ...
      notificationResult = await context.run('send-tenant-notifications', async () => {
        ...
        const fanOutResult = await fanOutToURLGroup(context, 'notify-tenant', {
      ...
      const successRate = calculateSuccessRate(operationsExecuted, operationsExecuted); // If we get here, operations succeeded
...
async function processTenantSyncOperations(
  ...
  return context.run('tenant-sync-operations', async () => {
    ...
    if (operationType === 'data_sync') {
      ...
      syncOperations.push({
        url: buildApiEndpoint(process.env.ANALYTICS_API!, tenantId, '/sync'),
      ...
      syncOperations.push({
        url: buildApiEndpoint(process.env.SEARCH_API!, tenantId, '/reindex'),
    ...
    if (operationType === 'feature_rollout') {
      // Sync to CDN
      syncOperations.push({
        url: buildApiEndpoint(process.env.CDN_API!, tenantId, '/invalidate'),
