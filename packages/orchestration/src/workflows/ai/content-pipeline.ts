...
import { batchAnalyzeContent, moderateContent } from '../../utils/ai-integration';
import { createWorkflowError, withWorkflowErrorHandling } from '../../utils/error-handling';
// Import utilities to eliminate DRY violations
import { calculateElapsedTime, formatPercentage, formatTimestamp } from '../../utils/helpers';
...
export async function processAIContentPipeline(
  ...
  return withWorkflowErrorHandling(
    async () => {
              ...
              return runWithDLQ(
                ...
                async () => {
                  const moderationRequest: ContentModerationRequest = {
                    ...
                    moderationRules: {
                      ...moderationRules,
                      checkHateSpeech: moderationRules.checkToxicity, // Default to same as toxicity check
                    },
                  };
      ...
      const _notificationResults = await context.run('send-notifications', async () => {
        const flaggedItems = processedResults.processedItems.filter(
          (item) => !item.moderation.safe,
        );
      ...
      const summary = await context.run('generate-pipeline-summary', async () => {
        ...
        const flaggedRate = formatPercentage(
          (processedResults.flaggedContent / processedContent) * 100,
        );
    ...
    async () => {
      ...
      const summary = await context.run('generate-analysis-summary', async () => {
        ...
        return {
          ...
          successRate: formatPercentage(
            (Object.keys(analysisResults).length / analysisCount) * 100,
          ),
          totalItems,
        };
      });
