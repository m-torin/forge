...
function updateErrorTracking(workflowRunId: string, stepName: string, error: Error): ErrorTracking {
  ...
  tracking.retryCount++;
  tracking.lastFailedAt = new Date().toISOString();
...
export async function sendToDLQ(
  ...
  console.log(`[DLQ] Sending to DLQ: ${originalStepName} failed ${tracking.retryCount} times`);

  try {
    if (config.useNativeDLQ && config.qstashClient) {
      // Use QStash native DLQ - messages automatically go to DLQ after max retries
      console.log(`[DLQ] Using QStash native DLQ for failed message`);
      ...
      console.log(`[DLQ] Successfully sent to custom DLQ: ${config.dlqEndpoint}`);
    } else {
      console.warn(`[DLQ] No DLQ configuration available - logging error only`);
      await logDLQMessage(dlqMessage, config);
    }
  } catch (dlqError) {
...
async function logDLQMessage(dlqMessage: DLQMessage, config: DLQConfig): Promise<void> {
  // Log to console (in production, you might want to send to logging service)
  ...
  if (config.failureCallback) {
    try {
      ...
      console.error('[DLQ] Failed to send failure callback:', error);
    }
  }
}
...
export async function runWithDLQ<T>(
  ...
  try {
    ...
    console.log(
      `[DLQ] Step ${stepName} failed (attempt ${updatedTracking.retryCount}/${config.maxRetries})`,
    );

    // Check if we should send to DLQ
    if (updatedTracking.retryCount >= config.maxRetries) {
      await sendToDLQ(context, `dlq-${stepName}`, config, err, stepName);
    ...
    if (!isRetryableError(err, config.retryableErrors)) {
      console.log(`[DLQ] Non-retryable error, sending to DLQ immediately`);
      await sendToDLQ(context, `dlq-${stepName}`, config, err, stepName);
...
export async function processDLQMessage(
  ...
  console.log(`[DLQ] Processing DLQ message for step: ${message.failedStep}`);
    ...
    handleDLQMessage: async (message) => {
      ...
      if (
        ...
        return {
          ...
          retryDelay: Math.min(5000 * message.retryCount, 30000), // Exponential backoff
        };
      }
