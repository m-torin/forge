...
describe('upstashWorkflowProvider', () => {
  ...
  describe('execute', () => {
    test('should execute workflow successfully', async () => {
      ...
      const definition = {
        ...
        name: 'Test Workflow',
        steps: [
          { id: 'step1', action: 'http', input: { url: 'https://api.test.com' } },
    ...
    test('should handle execution errors', async () => {
      mockQstashClient.publishJSON.mockRejectedValueOnce(new Error('QStash error'));
  ...
  describe('getExecution', () => {
    ...
    test('should throw error when Redis not configured', async () => {
      ...
      await expect(noRedisProvider.getExecution('execution-123')).rejects.toThrow(ProviderError);
    });
  });
  ...
  describe('listExecutions', () => {
    test('should list executions for workflow', async () => {
      ...
      const mockPipeline = {
        get: vi.fn().mockReturnThis(),
        exec: vi.fn().mockResolvedValue(executions.map(exec => [null, JSON.stringify(exec)])),
      };
    ...
    test('should filter executions by status', async () => {
      ...
      const executions = [
        { id: 'exec-1', status: 'completed', startedAt: new Date().toISOString() },
        { id: 'exec-2', status: 'running', startedAt: new Date().toISOString() },
      ];
    ...
    test('should filter executions by date range', async () => {
      ...
      const executions = [
        ...
        { id: 'exec-2', status: 'completed', startedAt: yesterday.toISOString() },
      ];
    ...
    test('should throw error when Redis not configured', async () => {
      ...
      await expect(noRedisProvider.listExecutions('workflow-123')).rejects.toThrow(ProviderError);
    });
  });
  ...
  describe('listWorkflowExecutions', () => {
    ...
    test('should apply filters and pagination', async () => {
      const executions = [
        { id: 'exec-1', status: 'completed', startedAt: new Date().toISOString() },
        { id: 'exec-2', status: 'running', startedAt: new Date().toISOString() },
        { id: 'exec-3', status: 'completed', startedAt: new Date().toISOString() },
      ];

      mockRedisClient.scan.mockResolvedValueOnce([
        '0',
        ['workflow:execution:exec-1', 'workflow:execution:exec-2', 'workflow:execution:exec-3'],
      ]);
  ...
  describe('scheduleWorkflow', () => {
    test('should schedule workflow successfully', async () => {
      ...
      expect(mockQstashClient.schedules.create).toHaveBeenCalledWith({
        destination: 'https://test.example.com/api/workflows/scheduled-workflow/execute',
        cron: '0 9 * * *',
    ...
    test('should throw error for workflow without schedule', async () => {
      ...
      await expect(provider.scheduleWorkflow(definition)).rejects.toThrow(ProviderError);
    });

    test('should handle QStash scheduling errors', async () => {
      mockQstashClient.schedules.create.mockRejectedValueOnce(new Error('QStash error'));
  ...
  describe('unscheduleWorkflow', () => {
    test('should unschedule workflow successfully', async () => {
      ...
      mockRedisClient.keys.mockResolvedValueOnce(['workflow:schedule:schedule-123']);
      ...
      expect(result).toBeTruthy();
      expect(mockRedisClient.del).toHaveBeenCalledWith('workflow:schedule:schedule-123');
    });
  ...
  describe('updateExecutionStatus', () => {
    test('should update execution status', async () => {
      ...
      await provider.updateExecutionStatus('execution-123', 'completed', 'step1', {
        result: 'success',
      });
  ...
  describe('error handling edge cases', () => {
    ...
    test('should handle batch processing in listExecutions', async () => {
      ...
      const mockPipeline = {
        ...
        exec: vi
          .fn()
          .mockResolvedValueOnce(executions.slice(0, 50).map(exec => [null, JSON.stringify(exec)]))
          .mockResolvedValueOnce(
            executions.slice(50, 100).map(exec => [null, JSON.stringify(exec)]),
          )
          .mockResolvedValueOnce(
            executions.slice(100, 150).map(exec => [null, JSON.stringify(exec)]),
          ),
      };
