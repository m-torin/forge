...
describe('batch Processing Patterns', () => {
  ...
  describe('batchManager', () => {
    ...
    describe('add and addMany', () => {
      test('should add single item to batch', async () => {
        ...
        expect(processor.processBatch).toHaveBeenCalledWith(['test-data', 'test-data-2']);
      });

      test('should add multiple items using addMany', async () => {
        const processor = {
          processBatch: vi.fn().mockResolvedValue(['result1', 'result2', 'result3']),
        };
    ...
    describe('batch management operations', () => {
      ...
      test('should reject new items after shutdown', async () => {
        ...
        await expect(manager.add('item')).rejects.toThrow('BatchManager is shutting down');
      });
  ...
  describe('integration Tests', () => {
    ...
    test('should handle mixed success and failure scenarios', async () => {
      ...
      await expect(promise2).rejects.toThrow('Individual processing failed for fail-item');
    });
    ...
    test('should handle concurrent batch operations', async () => {
      ...
      const promises = Array.from({ length: 8 }, (_, i) => manager.add(`item-${i}`));
