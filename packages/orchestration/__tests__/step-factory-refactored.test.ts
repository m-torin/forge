...
describe("step Factory System - DRY Refactored", () => {
  ...
  describe("template Tests", () => {
    const templateTests = [
      ...
      },
      {
        ...
        factory: () =>
          StepTemplates.notification("Send Alert", "Send notification alert"),
        expectedCategory: "notification",
        expectedTags: ["notification"],
      },
    ];

    templateTests.forEach(
      ({ name, factory, expectedCategory, expectedTags }) => {
        test(`should create ${name}`, () => {
          ...
          expect(step.metadata.tags).toStrictEqual(
  ...
  describe("registry Tests", () => {
    test("should register and retrieve steps", () => {
      ...
      const stepDef = createWorkflowStep(
        { name: step.name, version: "1.0.0" },
        async () => ({
          output: {},
          success: true,
          performance: { startTime: Date.now() },
  ...
  describe("error Handling Tests", () => {
    ...
    test("should handle execution errors", async () => {
      ...
      const step = createWorkflowStep(
        { name: stepData.name, version: "1.0.0" },
        async () => {
  ...
  describe("integration Tests", () => {
    test("should integrate with orchestration manager", async () => {
      ...
      const execution = await manager.executeWorkflow(workflow, {
        test: "data",
      });
    ...
    test("should handle random step configurations", () => {
      const randomSteps = Array.from({ length: 10 }, () =>
        stepGenerators.basic({
          ...
          action: testDataUtils.randomItem([
            "http",
            "database",
            "notification",
          ]),
