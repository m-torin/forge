...
vi.mock('../../src/shared/factories/step-factory/step-validation', async () => {
  const actual = await vi.importActual('../../src/shared/factories/step-factory/step-validation');
  return {
    ...actual,
...
describe('standardWorkflowStep', () => {
  ...
  describe('execute', () => {
    ...
    test('should validate output when configured', async () => {
      ...
      expect(validateStepOutput).toHaveBeenCalledWith('test-output', { schema: 'test' });
    });
  });
