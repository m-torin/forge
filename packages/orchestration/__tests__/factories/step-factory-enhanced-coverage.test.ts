...
vi.mock('../../src/shared/utils/errors', () => ({
  createOrchestrationError: vi.fn().mockImplementation(message => new Error(message)),
  createValidationError: vi.fn().mockImplementation(message => new Error(message)),
...
vi.mock('../../src/shared/factories/step-factory/step-validation', () => ({
  validateStepDefinition: vi.fn().mockReturnValue({ valid: true, errors: [] }),
...
describe('StandardWorkflowStep', () => {
  ...
  describe('execute', () => {
    ...
    it('should handle validation errors', async () => {
      ...
      vi.mocked(validateStepInput).mockRejectedValueOnce(new Error('Validation failed'));
...
describe('Step Enhancement Functions', () => {
  ...
  describe('withStepRetry', () => {
    ...
    it('should apply retry pattern to step execution', async () => {
      ...
      const enhancedStep = withStepRetry(stepFunction, { maxAttempts: 3, delay: 0 });
...
describe('Integration Tests', () => {
  it('should create and execute complex step with all patterns', async () => {
    ...
    // Apply patterns
    const enhancedStep = withStepRetry(
      withStepCircuitBreaker(
        withStepTimeout(withStepMonitoring(step.execute, { enableMetrics: true }), 5000),
        { failureThreshold: 3 },
      ),
      { maxAttempts: 2 },
    );
    ...
    const workflowStep = new StandardWorkflowStep({
      id: 'complex-workflow-step',
      name: 'Complex Workflow Step',
      execute: enhancedStep,
  ...
  it('should handle step failure with all error handling patterns', async () => {
    ...
    const workflowStep = new StandardWorkflowStep({
      id: 'failing-workflow-step',
      name: 'Failing Workflow Step',
  ...
  it('should compose multiple enhanced steps', async () => {
    ...
    const composedStep = compose(compose(enhancedStep1, enhancedStep2), enhancedStep3);
