...
// Attempt to import modern encryption functions
try {
  const { encryptFlagValues } = require('flags');
  encryptFunc = encryptFlagValues;
} catch (_error) {
...
export async function encryptFlags(values: Record<string, any>): Promise<string> {
  ...
  try {
    if (encryptFunc) {
      return await encryptFunc(values, secret);
 ...
 */
export function generateFlagsSecret(): string {
  try {
