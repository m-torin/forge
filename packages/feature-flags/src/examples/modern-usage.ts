...
import { modernEdgeConfigAdapter, modernPostHogAdapter } from '../adapters/modernAdapters';
import {
  analyticsConfig,
...
import { createModernFlagsDiscoveryEndpoint, getEnhancedProviderData } from '../server/discovery';
import {
  batchPrecompute,
  evaluateFlags,
  precomputeFlags,
...
export const recommendationAlgorithm = createVariantFlag(
  ...
  [
    ...
    { label: 'AI Enhanced', value: 'ai_enhanced' },
  ],
  {
...
export const flagsDiscoveryEndpoint = createModernFlagsDiscoveryEndpoint(
  async () => {
    return getEnhancedProviderData({
      posthog: {
