...
export async function trackFlagEvaluation(
  ...
  try {
    ...
    if (typeof window !== 'undefined' && window.va) {
      ...
      window.va('track', 'Feature Flag Evaluated', {
        flag_key: flagKey,
...
export async function trackExperimentAssignment(
  ...
  try {
    if (typeof window !== 'undefined' && window.va) {
      window.va('track', 'Experiment Assignment', {
...
export async function trackFlagConversion(
  ...
  try {
    ...
    if (typeof window !== 'undefined' && window.va) {
      window.va('track', eventData.event, eventData.properties);
    } else {
      await fetch('/_vercel/insights/track', {
        method: 'POST',
        headers: {
  ...
  try {
    ...
    if (typeof window !== 'undefined' && window.va) {
      ...
      batchData.forEach(data => {
        window.va?.('track', data.event, data.properties);
      });
    } else {
...
declare global {
  interface Window {
    va?: (event: string, eventName: string, properties?: Record<string, any>) => void;
  }
}
