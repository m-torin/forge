...
This package provides comprehensive analytics support for Next.js 15 applications with full App
Router, React Server Components, and middleware support.
...
```tsx
import { getServerFeatureFlag, getAllServerFeatureFlags } from '@repo/analytics/server';

export default async function FeaturePage() {
  // Single flag
  const showFeature = await getServerFeatureFlag('feature-key', process.env.POSTHOG_API_KEY!);

  // All flags
  const flags = await getAllServerFeatureFlags(process.env.POSTHOG_API_KEY!);
...
```ts
// middleware.ts
import { createAnalyticsMiddleware, composeMiddleware } from '@repo/analytics/server';
...
export const middleware = composeMiddleware(authMiddleware, analyticsMiddleware);
```
