...
export class PostHogClientProvider implements AnalyticsProvider, Partial<EnhancedPostHogProvider> {
  ...
  async track(event: string, properties: any = {}): Promise<void> {
    ...
    try {
      ...
      await this.reportError(_error, 'track', { event, properties });
    }
  }
  ...
  private async log(...args: any[]) {
    if (this.debugMode) {
      ...
      await logger.log('debug', 'PostHog Debug', { args });
    }
  }
  ...
  private async validateEventProperties(event: string, properties: any) {
    ...
    if (conflicts.length > 0) {
      ...
      await logger.log('warn', 'PostHog: Properties may conflict with reserved names', {
    ...
    try {
      ...
      await logger.log('warn', 'PostHog: Event properties contain circular references', {
        event,
  ...
  private async reportError(error: any, method: string, context: any) {
    if (this.debugMode) {
      ...
      await logger.log('error', 'PostHog Error', {
        method,
  ...
  async groupIdentify(
    ...
    try {
      ...
      await this.reportError(_error, 'groupIdentify', { groupKey, groupProperties, groupType });
    }
  }
}
