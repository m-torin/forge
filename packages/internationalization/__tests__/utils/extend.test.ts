...
describe("utils/extend", () => {
  test("createDictionary combines base and app dictionaries", async () => {
    ...
    const combinedDictionary = createDictionary(
      mockGetBaseDictionary,
      mockGetAppDictionary,
    );
  ...
  test("createDictionary handles async errors gracefully", async () => {
    const mockGetBaseDictionary = vi
      .fn()
      .mockRejectedValue(new Error("Base dictionary error"));
    ...
    await expect(combinedDictionary("en")).rejects.toThrow(
      "Base dictionary error",
    );
