...
i18nTestPatterns.testErrorHandling([
  ...
  },
  {
    ...
    operation: () => {
      const mockDictionaryLoader = createMockImplementations.dictionaryLoader();
      return mockDictionaryLoader.isLocaleSupported('invalid') ? 'invalid' : 'en';
    },
    expectedFallback: 'en',
  },
]);
...
i18nTestPatterns.testPerformance([
  ...
  },
  {
    ...
    operation: () => {
      ...
      for (let i = 0; i < 10; i++) {
        promises.push(Promise.resolve(mockDictionaryLoader.getDictionary('en')));
      }
  ...
  },
  {
    ...
    operation: () => {
      ...
      return testLocales.map(locale => mockDictionaryLoader.isLocaleSupported(locale));
    },
...
describe('server Edge Cases', () => {
  ...
  test('should handle large dictionary files', async () => {
    const mockLargeDictionary = createTestData.performanceData('large').dictionary;
