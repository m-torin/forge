...
describe("middleware Functionality", () => {
  ...
  test("should provide middleware configuration", async () => {
    ...
    expect(mockMiddleware.config.locales).toStrictEqual([
      "en",
      "fr",
      "es",
      "pt",
...
describe("middleware Integration", () => {
  test("should integrate with Next.js middleware", async () => {
    ...
    vi.mocked(
      require("next-international/middleware").createI18nMiddleware,
    ).mockImplementation(mockCreateI18nMiddleware);
    vi.mocked(require("@formatjs/intl-localematcher").match).mockImplementation(
      mockMatch,
    );
  ...
  test("should handle middleware error scenarios", async () => {
    ...
    expect(() => middlewareWithErrorHandling(invalidRequest)).toThrow(
      "Invalid request",
    );
  });
});
...
describe("middleware Edge Cases", () => {
  ...
  test("should handle very long Accept-Language headers", async () => {
    const longHeader =
      Array.from({ length: 50 }, (_, i) => `lang${i};q=0.${i % 10}`).join(",") +
      ",en;q=0.1";
