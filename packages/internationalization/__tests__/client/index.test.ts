...
i18nTestPatterns.testComponentI18n([
  {
    ...
    component: ({ href, children, locale }) => {
      ...
      const mockLink = vi
        .fn()
        .mockReturnValue(`<a href="/${locale}${href}">${children}</a>`);
...
i18nTestPatterns.testErrorHandling([
  ...
  },
  {
    ...
    operation: () => {
      const mockI18nClient = createMockImplementations.i18nClient(
        "invalid" as any,
      );
...
describe("client Integration Tests", () => {
  test("should integrate with Next.js navigation", async () => {
    ...
    vi.mocked(require("next/navigation").useParams).mockImplementation(
      mockUseParams,
    );
