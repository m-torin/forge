...
export function accountSecurityPlugin(options: AccountSecurityOptions = {}) {
  ...
  const checkAccountLockout = async (
    ...
    const resetAfterMs = config.resetFailedAttemptsAfter! * 60 * 1000;
  ...
  const recordFailedAttempt = async (email: string, ipAddress: string, userAgent: string) => {
    ...
    if (existing) {
      ...
      if (existing.attempts >= config.maxFailedAttempts!) {
        existing.lockedUntil = new Date(Date.now() + config.lockoutDuration! * 60 * 1000);
      }
