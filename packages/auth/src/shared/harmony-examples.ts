...
import { harmonyUtils, harmonyConfig } from './harmony';
...
export function exampleEmailValidation() {
  ...
  console.log('Email will be normalized:', willNormalize);

  // Get expected normalized email
  const normalized = harmonyUtils.getExpectedNormalizedEmail(email);
  console.log('Expected normalized email: ', normalized);
}
