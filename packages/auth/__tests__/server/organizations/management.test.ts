...
describe("organization Management", () => {
  ...
  describe("createOrganization", () => {
    test("should create organization successfully", async () => {
      ...
      vi.mocked(mockBetterAuthApi.createOrganization).mockResolvedValue(
        mockOrg,
      );
      ...
      expect(
        vi.mocked(mockBetterAuthApi.createOrganization),
    ...
    customTests: [
      {
        ...
        test: async () => {
          ...
          expect(
            vi.mocked(mockBetterAuthApi.updateOrganization),
          ).toHaveBeenCalledWith({
            body: {
              organizationId: "org-123",
              data: {
  ...
  createServerActionTestSuite({
    ...
    setup: () => {
      vi.mocked(mockBetterAuthApi.deleteOrganization).mockResolvedValue({
        success: true,
      });
    },
  });
  ...
  createServerActionTestSuite({
    ...
    expectedResult: { success: true },
    setup: () => {
      vi.mocked(mockBetterAuthApi.removeMember).mockResolvedValue({
        success: true,
      });
    },
  });
  ...
  createServerActionTestSuite({
    ...
    expectedResult: { success: true },
    setup: () => {
      vi.mocked(mockBetterAuthApi.cancelInvitation).mockResolvedValue({
        success: true,
      });
    },
  });
  ...
  createServerActionTestSuite({
    ...
    expectedResult: { success: true },
    setup: () => {
      vi.mocked(mockBetterAuthApi.rejectInvitation).mockResolvedValue({
        success: true,
      });
    },
  });
