...
describe("shared Utilities Integration", () => {
  describe("aPI Key Utilities", () => {
    test("should mask API keys correctly in various formats", async () => {
      ...
      expect(maskApiKey("pk_test_1234567890abcdef")).toBe(
        "pk_test_************cdef",
      );
      expect(maskApiKey("key_123456789012345678901234")).toBe(
        "key_1234****************1234",
      );
    ...
    test("should generate valid API key names", async () => {
      const { generateApiKeyName } = await import(
    ...
    test("should format permissions for display", async () => {
      const { formatPermissionsForDisplay, groupPermissionsByResource } =
        await import("../../src/shared/utils/api-keys");
    ...
    test("should validate API key format", async () => {
      const { isValidApiKeyFormat } = await import(
        "../../src/shared/utils/api-keys"
      );

      // Valid formats
      expect(
        isValidApiKeyFormat("sk-1234567890abcdef1234567890abcdef"),
      ).toBeTruthy();
      expect(
        isValidApiKeyFormat("pk_test_1234567890abcdef1234567890abcdef"),
      ).toBeTruthy();
      expect(
        isValidApiKeyFormat("key_123456789012345678901234567890"),
      ).toBeTruthy();
  ...
  describe("integration between utilities", () => {
    test("should work together for complete auth workflows", async () => {
      const { maskApiKey, isValidApiKeyFormat } = await import(
        "../../src/shared/utils/api-keys"
      );
