...
describe("shared teams functionality", () => {
  ...
  describe("tEAM_PERMISSIONS", () => {
    ...
    test("should have string values for all permissions", async () => {
      ...
      Object.values(permissions).forEach((category) => {
        Object.values(category as Record<string, string>).forEach(
          (permission) => {
            expect(typeof permission).toBe("string");
            expect(permission.length).toBeGreaterThan(0);
          },
  ...
  describe("roleHasPermission", () => {
    ...
    test("should handle invalid permissions", async () => {
      ...
      const result = teamsModule.roleHasPermission(
        "owner",
        "invalid:permission",
      );
  ...
  describe("canActOnUser", () => {
    test("should allow owner to act on any role", async () => {
      ...
      roles.forEach((role) => {
        const result = teamsModule.canActOnUser(
          "owner",
          role,
          "members:remove",
        );
    ...
    test("should not allow lower roles to act on higher roles", async () => {
      ...
      const result1 = teamsModule.canActOnUser(
        "admin",
        "owner",
        "members:remove",
      );
      const result2 = teamsModule.canActOnUser(
        "member",
        "admin",
        "members:remove",
      );
  ...
  describe("getPermissionsForRole", () => {
    ...
    test("should match DEFAULT_TEAM_ROLES permissions", async () => {
      ...
      const expectedPermissions =
        teamsModule.DEFAULT_TEAM_ROLES.admin.permissions;
  ...
  describe("edge cases", () => {
    test("should handle null and undefined inputs", async () => {
      ...
      expect(
        teamsModule.roleHasPermission(null as any, "team:read"),
      ).toBeFalsy();
