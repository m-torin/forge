...
describe("access Control and Permissions", () => {
  ...
  describe("roles Export", () => {
    ...
    test("should have the correct role hierarchy", () => {
      ...
      expect(owner.statements.billing.length).toBeGreaterThan(
        admin.statements.billing.length,
      );
      expect(owner.statements.project.length).toBeGreaterThan(
        admin.statements.project.length,
      );

      // Admin should have more permissions than member
      expect(admin.statements.billing.length).toBeGreaterThan(
        member.statements.billing.length,
      );
      expect(admin.statements.project.length).toBeGreaterThan(
        member.statements.project.length,
      );
      expect(admin.statements.settings.length).toBeGreaterThan(
        member.statements.settings.length,
      );
    });
  });
