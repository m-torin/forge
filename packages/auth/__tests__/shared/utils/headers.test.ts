...
describe("header utilities", () => {
  ...
  describe("header utility integration", () => {
    test("should create distinct header objects", async () => {
      ...
      expect((apiHeaders as Record<string, string>).Authorization).toBe(
        "Bearer api-key",
      );
      expect((bearerHeaders as Record<string, string>).Authorization).toBe(
        "Bearer bearer-token",
      );
      expect(
        (jsonHeaders as Record<string, string>).Authorization,
      ).toBeUndefined();

      // All should have JSON content type
      expect((apiHeaders as Record<string, string>)["Content-Type"]).toBe(
        "application/json",
      );
      expect((bearerHeaders as Record<string, string>)["Content-Type"]).toBe(
        "application/json",
      );
      expect((jsonHeaders as Record<string, string>)["Content-Type"]).toBe(
        "application/json",
      );
    });
