...
vi.mock("@repo/observability/server/next", () => ({
  logInfo: vi.fn((message: string, ...args: any[]) =>
    console.info(message, ...args),
  ),
  logError: vi.fn((message: string, ...args: any[]) =>
...
describe("middleware Factory", () => {
  ...
  beforeEach(() => {
    ...
    const mockApiMiddlewareInstance = vi
      .fn()
      .mockResolvedValue(NextResponse.next());
    const mockWebMiddlewareInstance = vi
      .fn()
      .mockResolvedValue(NextResponse.next());
    const mockNodeMiddlewareInstance = vi
      .fn()
      .mockResolvedValue(NextResponse.next());
  ...
  describe("createCombinedMiddleware", () => {
    ...
    test("should route API requests to API middleware", async () => {
      ...
      const apiMiddlewareInstance =
        mockCreateApiMiddleware.mock.results[0].value;
    ...
    test("should route web requests to web middleware", async () => {
      ...
      const webMiddlewareInstance =
        mockCreateWebMiddleware.mock.results[0].value;
  ...
  describe("createSmartMiddleware", () => {
    test("should detect nodejs runtime automatically", () => {
      ...
      const middleware = createSmartMiddleware(mockAuthConfig, {
        runtime: "auto",
      });
