...
export class Timer {
  ...
  duration(name?: string): number {
    const end = name && this.marks.has(name) ? this.marks.get(name)! : Date.now();
...
/**
 * HTML extraction utilities from original package
 */
import type { SelectorConfig, SelectorMap } from '../types/scraping-types';
