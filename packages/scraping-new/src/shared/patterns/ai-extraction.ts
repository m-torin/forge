...
import type { AIExtractionOptions, AIExtractionResult } from './types';
...
export async function extractWithAI(
  ...
  const { model = 'auto', schema, confidence = 0.8 } = options;
...
async function extractWithHero(
  ...
  try {
    ...
    throw new ScrapingError(
      `Hero AI extraction failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
      ScrapingErrorCode.AI_EXTRACTION_FAILED,
      { provider: 'hero' },
      error instanceof Error ? error : undefined,
    );
...
Return only the JSON data that matches this schema exactly.`;
  ...
  throw new ScrapingError(
    ...
    { schema, extractedData: result.data },
  );
}
...
export async function extractWithConsensus(
  ...
  if (results.length === 0) {
    throw new ScrapingError(
      ...
      ScrapingErrorCode.AI_EXTRACTION_FAILED,
      { models, errors: errors.map((e) => e.message) },
    );
  }
