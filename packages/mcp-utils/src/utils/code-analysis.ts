...
export function extractImports(content: string): string[] {
  ...
  try {
    // Try TypeScript/modern JS first with ts-morph
...
export function calculateComplexity(content: string): number {
  ...
  try {
    ...
    try {
      ...
      complexity = calculateComplexityFromAST(sourceFile);
    } catch (tsMorphError) {
      // Fallback to JavaScript parsing with acorn
      try {
        const ast = parseJS(content, {
...
function calculateComplexityFromAST(sourceFile: SourceFile): number {
  ...
  sourceFile.forEachDescendant(node => {
    ...
    if (Node.isIfStatement(node)) {
      ...
      const caseCount = node.getClauses().filter(clause => Node.isCaseClause(clause)).length;
