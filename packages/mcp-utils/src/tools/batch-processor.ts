...
import { err, ok, runTool } from '../utils/tool-helpers';
    ...
    return runTool('batch_processor', args.action, async () => {
      ...
      switch (action) {
        ...
        case 'filterCached': {
          ...
          // Simple cache simulation (in real implementation, this would check actual cache)
          ...
          for (const file of files) {
            ...
            const shouldCache =
              !file.includes('test') && !file.includes('spec') && Math.random() > 0.7;
            ...
            const queuedResults = await ioQueue.addAll(
              files.map(file => () => {
                ...
                return new Promise<any>((resolve, reject) => {
                  setImmediate(() => {
                    try {
                      const fileSize = fileSizes?.[file] || Math.floor(Math.random() * 500) + 50;
