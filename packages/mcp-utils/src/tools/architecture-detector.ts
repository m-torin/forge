  ...
  async execute(args: ArchitectureDetectorArgs): Promise<MCPToolResponse> {
    try {
      ...
      switch (action) {
        ...
        default:
          throw new Error(
            `Unknown action: ${action}. Supported actions: detectArchitecture, detectStateManagement, detectStyling, detectTesting, detectPort, detectPatterns`,
          );
      }
    } catch (error) {
      return {
        content: [
          {
            type: 'text',
            text: JSON.stringify({
              error: `Architecture detector error: ${error instanceof Error ? error.message : 'Unknown error'}`,
            }),
          },
        ],
        isError: true,
