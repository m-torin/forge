...
export const optimizationEngineTool = {
  ...
  async execute(args: OptimizationEngineArgs) {
    try {
      ...
      queueMicrotask(() => {
        process.stderr.write(
          `[${timestamp}] Optimization Engine: ${action} (session: ${sessionId})\n`,
        );
      });

      switch (action) {
        case 'analyzeBundleSize':
          return await analyzeBundleSize(packagePath, sessionId, args.buildResults, options);

        case 'detectCodeSplitting':
          return await detectCodeSplitting(packagePath, sessionId, options);

        case 'analyzeDynamicImports':
          return await analyzeDynamicImports(packagePath, sessionId, options);

        case 'optimizeAssetLoading':
          return await optimizeAssetLoading(packagePath, sessionId, options);

        case 'calculateTreeShaking':
          return await calculateTreeShaking(packagePath, sessionId, args.bundleAnalysis, options);

        case 'analyzePerformanceBottlenecks':
          return await analyzePerformanceBottlenecks(
            packagePath,
            sessionId,
        ...
        case 'profileRuntimePerformance':
          return await profileRuntimePerformance(packagePath, sessionId, options);
        ...
        case 'detectMemoryLeaks':
          return await detectMemoryLeaks(packagePath, sessionId, options);

        case 'analyzeRenderPerformance':
          return await analyzeRenderPerformance(packagePath, sessionId, options);

        case 'optimizeForVercel':
          return await optimizeForVercel(packagePath, sessionId, options);

        case 'optimizeForCloudflare':
          return await optimizeForCloudflare(packagePath, sessionId, options);

        case 'optimizeForAWS':
          return await optimizeForAWS(packagePath, sessionId, options);

        case 'optimizeForNetlify':
          return await optimizeForNetlify(packagePath, sessionId, options);

        case 'optimizeForDocker':
          return await optimizeForDocker(packagePath, sessionId, options);

        case 'analyzeBuildTime':
          return await analyzeBuildTime(packagePath, sessionId, args.buildResults, options);

        case 'optimizeBuildPipeline':
          return await optimizeBuildPipeline(packagePath, sessionId, options);

        case 'analyzeCachingStrategy':
          return await analyzeCachingStrategy(packagePath, sessionId, platform, options);

        case 'optimizeCDNUsage':
          return await optimizeCDNUsage(packagePath, sessionId, options);

        case 'analyzeCompressionEfficiency':
          return await analyzeCompressionEfficiency(
            packagePath,
            sessionId,
            args.buildResults,
            options,
        ...
        case 'detectPerformanceAntiPatterns':
          return await detectPerformanceAntiPatterns(packagePath, sessionId, options);

        case 'analyzeImageOptimization':
          return await analyzeImageOptimization(packagePath, sessionId, options);

        case 'optimizeCSSDelivery':
          return await optimizeCSSDelivery(packagePath, sessionId, options);

        case 'analyzeJavaScriptOptimization':
          return await analyzeJavaScriptOptimization(packagePath, sessionId, options);

        case 'detectUnusedCode':
          return await detectUnusedCode(packagePath, sessionId, options);

        case 'comprehensiveOptimization':
          return await comprehensiveOptimization(packagePath, sessionId, platform, options);

        case 'quickPerformanceAudit':
          return await quickPerformanceAudit(packagePath, sessionId, options);

        case 'deploymentOptimization':
          return await deploymentOptimization(packagePath, sessionId, platform, options);

        case 'generateOptimizationReport':
          return await generateOptimizationReport(
            packagePath,
            sessionId,
            args.performanceMetrics,
            options,
      ...
      if (error instanceof Error && error.message.includes('aborted')) {
        return err('Operation aborted', args.action, 'Optimization Engine');
      }

      return createEnhancedMCPErrorResponse(error, 'optimization_engine', {
        contextInfo: `Optimization Engine - ${args.action} at ${args.packagePath || 'unknown path'}`,
      });
    }
  },
};
