...
export class AdvancedMemoryMonitor {
  ...
  getMemoryPressure(): MemoryPressureLevel {
    ...
    let level: MemoryPressureLevel['level'] = 'optimal';
        ...
        content: [
          {
            type: 'text',
            text: JSON.stringify({
              error: `Advanced memory monitor error: ${error instanceof Error ? error.message : 'Unknown error'}`,
            }),
          },
        ],
        isError: true,
