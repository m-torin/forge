    ...
    try {
      ...
      queueMicrotask(() => {
        process.stderr.write(
          `[${timestamp}] Pattern Analyzer: ${action} (session: ${sessionId})\n`,
        );
      });
      ...
      return createEnhancedMCPErrorResponse(error, 'pattern_analyzer', {
        contextInfo: `Pattern Analyzer - ${args.action} at ${args.packagePath || 'unknown path'}`,
      });
    }
  },
};
