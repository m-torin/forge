...
export const contextManagerTool = {
  ...
  async execute(args: ContextManagerArgs): Promise<MCPToolResponse> {
    try {
                  ...
                  history: history.map(snapshot => ({
                    ...
                    duration:
                      snapshot.context.performanceMarks.length > 1
                        ? snapshot.context.performanceMarks[
                            snapshot.context.performanceMarks.length - 1
      ...
      const logger = createContextLogger('context-manager');
      logger.error(
      ...
      return createMCPErrorResponse(
        ...
        'context_manager',
        {
          contextInfo: `Context Manager - ${getCurrentContext() ? 'with context' : 'without context'}`,
        },
      );
    }
  },
