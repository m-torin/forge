...
const demoBreadcrumbs = [{ id: 1, name: 'Home', href: '/' as Route<string> }];

interface BreadcrumbProps {
  breadcrumbs?: { id: number; name: string; href: Route<string> }[];
  className?: string;
  currentPage?: string;
}
        ...
        {breadcrumbs.map((breadcrumb) => (
          <li key={breadcrumb.id}>
            <div className="flex items-center gap-x-2">
              {breadcrumb.href ? (
                <NextLink href={breadcrumb.href}>{breadcrumb.name}</NextLink>
              ) : (
