...
import { Cancel01Icon, DollarCircleIcon, Note01Icon, PaintBucketIcon, PercentCircleIcon, ResizeFieldRectangleIcon, SortingAZ02Icon } from '@hugeicons/core-free-icons';
...
const TabFilters = ({ className }: { className?: string }) => {
  ...
  const renderXClear = () => {
    return (
      <span className="ms-3 flex h-4 w-4 shrink-0 cursor-pointer items-center justify-center rounded-full bg-primary-500 text-white">
        <HugeiconsIcon icon={Cancel01Icon} strokeWidth={1.5} color="currentColor" size={10} />
      </span>
    );
  };
  ...
  const renderTabIsOnsale = () => {
    return (
      ...
      >
        <HugeiconsIcon icon={PercentCircleIcon} strokeWidth={1.5} color="currentColor" size={16} />
...
const FieldPopover = ({
  ...
  return (
    <Popover className="group relative">
      ...
      >
        <ButtonIcon strokeWidth={1.5} color="currentColor" size={16} />
