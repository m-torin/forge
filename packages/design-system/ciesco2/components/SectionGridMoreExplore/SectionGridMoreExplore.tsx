...
const SectionGridMoreExplore: FC<SectionGridMoreExploreProps> = ({
  ...
  const renderCollectionCard = (collection: TCollection, index: number) => {
    switch (boxCard) {
      ...
      case 'box6':
        return <CollectionCard6 collection={collection} bgSvgUrl={typeof svgImages[index] === 'string' ? svgImages[index] : (svgImages[index] as any)?.src || svgs1} />;
      default:
        return <CollectionCard4 collection={collection} bgSvgUrl={typeof svgImages[index] === 'string' ? svgImages[index] : (svgImages[index] as any)?.src || svgs1} />;
    }
  };
  ...
  return (
    <div className={clsx('relative', className)}>
      ...
      <div className="mt-20 flex justify-center">
        <ButtonSecondary href={"/collections/all" as any}>
