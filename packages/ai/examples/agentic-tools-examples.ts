/**
 * Agentic Tools Examples - AI SDK v5 Compliant
 *
...
async function conditionalToolsExample() {
  ...
  const tools = {
    ...
    detailedSearch: agenticTool({
      description: 'Perform detailed search with filtering',
      inputSchema: z.object({
        query: z.string(),
        filters: z.array(z.string()).optional(),
        sortBy: z.string().optional(),
      }),
      execute: async ({ query, filters: _filters, sortBy: _sortBy }) => ({
    ...
    expertAnalysis: agenticTool({
      ...
      execute: async ({ data: _data, analysisDepth }) => ({
...
async function productionToolExample() {
  ...
  const productionTool = agenticTool({
    ...
    execute: async ({ data: _data, processingMode, errorHandling }) => {
 ...
 */
async function runAgenticToolExamples() {
  console.log('🚀 Agentic Tools Examples - AI SDK v5 Compliant');
