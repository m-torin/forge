/**
 * Agent + Computer Use Integration
 ...
 * Main example runner
 */
export async function runComputerUseExamples() {
  console.log('Running agent + computer use examples...\n');
...
export const dynamicPatterns = {
   ...
   */
  resourceAware: (limits: { maxOutputTokens: number; maxTime: number }) => ({
    stopWhen: [
      ({ usage }: any) => (usage?.totalTokens || 0) > limits.maxOutputTokens,
