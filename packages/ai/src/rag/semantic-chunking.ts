...
export class SemanticDocumentChunker {
   ...
   */
  private async semanticSimilarityChunking(
    content: string,
  ): Promise<ChunkResult[]> {
    const sentences = this.splitIntoSentences(content);
    ...
    for (let i = 1; i < sentences.length; i++) {
      ...
      if (
        ...
        currentChunk.length > (this.config.maxChunkSize || 2000)
      ) {
        // Create chunk
        const chunk: ChunkResult = {
          content: currentChunk.trim(),
    ...
    for (let sectionIndex = 0; sectionIndex < sections.length; sectionIndex++) {
      ...
      for (const paragraph of paragraphs) {
        if (paragraph.length > (this.config.maxChunkSize || 1500)) {
          ...
          const sentenceChunks = this.groupSentences(
            sentences,
            this.config.chunkSize || 800,
          );
