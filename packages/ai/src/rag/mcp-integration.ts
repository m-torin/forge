...
export class MCPRAGBridge {
    ...
    return tool({
      ...
      inputSchema: z.object({
        ...
        maxRAGResults: z
          .number()
          .default(3)
          .describe("Maximum RAG context items to include"),
      }),
      execute: async (params: any) => {
        const { includeRAGContext, ragQuery, maxRAGResults, ...toolParams } =
          params;
        ...
        if (includeRAGContext) {
          try {
            const searchQuery =
              ragQuery || this.extractQueryFromParams(toolParams);
  ...
  createContextSharingTool() {
    return tool({
      description:
        "Share knowledge base context with MCP tools for enhanced responses",
      inputSchema: z.object({
        ...
        toolParams: z
          ...
          .describe("Parameters for the target tool"),
        maxContextItems: z
          .number()
          .default(5)
          .describe("Maximum context items to share"),
      }),
      execute: async ({
        targetTool,
        contextQuery,
        ...
        try {
          ...
          return {
            ...
            averageRelevance:
              ragContext.reduce((sum, item) => sum + item.score, 0) /
              ragContext.length,
  ...
  getServerStatus(): Array<{
    ...
    return Array.from(this.connectedServers.entries()).map(
      ([name, server]) => ({
        name,
