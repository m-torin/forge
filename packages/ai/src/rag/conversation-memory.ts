...
export class ConversationMemoryManager {
  ...
  async getRelevantContext(
    ...
    if (opts.includeCurrentConversation) {
      const conversation = await this.getOrCreateConversation(conversationId);
      context.conversationContext = this.selectRelevantMessages(
    ...
    if (opts.includeSummaries) {
      context.summaryContext = await this.getRelevantSummaries(
        conversationId,
        query,
      );
    }
    ...
    if (opts.includeRelatedConversations) {
      context.relatedContext = await this.getRelatedConversationContext(
        conversationId,
        query,
      );
    }
    ...
    context.totalContextLength =
      context.conversationContext.reduce(
        (sum, msg) => sum + msg.content.length,
        0,
      ) +
      context.summaryContext.reduce(
  ...
  async createContextSummary(
    ...
    if (
      !conversation ||
      conversation.messages.length < this.config.summaryInterval
    ) {
      return null;
    }
    ...
    const topics = this.config.enableTopicExtraction
      ? this.extractTopics(messagesToSummarize)
      : [];
  ...
  private trimContextByPriority(
    ...
    const sortedConversation = [...conversationContext].sort(
  ...
  private extractEntities(messages: ConversationMessage[]): string[] {
    ...
    const entities = words.filter(
      (word) => /^[A-Z][a-z]+$/.test(word) && word.length <= 30,
    );
  ...
  private async saveConversation(
    _conversation: ConversationThread,
  ): Promise<void> {
    // Implementation depends on your storage backend
  }
...
export function formatConversationContext(context: {
  ...
  if (context.relatedContext.length > 0) {
    ...
    context.relatedContext.forEach((message) => {
      parts.push(
        `**${message.role}**: ${message.content.substring(0, 200)}...`,
      );
    });
  }
