...
export class RAGService {
  ...
  async searchContext(
    ...
    const chunks: RAGDocumentChunk[] = relevantResults.map((result: any) => ({
      id: result.id,
    ...
    const relevanceScores = relevantResults.map((result: any) => result.score);
    const sources = [
      ...new Set(
        chunks.map((chunk) => chunk.metadata.title).filter(Boolean) as string[],
      ),
    ];
...
export function createRAGSystem(
  ...
  return {
    ...
    async query(
      query: string,
      options?: Parameters<RAGService["searchContext"]>[1],
    ) {
...
// Additional exports for server/index.ts compatibility
export class VectorRAGWorkflow {
  constructor(private config: RAGWorkflowConfig) {}
...
export function createRAGWorkflow(config: RAGWorkflowConfig) {
...
export async function quickRAG(
  ...
  const workflow = createRAGWorkflow(config);
  return workflow.process(query);
}

// Re-export types from vector (RAGContext defined above, so only export the others)
export type { RAGResponse, RAGWorkflowConfig } from "../types/vector";
