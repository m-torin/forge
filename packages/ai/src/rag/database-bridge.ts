...
import {
  UpstashAIVector as AIEnhancedVectorOperations,
  UpstashAIVector,
  type UpstashAIConfig as AIVectorConfig,
} from '../vector/ai-sdk-integration';
import { ragRetry } from './retry-strategies';

// Helper function to create AI Vector from environment
const createAIVectorFromEnv = (embeddingModel?: any, namespace?: string): UpstashAIVector => {
...
export class RAGDatabaseBridge {
    ...
    try {
      const result = await ragRetry.vector(async () =>
        this.config.useUpstashEmbedding
          ? await (this.vectorOps as any).upsertWithUpstashEmbedding(documents, {
              namespace: options?.namespace || this.config.namespace,
              batchSize: options?.batchSize,
            })
          : await (this.vectorOps as any).upsertWithEmbedding(documents, {
              namespace: options?.namespace || this.config.namespace,
              batchSize: options?.batchSize,
            }),
      );
  ...
  > {
    try {
      const results = await ragRetry.vector(
        async () =>
          await (this.vectorOps as any).semanticSearchWithAI(query, {
            topK: options?.topK || 5,
            namespace: options?.namespace || this.config.namespace,
  ...
  async batchAddDocuments(
    ...
    try {
      const result = await (this.vectorOps as any).batchProcessDocuments(documents, {
        namespace: options?.namespace || this.config.namespace,
  ...
  > {
    try {
      ...
      const results = await (this.vectorOps as any).query({
      ...
      return documents.filter((doc: any) => doc.content); // Only return docs with content
    } catch (error) {
  ...
  > {
    try {
      const results = await (this.vectorOps as any).fetch(Array.isArray(ids) ? ids : [ids], {
        namespace: options?.namespace || this.config.namespace,
  ...
  async deleteDocuments(
    ...
    try {
      const result = await (this.vectorOps as any).delete(ids, {
        namespace: options?.namespace || this.config.namespace,
      });
  ...
  async updateDocument(
    ...
    try {
      const result = await (this.vectorOps as any).update(id, {
  ...
  async getStoreInfo(): Promise<{
    ...
    try {
      const info = await (this.vectorOps as any).info();
      const embeddingModelInfo = (this.vectorOps as any).getEmbeddingModelInfo();
  ...
  async listNamespaces(): Promise<string[]> {
    try {
      const namespaces = await (this.vectorOps as any).listNamespaces();
  ...
  > {
    try {
      const results = await (this.vectorOps as any).findSimilar(referenceId, {
        topK: options?.topK || 5,
        namespace: options?.namespace || this.config.namespace,
        threshold: options?.threshold || 0.7,
      });
  ...
  updateConfig(newConfig: Partial<RAGDatabaseConfig>): void {
    ...
    if (newConfig.vectorConfig || newConfig.embeddingModel) {
      (this.vectorOps as any).updateConfig({
...
export class UpstashAIVectorAdapter {
  ...
  constructor(config: {
    ...
    this.bridge = new RAGDatabaseBridge({
      ...
      vectorConfig: {
        // dimensions property not supported in UpstashAIConfig
        namespace: config.namespace,
      } as any,
    });
  }
