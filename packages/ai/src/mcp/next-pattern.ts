 ...
 * ```
 */
export async function createMCPToolsForStreamText(
...
export async function testMCPConnectivity(configs: MCPClientConfig[]): Promise<{
  ...
  for (const config of configs) {
    try {
      const { tools, closeAllClients } = await createMCPToolsFromConfigs([
        config,
      ]);
...
export async function createMCPToolsWithStreamLifecycle(
  ...
  const lifecycleManager = createMCPStreamLifecycleManager(
    configs,
    lifecycleConfig,
  );

  // Initialize with full lifecycle management
  const { tools, clients, streamCallbacks } =
