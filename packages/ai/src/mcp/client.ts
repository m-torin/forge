...
function isRetryableError(error: Error, retryableErrors: string[]): boolean {
  ...
  return retryableErrors.some(
    (pattern) =>
      errorMessage.includes(pattern.toUpperCase()) || errorCode === pattern,
  );
}
    ...
    try {
      ...
      await withTimeout(
        async () => {
          ...
          const tempClient = (await experimental_createMCPClient({
            transport,
          })) as any;
  ...
  switch (transportConfig.type) {
    ...
    case "http":
      if (!transportConfig.httpUrl) {
        throw new MCPTransportError(
          "HTTP URL is required for HTTP transport",
          "http",
        );
      }
      try {
        ...
        throw new MCPTransportError(
          "Invalid HTTP URL format for HTTP transport",
          "http",
        );
      }
      break;
...
export function createTransportFromConfig(
  ...
  switch (transportConfig.type) {
    ...
    case "http":
      ...
      throw new Error(
        "HTTP transport not currently supported. Use stdio or sse instead.",
      );
...
export class MCPRequestManager {
  ...
  async addClient(config: MCPClientConfig): Promise<void> {
    ...
    try {
      ...
      if (config.gracefulDegradation) {
        logWarn(
          `MCP client '${config.name}' failed, continuing with degraded functionality`,
          {
  ...
  createAISDKErrorHandlers(): {
    ...
    const onUncaughtError = (error: Error) => {
      ...
      for (const [_clientName, errorHandler] of this.errorHandlers.entries()) {
        const handler = errorHandler.createUncaughtErrorHandler(
          () => this.close(),
          {
            enableRecovery: true,
    ...
    const onStreamError = (error: Error) => {
      ...
      for (const [_clientName, errorHandler] of this.errorHandlers.entries()) {
        const handler = errorHandler.createStreamErrorHandler(() =>
          this.close(),
        );
        handler(error);
      }
 ...
 */
export async function createMCPToolsForRequest(
...
export async function createMCPToolsFromConfigs(
  ...
  const clientPromises = configs.map(
    async (config): Promise<MCPClientResult | null> => {
      const retryConfig = config.retry || DEFAULT_RETRY_CONFIG;
        ...
        const result = await withRetryAndTimeout(
          async (): Promise<MCPClientResult> => {
            ...
            const toolSet = await withRetryAndTimeout(
              () =>
                (client?.tools() as Promise<Record<string, any>>) ??
                Promise.resolve({}),
              retryConfig,
              timeoutMs,
              `MCP tools retrieval (${config.name})`,
