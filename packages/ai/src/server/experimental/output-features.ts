...
export interface EnhancedOutputResult<T = any> {
  original: T;
  transformed: T;
  metadata: {
...
export class OutputProcessor {
  ...
  /**
   * Enhanced text generation with output processing
   */
  async generateEnhancedText(
    prompt: string,
    options?: Parameters<typeof generateText>[0] & {
      outputTransforms?: OutputTransformConfig;
    },
  ): Promise<EnhancedOutputResult<string>> {
    ...
    const metadata: EnhancedOutputResult['metadata'] = {
      transformsApplied,
      processingTime: 0,
    };
  ...
  /**
   * Enhanced object generation with validation and repair
   */
  async generateEnhancedObject<T>(
  ...
  ): Promise<EnhancedOutputResult<T>> {
    const startTime = Date.now();
    const transformsApplied: string[] = [];
  ...
  /**
   * Enhanced streaming with real-time processing
   */
  async streamEnhancedText(
    prompt: string,
 ...
 */
export const enhancedGeneration = {
  /**
  ...
  async text(
    ...
    return processor.generateEnhancedText(prompt);
  },
  ...
  async object<T>(
    ...
    return processor.generateEnhancedObject(
      schema,
      prompt,
  ...
  async stream(
    ...
    return processor.streamEnhancedText(prompt, {
