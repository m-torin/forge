...
export class RAGQualityTester {
  ...
  static async testRetrievalAccuracy(
    vectorStore: UpstashAIVector | MockVectorStore,
    testCases: Array<{
      query: string;
    ...
    for (const testCase of testCases) {
      const retrievalResults = await vectorStore.queryWithEmbedding(testCase.query, { topK: 10 });
      const retrievedIds = retrievalResults
        .map(r => r.metadata?.id || r.metadata?.documentId)
        .filter(Boolean);
