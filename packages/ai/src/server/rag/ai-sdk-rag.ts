...
export class AISDKRag {
  ...
  async addDocument(
    ...
    const tools = createUpstashVectorTools(this.vectorConfig);
    return tools.addDocument.execute({ content, title, metadata });
  }
  ...
  > {
    const tools = createUpstashVectorTools(this.vectorConfig);
    const results = await tools.getInformation.execute({ question: query, topK });
...
export function createAISDKRagFromEnv(options?: {
  ...
  return new AISDKRag({
    vectorUrl,
    vectorToken,
    namespace: options?.namespace ?? env.UPSTASH_VECTOR_NAMESPACE,
