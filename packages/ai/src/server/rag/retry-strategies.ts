...
import { executeWithCircuitBreaker, type CircuitBreakerConfig } from './circuit-breaker';
...
export class ConfigurableRetry {
  ...
  categorizeError(error: Error): ErrorType {
    ...
    if (name.includes('circuitbreaker') || message.includes('circuit breaker')) {
...
export async function retryWithCircuitBreaker<T>(
  ...
  if (!result.success) {
    ...
    throw result.errors[result.errors.length - 1] || new Error('All retry attempts failed');
  }
...
export const ragRetry = {
  ...
  embedding: async <T>(operation: () => Promise<T>): Promise<T> => {
    ...
    return retryWithCircuitBreaker('embedding', operation, retryInstance.config as any, {
      failureThreshold: 3,
  ...
  vector: async <T>(operation: () => Promise<T>): Promise<T> => {
    ...
    return retryWithCircuitBreaker('vector_operations', operation, retryInstance.config as any, {
      failureThreshold: 5,
  ...
  batch: async <T>(operation: () => Promise<T>): Promise<T> => {
    ...
    return retryWithCircuitBreaker('batch_operations', operation, retryInstance.config as any, {
      failureThreshold: 2,
