...
export class RAGSchemaValidator {
  ...
  async validateResponse<T extends z.ZodType>(
    ...
    try {
      ...
      if (error instanceof z.ZodError) {
        result.errors = error.issues.map((err: any) => `${err.path.join('.')}: ${err.message}`);
      } else {
      ...
      if (!this.config.strictMode) {
        try {
          ...
          if (partialData) {
            ...
            result.warnings.push('Partial data extracted despite validation errors');
          }
        } catch {
  ...
  private async assessQuality<T>(
    ...
    if (context?.sources) {
      const avgSourceScore =
        context.sources.reduce((sum, s) => sum + s.score, 0) / context.sources.length;
      metrics.sourceReliability = Math.min(avgSourceScore * 1.2, 1.0); // Boost slightly, cap at 1.0
    }

    // Assess structural integrity
    metrics.structuralIntegrity = this.assessStructuralIntegrity(response, schema);

    // Assess content relevance (basic heuristics)
    if (context?.query && typeof response === 'object' && response !== null) {
      metrics.contentRelevance = this.assessContentRelevance(response, context.query);
    }
  ...
  private assessStructuralIntegrity(response: any, _schema: z.ZodType): number {
    ...
    const hasRequiredField = commonRequiredFields.some(field => field in response);
  ...
  private assessContentRelevance(response: any, query: string): number {
    ...
    const lengthFactor = responseLength < 100 ? 0.7 : responseLength < 300 ? 0.85 : 1.0;
  ...
  private async performContentValidation<T>(
    ...
    if ('answer' in obj && typeof obj.answer === 'string') {
      ...
      if (obj.answer.toLowerCase().includes("i don't know") && obj.answer.length < 50) {
    ...
    if ('sources' in obj && Array.isArray(obj.sources)) {
      if (obj.sources.length === 0) {
        ...
        if (emptySources.length > 0) {
          result.warnings.push(`${emptySources.length} sources have empty content`);
        }
      }
    }
    ...
    if (this.config.customValidators) {
      for (const [validatorName, validator] of Object.entries(this.config.customValidators)) {
        try {
          ...
          result.warnings.push(`Custom validator '${validatorName}' threw error: ${error}`);
        }
      }
    }
  }
...
export const commonValidators = {
  ...
  qaResponse: new RAGSchemaValidator({
    ...
    customValidators: {
      hasSubstantialAnswer: (response: any) =>
        response?.answer && typeof response.answer === 'string' && response.answer.length >= 20,
  ...
  analysisResponse: new RAGSchemaValidator({
    ...
    customValidators: {
      ...
      hasSubstantialAnswer: (response: any) => response?.answer && response.answer.length >= 50,
    },
  }),
  ...
  factExtraction: new RAGSchemaValidator({
    ...
    customValidators: {
      hasValidFacts: (response: any) =>
        ...
        response.facts.every((fact: any) => fact.statement && fact.confidence !== undefined),
    },
  }),
};
