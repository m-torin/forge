...
${result.content}`;
        ...
        const averageRelevance =
          contextResults.reduce((sum, r) => sum + r.score, 0) / contextResults.length;
        const responseTime = Date.now() - startTime;
...
export function createKnowledgeStatssTool(config: RAGToolConfig) {
  return tool({
    ...
    inputSchema: z.object({
      ...
      includeHealth: z.boolean().optional().default(true).describe('Include health status'),
    }),
 ...
 */
export function createAddResourceTool(config: RAGToolConfig) {
  return tool({
    description:
  ...
  return tool({
    ...
    execute: async ({ query, steps }) => {
      ...
      for (let i = 0; i < steps; i++) {
        const stepResults = await config.vectorStore.queryDocuments(currentQuery, {
