...
): (prompt: ModelMessage[], isReasoningEnabled?: boolean) => TestStreamPart[] {
  const { responses, defaultResponse, enableReasoning = false } = config;

  return (prompt: ModelMessage[], isReasoningEnabled = enableReasoning) => {
    const recentMessage = prompt[prompt.length - 1];
