...
export class EnhancedTool<TParams extends z.ZodTypeAny = z.ZodTypeAny, TResult = any> {
  ...
  private async performSecurityChecks(
    ...
    if (security.requiredPermissions?.length) {
      ...
      if (!hasPermission) {
        throw new ApplicationAIError('forbidden:tool', 'Insufficient permissions', {
          required: security.requiredPermissions,
        });
      }
    }
  }
...
export class ToolExecutionFramework {
  ...
  register<TParams extends z.ZodTypeAny, TResult>(
    definition: EnhancedToolDefinition<TParams, TResult>,
  ): void {
    const tool = new EnhancedTool(definition);
  ...
  getByTags(tags: string[]): EnhancedTool[] {
...
export function createEnhancedTool<TParams extends z.ZodTypeAny, TResult>(
  definition: EnhancedToolDefinition<TParams, TResult>,
): EnhancedTool<TParams, TResult> {
  return new EnhancedTool(definition);
}
