...
export class ToolRegistry {
  ...
  async loadTool(toolId: string, category?: string): Promise<Tool> {
    ...
    const loader = category ? this.loaders.get(category) : this.loaders.values().next().value;
    if (!loader) {
   ...
   */
  async executeTool(toolId: string, args: any, context?: Record<string, any>): Promise<any> {
   ...
   */
  getMetrics(toolId?: string): ToolPerformanceMetrics | Map<string, ToolPerformanceMetrics> | null {
    if (!this.performanceMetrics) {
      return null;
    }
  ...
  generateUsageReport(): {
    ...
    const underperformingTools = sortedMetrics
      .slice(-5)
      .filter(([, metrics]) => metrics.successRate < 0.5 && metrics.totalCalls > 5)
      .map(([toolId]) => toolId);
