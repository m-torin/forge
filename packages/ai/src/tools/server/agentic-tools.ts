...
// TODO: Implement step conditions when agents module is created
// import { stepCountIs } from '../agents/step-conditions';
const stepCountIs = (count: number): ((options: { steps: any[] }) => boolean) => {
...
export interface AgenticToolConfig<TParams = any, TResult = any> {
  ...
  toModelOutput?: (result: TResult) => {
    type: 'content';
    value: Array<
      { type: 'text'; text: string } | { type: 'media'; mediaType: string; data: string } | any
    >;
  };
}
    ...
    execute: async (
      ...
      try {
        ...
        if (config.trackHistory) {
          stepHistory.addStep({
            text: '',
            toolCalls: [{ toolName: config.description, input: args }],
            toolResults: [{ toolName: config.description, output: result }],
        ...
        if (config.repairConfig) {
          ...
          if (repairResult.success) {
            ...
            if (config.trackHistory) {
              stepHistory.addStep({
                text: '',
                toolCalls: [{ toolName: config.description, input: repairResult.repairedInput }],
                toolResults: [{ toolName: config.description, output: repairResult.result }],
                finishReason: 'tool-calls',
                usage: {},
              });
            }
  ...
  return {
    ...
    prepareStep: async (context: any) => {
        ...
        if (typeof config.activeToolsStrategy === 'function') {
          ...
          switch (config.activeToolsStrategy) {
            ...
            case 'conditional':
              ...
              if (lastStep?.toolResults?.some((r: any) => r.output?.needsMoreTools)) {
                activeTools = toolNames;
              } else {
      ...
      const userPrep = config.hooks?.prepareStep
        ...
        : config.hooks?.experimental_prepareStep
    ...
    return createAgentWorkflow({
      ...
      hooks: {
        prepareStep: async ({ stepNumber, steps }) => {
          ...
          const evalResult = steps[0]?.toolResults?.[0]?.output;
      ...
      hooks: {
        prepareStep: async ({ stepNumber, steps }) => {
          ...
          if (isEvenStep) {
            ...
            const lastValidation = steps[steps.length - 1]?.toolResults?.[0]?.output;
