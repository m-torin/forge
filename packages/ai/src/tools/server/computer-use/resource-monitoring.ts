...
export class ComputerToolMonitor {
   ...
   */
  preActionCheck(input: ComputerToolInput): {
    allowed: boolean;
    reason?: string;
  } {
    ...
    if (
      ...
      !this.config.allowedActions.includes(input.action as any)
    ) {
    ...
    if (recentActions.length >= (this.config.maxActionsPerMinute || 60)) {
      ...
      return {
        allowed: false,
        reason: "Rate limit exceeded for actions per minute",
      };
    }

    // Check screenshot rate limits
    if (input.action === "screenshot") {
      const recentScreenshots = recentActions.filter(
        (action) => action.action === "screenshot",
      );

      if (
        recentScreenshots.length >= (this.config.maxScreenshotsPerMinute || 30)
      ) {
        this.metrics.security.unauthorizedAccessAttempts++;
        return {
          allowed: false,
          reason: "Rate limit exceeded for screenshots per minute",
        };
      }
    }
  ...
  private updateResourceMetrics(
    ...
    this.metrics.cpu.averageUsagePercent =
      (this.metrics.cpu.averageUsagePercent + currentCPU) / 2;
    this.metrics.cpu.peakUsagePercent = Math.max(
      this.metrics.cpu.peakUsagePercent,
      currentCPU,
    );

    // Memory metrics
    this.metrics.memory.peakMemoryMB = Math.max(
      this.metrics.memory.peakMemoryMB,
      currentMemory,
    );
    this.metrics.memory.averageMemoryMB =
      (this.metrics.memory.averageMemoryMB + currentMemory) / 2;

    // I/O metrics
    if (input.action === "screenshot") {
      this.metrics.io.screenshotCount++;
      this.metrics.io.avgScreenshotTime =
        (this.metrics.io.avgScreenshotTime + executionTime) / 2;

      if (result.screenshot) {
        const screenshotSizeMB =
          this.estimateBase64Size(result.screenshot) / (1024 * 1024);
      ...
      this.metrics.io.avgInputActionTime =
        (this.metrics.io.avgInputActionTime + executionTime) / 2;
    }
  ...
  private updateSecurityMetrics(
    ...
    this.metrics.security.riskScore = Math.max(
      this.metrics.security.riskScore,
      riskScore,
    );
  ...
  private detectSuspiciousPatterns(): void {
    ...
    const screenshotCount = recentActions.filter(
    ...
    if (highRiskActions > 3) {
      this.metrics.security.suspiciousActionPatterns.push(
        "high_risk_clustering",
      );
    }
  ...
  generateSecurityReport(): {
    ...
    if (this.metrics.memory.memoryLeakDetected) {
      ...
      recommendations.push(
        "Investigate memory management in tool implementations",
      );
    }

    if (
      this.metrics.errors.totalErrors >
      this.metrics.cpu.actionsPerformed * 0.1
    ) {
      securityIssues.push({
        severity: "low",
...
export class ComputerToolSecurityTester {
  ...
  async runSecurityTests(computerTool: any): Promise<{
    ...
    try {
      ...
      const actionValidationTest =
        await this.testActionValidation(computerTool);
  ...
  private async testBlockedRegions(
    ...
    try {
      ...
      const passed =
        !result.success ||
        this.monitor.getCurrentMetrics().security.sensitiveAreaAccesses > 0;
  ...
  private async testMemoryLeakDetection(
    ...
    try {
      ...
      const memoryIncrease =
        metrics.memory.peakMemoryMB - metrics.memory.baselineMemoryMB;
...
export const computerToolSecurityUtils = {
   ...
   */
  validateSecurityConfig(config: ComputerToolSecurityConfig): {
    valid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
