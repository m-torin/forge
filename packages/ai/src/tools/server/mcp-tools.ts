/**
 * Enhanced MCP Integration as Tools - Direct AI SDK v5 Pattern
 *
 ...
 * Uses direct AI SDK tool() function for optimal compatibility.
 */
      ...
      for (const toolDef of tools) {
        ...
        const toolInstance = aiTool({
          ...
          execute: async (input: any, _options: any) => {
            try {
              const result = await mcpClient.executeServerTool(
                serverName,
                toolDef.name,
                input,
              );
 ...
 */
export async function createServerToolset(
  serverName: string,
): Promise<Record<string, Tool>> {
