...
import type { VectorDB, VectorQueryResult } from '../../types/vector';
    ...
    findSimilarContent: tool({
      ...
      execute: async ({
        ...
        const filteredResults =
          excludeOriginal && isId
            ? results.filter((r: VectorQueryResult) => r.id !== input)
            : results;

        return {
          input,
          results: filteredResults.slice(0, topK).map((result: VectorQueryResult) => ({
            id: result.id,
            content: result.metadata?.content || '',
 ...
 */
function createVectorContextMiddleware(config: VectorToolsConfig) {
  return {
    async enrichContext(
      messages: any[],
      ...
      const relevantContext = contextResults
        .filter((r: VectorQueryResult) => r.score >= (config.similarityThreshold || 0.7))
 ...
 */
interface VectorizedMessage<T = any> {
...
type VectorEnrichedStream<T = any> = {
...
class EmbeddingWorkflow<T = any> {
  ...
  async findSemanticallyRelated(
    ...
    return results
      .filter((r: VectorQueryResult) => r.score >= threshold)
      .map((r: VectorQueryResult) => ({
        id: r.id,
        content: r.metadata?.content || '',
        score: r.score,
