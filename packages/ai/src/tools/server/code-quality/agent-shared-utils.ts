...
interface AgentTools {
  ...
  extractObservation: (entity: any, key: string) => string | null;
 ...
 */
async function createAgentLogger(name: string, level: string = 'info'): Promise<AgentLogger> {
  const prefix = `[${name}]`;
 ...
 */
async function createSmartCache(
  name: string,
  maxSize: number = 100,
  ttl: number = 3600000,
): Promise<AgentCache> {
 ...
 */
async function safeStringify(obj: any, maxLength: number = 75000): Promise<string> {
  try {
...
/**
 * Extract observation from entity
 */
export function extractObservation(entity: any, key: string): string | null {
 ...
 */
async function createEntityName(entityType: string, identifier: string): Promise<string> {
  try {
 ...
 */
async function createStandardAgent(config: StandardAgentConfig): Promise<AgentTools> {
  const {
    name,
    cacheSize = 100,
 ...
 */
function createStandardErrorHandler(logger: AgentLogger) {
 ...
 */
function parseAgentRequest(userMessage: string): any {
  try {
 ...
 */
function createErrorResponse(error: string | Error, details?: any) {
  return {
    status: 'error',
