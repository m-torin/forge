...
import { BoundedCache } from '@repo/mcp-server';
...
export function createCodeQualityWorkflow(config: CodeQualityWorkflowConfig = {}) {
  ...
  const prepareStepHook = async ({ stepNumber, steps }: any) => {
  ...
  const onStepFinish = async ({ toolCalls, toolResults, stepNumber, finishReason }: any) => {
    ...
    if (toolCalls && toolCalls.length > 0) {
      ...
      if (errors.length > 0) {
        logError(
          `Step ${stepNumber} errors: ${errors
            .map((e: any) => JSON.stringify(e.output ?? e.errorText ?? e.error ?? e, null, 2))
            .join(', ')}`,
        );
      }
    }
  ...
  const experimental_repairToolCall = async ({
    ...
    switch (toolCall.toolName) {
      case 'createWorktree':
        ...
        if (attempt < maxRetries && toolCall.input) {
      ...
      case 'discoverFiles':
        ...
        if (attempt < maxRetries) {
          const currentInput = toolCall.input || {};
          const moreExcludes = [
            ...(currentInput.excludePatterns || []),
            '**/*.min.js',
            '**/vendor/**',
          ];
          return { ...toolCall, input: { ...currentInput, excludePatterns: moreExcludes } };
        }
        break;
      ...
      case 'analyzeCode':
        ...
        if (attempt < maxRetries) {
          const currentInput = toolCall.input || { options: {} };
          ...
          return { ...toolCall, input: { ...currentInput, options: reducedOptions } };
        }
        break;
