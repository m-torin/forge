...
// Handle Babel traverse default export properly
const typedTraverse = typeof traverse === 'function' ? traverse : (traverse as any).default;
...
class DocumentationGenerator {
  ...
  async generate(options: z.infer<typeof inputSchema>): Promise<DocumentationResult> {
    ...
    if (result.api) {
      ...
      const apiPath = `${options.outputDir}/api.${this.getFileExtension(options.format)}`;
    ...
    if (result.components) {
      ...
      const componentPath = `${options.outputDir}/components.${this.getFileExtension(options.format)}`;
    ...
    if (result.architecture) {
      ...
      const archPath = `${options.outputDir}/architecture.${this.getFileExtension(options.format)}`;
    ...
    if (result.changelog) {
      ...
      const changelogPath = `${options.outputDir}/CHANGELOG.md`;
          ...
          if (path.node.declaration?.type === 'FunctionDeclaration') {
            ...
            if (funcName && ['GET', 'POST', 'PUT', 'DELETE', 'PATCH'].includes(funcName)) {
              ...
              if (comment) {
                ...
                endpoint.inputSchema = comment.parameters;
              }
            }
          }
        },
  ...
  private detectAuthMethod(endpoints: APIEndpoint[]): string {
    ...
    const hasAuth = endpoints.some(e =>
      e.inputSchema.some((p: any) => authHeaders.includes(p.name.toLowerCase())),
    );
  ...
  private async findComponentExamples(
    ...
    try {
      ...
      for (const file of testFiles.slice(0, 3)) {
        ...
        const usageMatch = content.match(new RegExp(`<${componentName}[^>]*>`, 'g'));
        if (usageMatch) {
