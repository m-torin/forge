  ...
  return {
    createNamespace: tool({
      ...
      execute: async ({
        ...
        const existingNamespaces = (await (vectorDB as any).listNamespaces?.()) || [];
        ...
        await vectorDB.upsert([
          {
            id: tempId,
            // Minimal record to create namespace
            metadata: {
              __temp: true,
    ...
    listNamespaces: tool({
      ...
      execute: async ({ includeStats }: { includeStats?: boolean }) => {
        try {
          const namespaces = (await (vectorDB as any).listNamespaces?.()) || [];
          ...
          const namespacesWithStats = await Promise.all(
            namespaces.map(async (ns: any) => {
              try {
                const info = await (vectorDB as any).describe?.();
                return {
                  name: ns,
    ...
    deleteNamespace: tool({
      ...
      execute: async ({
        ...
        await (vectorDB as any).deleteNamespace?.(namespace);
    ...
    migrateNamespace: tool({
      ...
      execute: async ({
        ...
        if (deleteSource && totalMigrated > 0) {
          await (vectorDB as any).deleteNamespace?.(sourceNamespace);
        }
