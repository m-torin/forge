...
export function selectModel(config: ModelSelectionConfig): string | null {
  ...
  const availableModels = candidateModels.filter(modelId => {
    ...
    if (!canUserAccessModel(config.userTier, costTier, modelSupportsReasoning(modelId))) {
      return false;
    }
    ...
    if (config.preferProvider && modelConfig.provider !== config.preferProvider) {
      return false;
    }
...
export function validateModelForUseCase(
  ...
  // Check user access
  const costTier = MODEL_COST_TIERS[modelId] || 'medium';
  if (!canUserAccessModel(userTier, costTier, modelSupportsReasoning(modelId))) {
...
export function trackModelUsage(modelId: string, inputTokens: number, outputTokens: number): void {
  ...
  current.estimatedCost += (inputTokens + outputTokens) * costMultiplier * 0.00001;
