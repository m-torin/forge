...
> = {
  ...
  "creative-writing": {
    ...
    patterns: [
      /write\s+(a|an)\s+(story|poem|essay)/,
      /create\s+(a|an)\s+(character|plot)/,
    ],
  ...
  multimodal: {
    ...
    patterns: [
      /describe\s+(this|the)\s+(image|picture|photo)/,
...
export function classifyTask(prompt: string): TaskClassification {
  ...
  for (const [taskType, { keywords, patterns }] of Object.entries(
    TASK_PATTERNS,
  )) {
...
export function getOptimalModelForTask(
  ...
  const suitableModels = Object.entries(MODEL_OPTIMIZATION)
    .filter(
      ([modelId]) =>
        availableModelIds.length === 0 || availableModelIds.includes(modelId),
    )
 ...
 */
export function getTaskTypeInfo(taskType: TaskType): {
  emoji: string;
  description: string;
} {
  return {
 ...
 */
export function getModelOptimizationInfo(
  modelId: string,
): ModelOptimizationInfo | undefined {
