...
export function getBestModelForTask(
  task: "reasoning" | "vision" | "code" | "chat",
): string {
 ...
 */
export const REASONING_MODELS = getModelsByCapability("reasoning").map(
  (m) => m.id,
);
...
export const COMPUTER_USE_MODELS = getModelsByCapability("computer-use").map(
  (m) => m.id,
);
