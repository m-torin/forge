...
describe('streamBuffer', () => {
  ...
  test('should flush by size limit', async () => {
    const writes: any[] = [];
    const mockWriter = { write: vi.fn((data: any) => writes.push(data)) } as any;
    const onFlush = vi.fn();
...
describe('streamAggregator', () => {
  test('should aggregate multiple streams', async () => {
    ...
    expect(results.some(item => item.streamId === 'stream1' && item.value === 'a1')).toBeTruthy();
    expect(results.some(item => item.streamId === 'stream2' && item.value === 'b1')).toBeTruthy();
  });
...
describe('writeToStream', () => {
  ...
  test('should handle write errors gracefully', async () => {
    ...
    await expect(writeToStream(mockWriter, testData)).rejects.toThrow('Write error');
  });
