...
describe('tool Execution with AI SDK V5', () => {
  ...
  describe('tool Definition and Execution', () => {
    ...
    test('should handle tool execution errors', async () => {
      ...
      await expect(failingTool.execute({ input: 'test' })).rejects.toThrow('Tool execution failed');
    });
  });
        ...
        doStream: async () => ({
          stream: simulateReadableStream({
            chunks: [
              ...
              { type: 'text-delta', id: 'text-1', text: 'I will search for that. ' },
              {
                type: 'tool-call',
                toolCallId: 'search-1',
                toolName: 'searchTool',
              ...
              { type: 'text-delta', id: 'text-1', text: 'Here are the results: ' },
              {
                type: 'tool-result',
                toolCallId: 'search-1',
      ...
      const model = new MockLanguageModelV2({
        doStream: async () => ({
          stream: simulateReadableStream({
            chunks: [
              { type: 'text-start', id: 'text-1' },
              { type: 'text-delta', id: 'text-1', text: 'Let me calculate that... ' },
              {
                type: 'tool-call',
                toolCallId: 'calc-1',
              ...
              { type: 'text-delta', id: 'text-1', text: 'Now formatting as currency... ' },
              {
                type: 'tool-call',
                toolCallId: 'format-1',
              ...
              { type: 'text-delta', id: 'text-1', text: 'The result is $15.00' },
              { type: 'text-end', id: 'text-1' },
              {
                type: 'finish',
                finishReason: 'stop',
      ...
      const model = new MockLanguageModelV2({
        doStream: async () => ({
          stream: simulateReadableStream({
            chunks: [
              { type: 'text-start', id: 'text-1' },
              { type: 'text-delta', id: 'text-1', text: 'Streaming with telemetry' },
              { type: 'text-end', id: 'text-1' },
              {
                type: 'finish',
                finishReason: 'stop',
