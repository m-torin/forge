...
describe("tool Factory", () => {
  test("should import tool factory successfully", async () => {
    const toolFactory = await import(
      "../../../src/server/tools/factory-simple"
    );
  ...
  test(
    ...
    async () => {
      ...
      const mockExecute = vi
        .fn()
        .mockResolvedValue(
          IS_INTEGRATION_TEST ? "integration test result" : "test result",
        );
  ...
  test(
    ...
    async () => {
      const { createSimpleTool } = await import(
        "../../../src/server/tools/factory-simple"
      );
  ...
  test(
    ...
    async () => {
      ...
      const mockExecute = vi.fn().mockImplementation(async () => {
        throw new Error(
          IS_INTEGRATION_TEST ? "Integration test error" : "Mock test error",
        );
      });
