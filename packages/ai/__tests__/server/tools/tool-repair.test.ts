...
describe('tool Repair', () => {
  describe('basic Tool Repair Functionality', () => {
    test('should repair malformed tool call JSON', async () => {
      ...
      const repairFunction = vi.fn().mockImplementation(async ({ toolCall, error }) => {
    ...
    test('should repair invalid schema validation', async () => {
      ...
      const repairFunction = vi.fn().mockImplementation(async ({ toolCall, error }) => {
        ...
        return {
          ...
          repairReason: 'Fixed validation errors: empty name, negative age, invalid email',
        };
      });
    ...
    test('should handle unrepairable tool calls', async () => {
      ...
      const model = createToolCallingModel(['Attempting unknown tool...'], [unreparableToolCall]);
  ...
  describe('streaming Tool Repair', () => {
    ...
    test('should handle multiple tool repairs in streaming', async () => {
      ...
      const repairFunction = vi.fn().mockImplementation(async ({ toolCall, error }) => {
        if (toolCall.toolName === 'validator' && toolCall.input.value === '') {
          return {
            repaired: true,
            repairedToolCall: {
              ...toolCall,
        ...
        if (toolCall.toolName === 'formatter' && toolCall.input.style === 'invalid_style') {
          return {
            repaired: true,
            repairedToolCall: {
              ...toolCall,
      ...
      const result = streamText({
        ...
        tools: {
          ...
          formatter: {
            ...
            inputSchema: z.object({
              text: z.string(),
              style: z.enum(['uppercase', 'lowercase', 'capitalize', 'reverse']),
            }),
      ...
      const validatorCall = toolCallChunks.find(chunk => chunk.toolName === 'validator');
      const formatterCall = toolCallChunks.find(chunk => chunk.toolName === 'formatter');
  ...
  describe('advanced Tool Repair Patterns', () => {
    test('should repair with context-aware fixes', async () => {
      ...
      const repairFunction = vi.fn().mockImplementation(async ({ toolCall, error, context }) => {
        const repairedInput = { ...toolCall.input };
        ...
        // Fix invalid date
        if (repairedInput.filters?.date && !isValidDate(repairedInput.filters.date)) {
      ...
      expect(result.toolCalls[0].input.query).toBe('ai sdk documentation guide');
    ...
    test('should handle repair chains and recursive fixes', async () => {
      ...
      const repairFunction = vi
        .fn()
        .mockImplementation(async ({ toolCall, error, previousRepairAttempts }) => {
          repairAttempts++;
  ...
  describe('tool Repair Error Scenarios', () => {
    test('should handle repair function failures', async () => {
      const failingRepairFunction = vi.fn().mockImplementation(async ({ toolCall, error }) => {
    ...
    test('should handle repair timeout scenarios', async () => {
      const slowRepairFunction = vi.fn().mockImplementation(async ({ toolCall, error }) => {
