...
import { collectFullStreamChunks, waitForStreamCompletion } from '../../test-utils/streams';
...
describe('execution Framework', () => {
  describe('step Count Stop Conditions', () => {
    ...
    test('should handle stepCountIs with multiple tool calls', async () => {
      ...
      const model = createMultiStepToolModel(['Multi-step', ' workflow'], toolCalls, 2);
  ...
  describe('tool Call Stop Conditions', () => {
    test('should stop when specific tool is called using hasToolCall pattern', async () => {
      ...
      const model = createToolCallingModel(['Calling target tool...'], [toolCall]);
      ...
      const targetCall = toolCallChunks.find(chunk => chunk.toolName === 'targetTool');
    ...
    test('should continue when non-target tools are called', async () => {
      const toolCalls = [
        ...
        { toolCallId: 'other-2', toolName: 'anotherTool', input: { data: 'test' } },
      ];

      const model = createMultiStepToolModel(['Calling other tools...'], toolCalls, 2);
      ...
      expect(toolCallChunks).toHaveLength(2);
      expect(toolCallChunks.some(chunk => chunk.toolName === 'otherTool')).toBeTruthy();
      expect(toolCallChunks.some(chunk => chunk.toolName === 'anotherTool')).toBeTruthy();
      expect(toolCallChunks.some(chunk => chunk.toolName === 'targetTool')).toBeFalsy();
    });
  });
  ...
  describe('multi-Step Traces and Tool Validation', () => {
    test('should validate tool call trace in multi-step execution', async () => {
      const toolSequence = [
        ...
        { toolCallId: 'trace-2', toolName: 'processTool', input: { data: 'test' } },
        { toolCallId: 'trace-3', toolName: 'finalizeTool', input: { complete: true } },
      ];
    ...
    test('should handle tool execution results in trace', async () => {
      ...
      const chunks = await collectFullStreamChunks(result);
      const toolCallChunks = chunks.filter(chunk => chunk.type === 'tool-call');
      const toolResultChunks = chunks.filter(chunk => chunk.type === 'tool-result');
    ...
    test('should validate tool input schema at runtime', async () => {
      ...
      const model = createToolCallingModel(['Validating input...'], [validToolCall]);
  ...
  describe('complex Execution Patterns', () => {
    test('should handle conditional tool calling based on previous results', async () => {
      ...
      const conditionalTools = [
        { toolCallId: 'check-1', toolName: 'checkCondition', input: { condition: 'ready' } },
        { toolCallId: 'action-1', toolName: 'executeAction', input: { action: 'proceed' } },
      ];
      ...
      const checkCall = toolCallChunks.find(chunk => chunk.toolName === 'checkCondition');
      const actionCall = toolCallChunks.find(chunk => chunk.toolName === 'executeAction');
    ...
    test('should handle error recovery in multi-step execution', async () => {
      const recoveryTools = [
        { toolCallId: 'attempt-1', toolName: 'riskyTool', input: { attempt: 1 } },
        { toolCallId: 'recovery-1', toolName: 'recoveryTool', input: { recover: true } },
      ];
      ...
      const riskyCall = toolCallChunks.find(chunk => chunk.toolName === 'riskyTool');
      const recoveryCall = toolCallChunks.find(chunk => chunk.toolName === 'recoveryTool');
  ...
  describe('stop Condition Edge Cases', () => {
    ...
    test('should handle very high maxSteps with limited tool calls', async () => {
      ...
      const model = createToolCallingModel(['Single call only'], [singleToolCall]);
