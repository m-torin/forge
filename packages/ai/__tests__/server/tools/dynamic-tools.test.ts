...
import { createMultiStepToolModel, createToolCallingModel } from '../../test-utils/models';
...
describe('dynamic Tools', () => {
    ...
    test('should handle dynamic tool generation based on context', async () => {
      const createContextualTool = (context: string) => {
        return dynamicTool({
          ...
          inputSchema: z.object({
            action: z.string().describe(`Action to perform in ${context} context`),
              ...
              switch (dataType) {
                case 'text':
                  return {
                    ...
                    sentiment: 'neutral',
                    metadata: options?.includeMetadata ? { encoding: 'utf-8' } : undefined,
                  };
                case 'image':
                  return {
                    type: 'image-analysis',
                    ...
                    format: 'jpeg',
                    metadata: options?.includeMetadata ? { colorSpace: 'RGB' } : undefined,
                  };
                case 'audio':
                  return {
                    type: 'audio-analysis',
                    duration: '3:45',
                    format: 'mp3',
                    metadata: options?.includeMetadata ? { bitrate: '320kbps' } : undefined,
                  };
                case 'video':
                  return {
                    type: 'video-analysis',
                    duration: '10:30',
                    resolution: '4K',
                    metadata: options?.includeMetadata ? { codec: 'H.264' } : undefined,
                  };
              }
            },
          }),
    ...
    test('should handle conditional tool behavior with type guards', async () => {
      const conditionalTool = dynamicTool({
        ...
        execute: async ({ operation, target, data, permissions }) => {
          ...
          if (operation === 'delete' && (!permissions || !permissions.includes('admin'))) {
            throw new Error('Admin permissions required for delete operations');
          }

          switch (operation) {
            case 'read':
              return { operation: 'read', target, result: `Read data from ${target}` };
            case 'write':
              return {
                operation: 'write',
                target,
  ...
  describe('dynamic Tools in Streaming Context', () => {
    test('should work with dynamic tools in streaming workflows', async () => {
      ...
      const model = createMultiStepToolModel(['Processing stream...'], toolCalls, 1);
    ...
    test('should handle dynamic tool errors in streaming context', async () => {
      ...
      const model = createToolCallingModel(['Attempting processing...'], [errorToolCall]);
  ...
  describe('advanced Dynamic Tool Patterns', () => {
    test('should support dynamic tool chaining and composition', async () => {
      ...
      const pipeline = createProcessingPipeline(['validate', 'transform', 'analyze']);
      ...
      expect(stage2Result.input).toContain('Processed raw data through validate');
    });
    ...
    test('should handle dynamic tool metadata and versioning', async () => {
      const createVersionedTool = (version: string, features: string[]) => {
        return dynamicTool({
          ...
          execute: async ({ command, version: requestedVersion, enabledFeatures, config }) => {
            const supportedFeatures = features.filter(f => enabledFeatures?.includes(f));
            ...
            return {
              ...
              config: config || {},
              executed: true,
              compatibility: requestedVersion === version ? 'exact' : 'compatible',
            };
          },
        });
      };
      ...
      const v2Tool = createVersionedTool('2.0.0', ['basic', 'validation', 'advanced', 'streaming']);
      ...
      expect(v2Result.supportedFeatures).toEqual(['basic', 'advanced', 'streaming']);
