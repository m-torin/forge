...
describe('telemetry', () => {
  describe('basic Telemetry Configuration', () => {
    ...
    test('should validate telemetry metadata structure', () => {
      ...
      assertTelemetryMetadata(config, ['service', 'version', 'environment', 'requestId', 'userId']);
    });
  });
  ...
  describe('generateText Telemetry', () => {
    test('should capture telemetry for generateText operations', async () => {
      ...
      telemetryCollector.recordSpan(telemetryFunctionTests.generateText.spanName, {
      ...
      const span = telemetryCollector.findSpanByName(telemetryFunctionTests.generateText.spanName);
      expect(span?.attributes['ai.function.id']).toBe(telemetryConfig.functionId);
    });
    ...
    test('should handle telemetry with tool calls in generateText', async () => {
      ...
      const toolSpan = telemetryCollector.findSpanByName('ai.generateText.doGenerate.tool');
  ...
  describe('streamText Telemetry', () => {
    test('should capture telemetry for streamText operations', async () => {
      const model = createStreamingTextModel(['Streaming', ' with', ' telemetry']);
      const telemetryConfig = createTelemetryConfig({
      ...
      telemetryCollector.recordSpan(telemetryFunctionTests.streamText.spanName, {
        'ai.function.id': telemetryConfig.functionId,
        'ai.model.id': model.modelId,
    ...
    test('should track streaming performance metrics', async () => {
      ...
      telemetryCollector.recordSpan('ai.streamText.performance', {
        ...
        'ai.performance.tokens_per_second': completion.usage.totalTokens / (duration / 1000),
        'ai.performance.chunks_per_second': 2 / (duration / 1000),
      });

      const perfSpan = telemetryCollector.findSpanByName('ai.streamText.performance');
      expect(perfSpan?.attributes['ai.performance.duration_ms']).toBeLessThan(1000);
    });
  });
  ...
  describe('object Generation Telemetry', () => {
    test('should capture telemetry for generateObject operations', async () => {
      ...
      telemetryCollector.recordSpan(telemetryFunctionTests.generateObject.spanName, {
        'ai.function.id': telemetryConfig.functionId,
        'ai.schema.type': 'zod',
  ...
  describe('embedding Telemetry', () => {
    test('should capture telemetry for embed operations', async () => {
      ...
      const embedSpan = telemetryCollector.findSpanByName(telemetryFunctionTests.embed.spanName);
    ...
    test('should capture telemetry for embedMany operations', async () => {
      ...
      const values = ['First text to embed', 'Second text to embed', 'Third text to embed'];
  ...
  describe('reasoning Telemetry', () => {
    test('should capture reasoning tokens in telemetry', async () => {
      ...
      telemetryCollector.recordSpan('ai.generateText.reasoning', {
        ...
        'ai.reasoning.ratio': result.usage.reasoningTokens! / result.usage.totalTokens,
      });

      const reasoningSpan = telemetryCollector.findSpanByName('ai.generateText.reasoning');
      expect(reasoningSpan?.attributes['ai.reasoning.enabled']).toBeTruthy();
      expect(reasoningSpan?.attributes['ai.usage.reasoning_tokens']).toBeGreaterThan(0);
    });
  });
  ...
  describe('telemetry Scenarios', () => {
    ...
    test('should handle input-only recording scenario', async () => {
      ...
      const inputSpan = telemetryCollector.findSpanByName('ai.generateText.input');
    ...
    test('should handle custom metadata scenario', async () => {
      ...
      const customSpan = telemetryCollector.findSpanByName('ai.generateText.custom');
      expect(customSpan?.attributes['ai.metadata.testType']).toBe('integration');
    });
  ...
  describe('telemetry Error Handling', () => {
    test('should handle telemetry when model operations fail', async () => {
      ...
      const errorSpan = telemetryCollector.findSpanByName('ai.generateText.error');
  ...
  describe('telemetry Assertions and Validation', () => {
    test('should validate telemetry span structure', () => {
      const expectedSpans = [
        { name: 'ai.generateText.doGenerate', attributes: { 'ai.model.id': 'test-model' } },
        { name: 'ai.streamText.doStream', attributes: { 'ai.operation.name': 'streamText' } },
      ];

      // Simulate spans
      telemetryCollector.recordSpan('ai.generateText.doGenerate', { 'ai.model.id': 'test-model' });
