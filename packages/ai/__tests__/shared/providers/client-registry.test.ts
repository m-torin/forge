...
describe('client Provider Registry', () => {
  ...
  describe('helper Functions', () => {
    ...
    describe('listAvailableModels', () => {
      test('should list all models across providers', () => {
        ...
        expect(models.some(m => m.id === 'claude-3-5-sonnet-20241022')).toBeTruthy();
      ...
      test('should filter models by provider', () => {
        ...
        expect(openaiModels.some(m => m.id === 'claude-3-5-sonnet-20241022')).toBeFalsy();
      });
      ...
      test('should filter models by capability', () => {
        ...
        expect(multimodalModels.every(m => m.metadata?.multimodal)).toBeTruthy();
      });
      ...
      test('should filter models by cost range', () => {
        ...
        expect(cheapModels.every(m => (m.metadata?.inputCost || 0) <= 0.01)).toBeTruthy();
      });
    });
    ...
    describe('createProviderChain', () => {
      ...
      test('should fail if all providers in chain fail', async () => {
        const chain = createProviderChain([{ provider: 'openai', model: 'gpt-4' }]);

        vi.spyOn(chain.providers[0], 'execute')
          .mockImplementation()
          .mockRejectedValue(new Error('Provider failed'));

        await expect(chain.execute({ prompt: 'Test' })).rejects.toThrow('All providers failed');
      });
    });
    ...
    describe('getCostEstimate', () => {
      ...
      test('should support batch cost calculation', () => {
        const estimates = getCostEstimate([
          ...
          { model: 'claude-3-5-sonnet-20241022', inputTokens: 2000, outputTokens: 1000 },
        ]);
  ...
  describe('error Handling and Edge Cases', () => {
    test('should handle empty provider list gracefully', () => {
      ...
      // Clear all providers
      emptyRegistry.listProviders().forEach(p => emptyRegistry.removeProvider(p));
    ...
    test('should handle concurrent provider operations', async () => {
      ...
      expect(results.every(r => r.status === 'fulfilled' || r.status === 'rejected')).toBeTruthy();
    });
  });
});
