...
export class MemoryAwareCache<K extends {} = string, V = any> {
  ...
  private startPeriodicCleanup(): void {
    this.cleanupInterval = setInterval(async () => {
      try {
        ...
        console.error('[MemoryAwareCache] Periodic maintenance error:', error);
        // Continue periodic cleanup despite errors
      }
  ...
  private async compressValue(value: V): Promise<{ data: any; size: number } | null> {
    ...
    try {
      ...
      // Base64 encoding for storage (lightweight alternative to compression)
...
export const memoryAwareCacheTool = {
  ...
  async execute(args: MemoryAwareCacheArgs): Promise<MCPToolResponse> {
