...
import { runTool } from './tool-helpers';
...
export const contextSessionManagerTool = {
  ...
  async execute(args: ContextSessionManagerArgs): Promise<MCPToolResponse> {
    return runTool('context_session_manager', args.action, async () => {
...
async function detectPackageScope(
  ...
  // Real implementation - read package.json and analyze actual project structure
  const fs = require('fs');
  const path = require('path');

  let packageName = 'detected-package';
  let version = '1.0.0';
  let projectType = 'Node.js Project';
  ...
  try {
    ...
    if (fs.existsSync(packageJsonPath)) {
      ...
      else if (deps.react) projectType = 'React Application';
      else if (deps.express) projectType = 'Express Server';
  ...
  const result = {
    ...
    type: projectType,
    packagePath,
    isNearestPackage: packageJsonExists,
    packageStructure: {
      hasPackageJson: packageJsonExists,
      hasNodeModules: fs.existsSync(path.join(packagePath, 'node_modules')),
      hasSrcDirectory: fs.existsSync(path.join(packagePath, 'src')),
      hasConfigFiles:
        fs.existsSync(path.join(packagePath, 'tsconfig.json')) ||
        fs.existsSync(path.join(packagePath, '.eslintrc.js')) ||
        fs.existsSync(path.join(packagePath, '.eslintrc.json')),
    },
  };
