...
export interface SecurityScannerArgs extends AbortableToolArgs {
  ...
  | 'detectSecrets'
    ...
    | 'comprehensiveScan' // All security scans with aggregated results
    | 'quickScan' // Critical issues only (secrets + high-severity vulnerabilities)
    | 'vulnScan' // All vulnerability types except dependencies
    | 'criticalOnly'; // Critical severity only across all types
    ...
    return runTool('security_scanner', args.action, async () => {
      ...
      switch (action) {
        case 'detectSecrets': {
          ...
          // Validate inputs
          // Default to packagePath as allowed base to prevent path traversal outside repo
          const allowedBases =
            allowedBasePaths && allowedBasePaths.length > 0
              ? allowedBasePaths
              : packagePath
                ? [packagePath]
                : [process.cwd()];
        ...
        case 'comprehensiveScan': {
          ...
          const comprehensiveResult = await performComprehensiveScan(
            packagePath,
            sessionId,
            scanDepth,
            signal,
...
// REGEX_TIMEOUT_MS imported from @repo/core-utils
...
async function scanDependencyVulnerabilities(
  ...
  // Currently returns basic structure - extend with actual dependency scanning
  return {
    vulnerabilities: [],
    summary: {
      critical: 0,
...
async function performFullSecurityScan(
  ...
  safeThrowIfAborted(signal);
  // Implementation stub - full functionality would scan all files
  return {
    secrets: [],
    vulnerabilities: {
      injection: [],
...
async function performComprehensiveScan(
  ...
  depth: string = 'standard',
  signal?: AbortSignal,
): Promise<SecurityScanResult & { progress: number; duration: number }> {
  safeThrowIfAborted(signal);
...
async function performQuickScan(
  ...
  sessionId?: string,
  signal?: AbortSignal,
): Promise<SecurityScanResult & { scanType: string }> {
  safeThrowIfAborted(signal);

  // Quick scan focuses on secrets and critical vulnerabilities only
  const quickResult: SecurityScanResult = {
    secrets: [],
    vulnerabilities: {
      injection: [],
      xss: [],
...
async function performVulnScan(
  packagePath: string,
  sessionId?: string,
  depth: string = 'standard',
  signal?: AbortSignal,
  ...
  // Vulnerability scan excludes dependencies, focuses on code vulnerabilities
  const vulnResult: SecurityScanResult = {
    secrets: [],
    vulnerabilities: {
      injection: [],
      xss: [],
    ...
    dependencies: [], // Explicitly empty for vuln-only scan
    summary: {
      critical: 0,
      high: 0,
      medium: 0,
...
async function performCriticalOnlyScan(
  packagePath: string,
  sessionId?: string,
  signal?: AbortSignal,
): Promise<SecurityScanResult & { scanType: string }> {
  ...
  // Critical-only scan filters all results to critical severity
...
async function* streamProjectSecurityScan(
  ...
  // Discover actual files in the project
  const fs = require('fs');
  const path = require('path');

  const discoverProjectFiles = (dirPath: string): string[] => {
    const files: string[] = [];
    try {
      ...
      console.warn(`Failed to discover files in ${dirPath}:`, error);
    }
    return files;
  };
