...
class SecurityValidator {
  ...
  private static readonly THREAT_PATTERNS = [
    ...
    {
      pattern: /[;&|`${}()]/g,
      threat: 'Command Injection Characters',
      level: 'high' as const,
    },
    {
      pattern: /\.\.\/|\.\.\\/g,
...
export function validateFilePath(
  ...
  // Manual length check for custom max length (Zod v3 compatibility)
  ...
  const basicValidation = filePathSchema.safeParse(inputPath);
...
export function validateFilePathZod(
  ...
  if (inputPath.length > maxLength) {
    return {
      success: false,
      error: `Path exceeds maximum length of ${maxLength}`,
      issues: [`Path exceeds maximum length of ${maxLength}`],
    };
  }
