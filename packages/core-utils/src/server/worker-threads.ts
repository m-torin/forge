...
// Simple ID generator for worker pools
const idGenerators = {
  workerPoolId: () => `pool_${Date.now()}_${Math.random().toString(36).substring(7)}`,
};
...
class SharedArrayBufferSecurity {
  ...
  static createSecureBuffer(size: number): SharedArrayBuffer {
    ...
    const jitter = Math.floor(Date.now() % 64); // 0-63 byte jitter using deterministic time-based approach
      ...
      switch (action) {
        case 'createPool': {
          ...
          const workerScript = createWorkerScript({
            ...
            computeMatrix: (data: { size: number; operation: string }) => {
              ...
              // Fill with deterministic numbers for testing
              for (let i = 0; i < size; i++) {
                for (let j = 0; j < size; j++) {
                  matrix[i][j] = Math.floor(((i * size + j) * 37) % 100); // Deterministic pattern
                }
              }
              ...
              try {
                ...
                const safePoolDestroy = async () => {
                  try {
                    ...
                    console.warn(`Pool destroy error: ${error}`);
                  }
                };
                ...
                const safeFileUnlink = async () => {
                  try {
                    ...
                    console.warn(`Script file cleanup error: ${error}`);
                  }
                };
                ...
                const safeDirRemove = async () => {
                  try {
                    ...
                    console.warn(`Temp directory cleanup error: ${error}`);
                  }
                };
        ...
        case 'dataProcessingDemo': {
          ...
          const testArray = Array.from(
            { length: dataSize },
            (_, i) => Math.floor((i * 97 + 13) % 1000), // Deterministic sequence
          );
        ...
        case 'atomicsDemo': {
          ...
          for (let i = 0; i < demoMaxWorkers; i++) {
            operations.push(async () => {
              ...
              // Simulate work time with deterministic delay
