...
export interface VectorDatabaseAdapter extends DatabaseAdapter {
  ...
  // Metadata operations
  updateMetadata<T>(id: string, metadata: Record<string, any>, options?: { namespace?: string }): Promise<T>;
  ...
  queryByText<T>(text: string, options?: {
    topK?: number;
    filter?: string;
    includeMetadata?: boolean;
    includeVectors?: boolean;
  ...
  }[], options?: { namespace?: string }): Promise<T>;
  
  upsertDense<T>(vectors: {
    id: string;
    vector: number[];
    metadata?: Record<string, any>;
  }[], options?: { namespace?: string }): Promise<T>;
  
  upsertSparse<T>(vectors: {
    id: string;
    sparseVector: {
      indices: number[];
      values: number[];
...
export interface RedisDatabaseAdapter extends DatabaseAdapter {
  ...
  setWithExpiration<T>(collection: string, id: string, data: any, expirationSeconds: number): Promise<T>;
