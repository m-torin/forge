...
describe('Upstash Vector Four-File Pattern', () => {
  ...
  describe('Basic Vector Operations', () => {
    it('should upsert vectors', async () => {
      ...
      expect(mockVectorClient.upsert).toHaveBeenCalledWith(vectors, { namespace: 'test-namespace' });
    ...
    it('should fetch vectors by IDs', async () => {
      ...
      expect(result.vectors).toHaveProperty('vec1');
    });
  ...
  describe('Error Handling', () => {
    it('should handle missing reference vector in findSimilar', async () => {
      mockVectorClient.fetch = vi.fn(() => Promise.resolve({
        vectors: {}
      }));

      await expect(vectorOps.findSimilar('nonexistent')).rejects.toThrow('Reference vector nonexistent not found');
    });

    it('should handle empty query results', async () => {
      mockVectorClient.query = vi.fn(() => Promise.resolve({
        matches: null
      }));
