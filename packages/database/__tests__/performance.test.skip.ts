  ...
  describe('Concurrent Operations Performance', () => {
    it('should handle concurrent reads efficiently', async () => {
      ...
      for (let i = 0; i < concurrentReads; i++) {
        ...
        readPromises.push(
          redisHelper.getAdapter().findUnique('users', { id: randomUser.id })
        );
      }
    ...
    it('should handle high-frequency operations', async () => {
      ...
      for (let i = 0; i < operationCount; i++) {
        ...
        if (i % 3 === 0) {
          ...
          operations.push(
            redisHelper.getAdapter().update('stress', prevId, { updated: true })
          ...
          operations.push(
            redisHelper.getAdapter().findUnique('stress', { id: prevId })
          );
        }
      }
