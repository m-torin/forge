...
const mockRedisClient = {
  ...
  get: vi.fn((key: string) => {
    if (key === 'user:1') {
      return Promise.resolve(JSON.stringify({ id: '1', name: 'Test User', email: 'test@example.com' }));
    }
...
describe('Redis Four-File Pattern', () => {
  ...
  describe('List Operations', () => {
    it('should push to left of list', async () => {
      ...
      expect(mockRedisClient.lpush).toHaveBeenCalledWith('list:key', JSON.stringify({ item: 1 }), JSON.stringify({ item: 2 }));
      expect(result).toBe(1);
    });
