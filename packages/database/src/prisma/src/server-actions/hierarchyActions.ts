...
export async function checkCircularReferenceAction(
  ...
  try {
    ...
    logError(`Error checking circular reference for ${model}`, {
      error: error instanceof Error ? error : new Error(String(error)),
      operation: 'check_circular_reference',
      metadata: { model },
    });
    return {
      success: false,
...
export async function getDescendantsAction(
  ...
  try {
    ...
    logError(`Error getting descendants for ${model}`, {
      error: error instanceof Error ? error : new Error(String(error)),
...
export async function buildHierarchyTreeAction(
  ...
  try {
    ...
    logError(`Error building hierarchy tree for ${model}`, {
      error: error instanceof Error ? error : new Error(String(error)),
      operation: 'build_hierarchy_tree',
      metadata: { model },
    });
    return {
      success: false,
...
export async function getValidParentOptionsAction(
  ...
  try {
    ...
    logError(`Error getting valid parent options for ${model}`, {
      error: error instanceof Error ? error : new Error(String(error)),
      operation: 'get_valid_parent_options',
      metadata: { model },
    });
    return {
      success: false,
