...
const logger = {
  error: (message: string, ...args: any[]) =>
    console.error(`[database:hierarchy] ${message}`, ...args),
  warn: (message: string, ...args: any[]) =>
    console.warn(`[database:hierarchy] ${message}`, ...args),
  info: (message: string, ...args: any[]) =>
    console.info(`[database:hierarchy] ${message}`, ...args),
  log: (message: string, ...args: any[]) => console.log(`[database:hierarchy] ${message}`, ...args),
};
...
export async function checkCircularReferenceAction(
  ...
  try {
    ...
    logger.error(`Error checking circular reference for ${model}:`, error);
    return {
      success: false,
...
export async function getDescendantsAction(
  ...
  try {
    ...
    logger.error(`Error getting descendants for ${model}:`, error);
    return {
      success: false,
...
export async function buildHierarchyTreeAction(
  ...
  try {
    ...
    logger.error(`Error building hierarchy tree for ${model}:`, error);
    return {
      success: false,
...
export async function getValidParentOptionsAction(
  ...
  try {
    ...
    logger.error(`Error getting valid parent options for ${model}:`, error);
    return {
      success: false,
