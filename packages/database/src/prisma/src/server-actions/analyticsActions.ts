...
import { countReviewsOrm, groupByReviewsOrm } from '../orm/guestActions/guestActionsOrm';
import { countCartsOrm } from '../orm/orders/cartOrm';
import { countOrdersOrm } from '../orm/orders/ordersOrm';
import { countTransactionsOrm } from '../orm/orders/transactionOrm';
import {
  countRegistriesOrm,
...
export async function getCountByFieldAction(
  ...
  try {
    ...
    logError(`Error getting count by ${groupByField} for ${model}`, {
      error: error instanceof Error ? error : new Error(String(error)),
      operation: 'get_count_by_field',
      metadata: { model, groupByField },
    });
    return {
      success: false,
...
export async function getActivityStatsAction(
  ...
  try {
    ...
    logError(`Error getting activity stats for ${model}`, {
      error: error instanceof Error ? error : new Error(String(error)),
      operation: 'get_activity_stats',
      metadata: { model },
    });
    return {
      success: false,
...
export async function getAggregateStatsAction(
  ...
  try {
    ...
    logError(`Error getting aggregate stats for ${model}`, {
      error: error instanceof Error ? error : new Error(String(error)),
      operation: 'get_aggregate_stats',
      metadata: { model },
    });
    return {
      success: false,
...
export async function getConversionRateAction(
  ...
  try {
    ...
    logError(`Error calculating conversion rate for ${model}`, {
      error: error instanceof Error ? error : new Error(String(error)),
