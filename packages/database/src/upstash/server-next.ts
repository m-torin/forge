...
import type { QueryResult, FetchResult } from '@upstash/vector';

// Temporary console logging until orchestration package is stable
const logger = {
  error: (message: string, ...args: any[]) =>
    console.error(`[database:upstash] ${message}`, ...args),
  warn: (message: string, ...args: any[]) => console.warn(`[database:upstash] ${message}`, ...args),
  info: (message: string, ...args: any[]) => console.info(`[database:upstash] ${message}`, ...args),
  log: (message: string, ...args: any[]) => console.log(`[database:upstash] ${message}`, ...args),
};
...
> {
  ...
  if (options?.threshold !== undefined) {
    results = results.filter((match: QueryResult<Dict>) => match.score >= options.threshold!);
  }
