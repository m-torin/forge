...
const createMCPSecurityRules = (): RulesRecord => ({
    ...
    SEVERITY.ERROR,
    {
      paths: [
        ...
        },
        {
          name: "child_process",
          message:
            "Child process spawning is not permitted in MCP tools for security reasons.",
        },
        {
          name: "fs",
          message:
  ...
  "no-restricted-syntax": [
    ...
    {
      selector:
        'CallExpression[callee.object.name="Atomics"][callee.property.name="wait"]',
      message:
    ...
    {
      selector:
        'CallExpression[callee.object.name="vm"][callee.property.name="runInNewContext"]',
      message:
    ...
    {
      selector:
        'CallExpression[callee.object.name="vm"][callee.property.name="runInThisContext"]',
      message:
        ...
        "error",
        {
          paths: [
            ...
            // Still restrict fs - even hardened VM files should use centralized helpers
            {
              name: "fs",
              message:
                "Use centralized file helpers in utils/files.ts with path validation.",
            },
            {
              name: "node:fs",
              message:
