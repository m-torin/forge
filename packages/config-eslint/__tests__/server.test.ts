...
describe('Server ESLint Configuration', () => {
  ...
  it('includes server-specific configuration', () => {
    ...
    const serverSpecificConfig = serverConfig.find(
      (config) =>
        ...
        config.files.some((pattern: string) => pattern.includes('.ts')) &&
        config.plugins &&
        config.plugins.node,
    );
  ...
  it('includes stricter TypeScript rules for server code', () => {
    ...
    if (configWithStrictTSRules?.rules) {
      expect(configWithStrictTSRules.rules['@typescript-eslint/no-misused-promises']).toBe('error');
      expect(configWithStrictTSRules.rules['@typescript-eslint/no-floating-promises']).toBe(
        'error',
      );
      expect(configWithStrictTSRules.rules['@typescript-eslint/await-thenable']).toBe('error');
      expect(
        configWithStrictTSRules.rules['@typescript-eslint/explicit-function-return-type'],
      ).toBe('off');
    } else {
  ...
  it('includes Node.js specific rules', () => {
    ...
    if (configWithNodeRules?.rules) {
      ...
      expect(configWithNodeRules.rules['node/no-extraneous-import']).toBe('error');
      // Some rules are intentionally disabled
      expect(configWithNodeRules.rules['node/no-missing-import']).toBe('off');
      expect(configWithNodeRules.rules['node/no-unsupported-features/es-syntax']).toBe('off');
    } else {
  ...
  it('includes additional security rules for server code', () => {
    ...
    if (configWithSecurityRules?.rules) {
      expect(configWithSecurityRules.rules['security/detect-unsafe-regex']).toBe('error');
      expect(configWithSecurityRules.rules['security/detect-eval-with-expression']).toBe('error');
      expect(configWithSecurityRules.rules['security/detect-buffer-noassert']).toBe('error');
    } else {
  ...
  it('includes relaxed rules for test files', () => {
    ...
    const testFilesConfig = serverConfig.find(
      (config) =>
        ...
        config.files &&
        Array.isArray(config.files) &&
        config.files.some((pattern: string) => pattern.includes('test.')),
    );

    expect(testFilesConfig).toBeDefined();
    // Add checks for potentially undefined properties
    if (testFilesConfig?.rules) {
      expect(testFilesConfig.rules['@typescript-eslint/no-explicit-any']).toBe('off');
      expect(testFilesConfig.rules['@typescript-eslint/explicit-function-return-type']).toBe('off');
      expect(testFilesConfig.rules['import/no-extraneous-dependencies']).toBe('off');
    } else {
