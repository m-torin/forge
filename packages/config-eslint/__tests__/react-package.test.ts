...
describe('React Package ESLint Configuration', () => {
  ...
  it('includes JSX language options', () => {
    ...
    // Add checks for potentially undefined properties
    expect(configWithJSXOptions?.languageOptions?.parserOptions?.ecmaFeatures?.jsx).toBe(true);
  });
  ...
  it('includes React settings', () => {
    ...
    expect((configWithReactSettings?.settings?.react as any)?.version).toBe('detect'); // Use 'as any'
  ...
  it('includes React rules', () => {
    ...
    if (configWithReactRules?.rules) {
      ...
      expect(configWithReactRules.rules['react/jsx-fragments']).toEqual(['error', 'syntax']);
  ...
  it('includes React Hooks rules', () => {
    ...
    if (configWithHooksRules?.rules) {
      expect(configWithHooksRules.rules['react-hooks/rules-of-hooks']).toBe('error');
      expect(configWithHooksRules.rules['react-hooks/exhaustive-deps']).toBe('warn');
    } else {
  ...
  it('includes accessibility rules', () => {
    ...
    if (configWithA11yRules?.rules) {
      ...
      expect(configWithA11yRules.rules['jsx-a11y/img-redundant-alt']).toBe('error');
    } else {
