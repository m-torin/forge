...
describe.todo('next.js Sentry Instrumentation Utilities', () => {
  describe.todo('createInstrumentation', () => {
    ...
    test('should include verbose logging when enabled', () => {
      ...
      expect(result).toContain("console.log('[Observability] Initializing for Node.js runtime')");
      expect(result).toContain("console.log('[Observability] Initializing for Edge runtime')");
      expect(result).toContain(
        "console.log('[Observability] Skipping initialization in development')",
      );
      expect(result).toContain("console.error('[Observability] Error in development:', error)");
    });
  ...
  describe.todo('createClientInstrumentation', () => {
    ...
    test('should use environment variables for configuration', () => {
      ...
      expect(result).toContain('enableLogs: env.NEXT_PUBLIC_SENTRY_ENABLE_LOGS ?? true');
    });
    ...
    test('should include verbose logging when enabled', () => {
      ...
      expect(result).toContain("console.log('[Observability] Client-side Sentry initialized')");
    });
  });
  ...
  describe.todo('createServerInstrumentation', () => {
    test('should generate server-side instrumentation', () => {
      ...
      expect(result).toContain('Sentry.init({');
      expect(result).toContain('dsn: env.SENTRY_DSN || env.NEXT_PUBLIC_SENTRY_DSN');
      expect(result).toContain('sendDefaultPii: env.SENTRY_SEND_DEFAULT_PII ?? true');
    });

    test('should use environment variables for server configuration', () => {
      const result = createServerInstrumentation();

      expect(result).toContain('environment: env.SENTRY_ENVIRONMENT || process.env.NODE_ENV');
      expect(result).toContain('release: env.SENTRY_RELEASE');
      expect(result).toContain('tracesSampleRate: env.SENTRY_TRACES_SAMPLE_RATE || 1.0');
      expect(result).toContain('profilesSampleRate: env.SENTRY_PROFILES_SAMPLE_RATE || 0');
    ...
    test('should include verbose logging when enabled', () => {
      ...
      expect(result).toContain("console.log('[Observability] Server-side Sentry initialized')");
    });
  });
  ...
  describe.todo('createEdgeInstrumentation', () => {
    test('should generate edge runtime instrumentation', () => {
      ...
      expect(result).toContain('// Note: Some features may be limited in edge runtime');
    });
    ...
    test('should include verbose logging when enabled', () => {
      ...
      expect(result).toContain("console.log('[Observability] Edge runtime Sentry initialized')");
    });
  });
