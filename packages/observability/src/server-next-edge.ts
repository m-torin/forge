...
export async function onRequestError(error: Error, request?: Request) {
  // Edge runtime: throw the error instead of logging
  throw new Error(`[Observability Edge] Request error: ${error} - Request: ${request}`);
}

export { edgeObservabilityManager as observabilityManager };

// Export edge-compatible logger functions
export {
  configureLogger,
  createLogger,
  logDebug,
