...
export class SentryEdgeProvider implements ObservabilityProvider {
  ...
  private parseDSN(dsn: string): void {
    try {
      ...
      console.error('[Sentry Edge] Invalid DSN: ', error);
    }
  }
  ...
  private async sendEvent(event: SentryEvent): Promise<void> {
    if (!this.endpoint || !this.publicKey) {
      console.error('[Sentry Edge] Cannot send event: missing endpoint or key');
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        console.error(
          '[Sentry Edge] Failed to send event:',
          response.status,
          await response.text(),
        );
      }
    } catch (error: any) {
      console.error('[Sentry Edge] Failed to send event: ', error);
    }
  }
