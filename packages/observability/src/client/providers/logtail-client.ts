/**
 * Logtail/BetterStack provider for client-side production logging
 * Browser-compatible implementation using @logtail/js for non-Next.js environments
 */
...
export class LogtailClientProvider implements ObservabilityProvider {
  ...
  async initialize(config: ObservabilityProviderConfig): Promise<void> {
    ...
    if (!this.config.sourceToken) {
      if (this.isDevelopment) {
        console.warn('[Logtail] No source token provided, using console fallback');
        this.isInitialized = true;
        return;
      }
    ...
    try {
      // Dynamically import Logtail Browser to avoid bundling if not used
      const { Browser: Logtail } = await import('@logtail/js');
