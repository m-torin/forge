...
describe.todo('web-vitals', () => {
  ...
  describe('reportWebVitals', () => {
    test('should report web vital with transaction', () => {
      ...
      expect(mockSpan.setData).toHaveBeenCalledWith('lcp', { value: 2500, unit: 'millisecond' });
      expect(mockSpan.setAttribute).toHaveBeenCalledWith('webvital.lcp.rating', 'good');
      expect(mockSpan.setData).toHaveBeenCalledWith('webvital.lcp.attribution', {
        element: 'img',
        url: '/hero.jpg',
      });
    ...
    test('should handle CLS metric with none unit', () => {
      ...
      expect(mockSpan.setData).toHaveBeenCalledWith('cls', { value: 0.15, unit: 'none' });
    ...
    test('should compute rating when not provided', () => {
      ...
      expect(mockSpan.setAttribute).toHaveBeenCalledWith('webvital.fcp.rating', 'good');
      expect(Sentry.addBreadcrumb).toHaveBeenCalledWith({
        category: 'web-vital',
  ...
  describe('createWebVitalsHandler', () => {
    ...
    test('should filter poor metrics when onlyReportPoor is true', () => {
      ...
      handler(poorMetric);
      expect(mockSpan.setData).toHaveBeenCalledWith('lcp', { value: 5000, unit: 'millisecond' });
    });
    ...
    test('should send as transactions when enabled', () => {
      ...
      expect(mockSpan.setData).toHaveBeenCalledWith('cls', { value: 0.2, unit: 'none' });
  ...
  describe('initWebVitalsTracking', () => {
    test('should initialize tracking in browser environment', () => {
      ...
      expect(mockAddEventListener).toHaveBeenCalledWith('web-vitals', expect.any(Function));
    });
  ...
  describe('trackCustomMetric', () => {
    test('should track custom metric with transaction', () => {
      ...
      expect(mockSpan.setAttribute).toHaveBeenCalledWith('endpoint', '/api/users');
      expect(mockSpan.setData).toHaveBeenCalledWith('custom_metric.api.response_time', {
        count: 50,
      });
  ...
  describe('trackPageLoadPerformance', () => {
    test('should track page load performance', () => {
      ...
      expect(mockSpan.setData).toHaveBeenCalledWith('transfer_size', { value: 2048, unit: 'byte' });
      ...
      expect(mockSpan.setAttribute).toHaveBeenCalledWith('navigation.type', 'navigate');
    });
