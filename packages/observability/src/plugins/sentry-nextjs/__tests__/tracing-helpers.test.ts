...
describe.todo('tracing-helpers', () => {
  ...
  describe('withApiRouteTracing', () => {
    ...
    test('should handle errors', async () => {
      ...
      expect(mockSpan.setStatus).toHaveBeenCalledWith({ code: 2, message: 'internal error' });
    });
  });
  ...
  describe('withServerActionTracing', () => {
      ...
      expect(Sentry.startSpan).toHaveBeenCalledWith(
        {
          ...
          op: 'server.action',
          attributes: {
            'server.action.formData': '{"name":"test","email":"test@example.com"}',
          },
        },
        expect.any(Function),
      );
    ...
    test('should handle errors', async () => {
      ...
      await expect(withServerActionTracing('MyAction', action)).rejects.toThrow('Action error');
      expect(mockSpan.setStatus).toHaveBeenCalledWith({ code: 2, message: 'internal error' });
    });
  });
  ...
  describe('withRouteHandlerTracing', () => {
    ...
    test('should parameterize dynamic routes', async () => {
      ...
      const wrappedHandler = withRouteHandlerTracing(handler, { route: '/api/user/[id]' });
  ...
  describe('withHttpSpan', () => {
    test('should create HTTP span', async () => {
      ...
      const result = await withHttpSpan('https://api.example.com/users', request, {
        method: 'POST',
        statusCode: 201,
      });
  ...
  describe('recordWebVital', () => {
    ...
    test('should handle custom units', () => {
      ...
      expect(mockSpan.setData).not.toHaveBeenCalledWith('vital.CLS', expect.anything());
    });
  });
