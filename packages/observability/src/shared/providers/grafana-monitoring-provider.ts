...
export class GrafanaMonitoringProvider implements ObservabilityProvider {
  ...
  async captureException(error: Error, context?: ObservabilityContext): Promise<void> {
    ...
    const logEntry: GrafanaLogEntry = {
      ...
      labels: {
        ...
        environment: this.config!.monitoring.service.environment,
        ...context?.tags,
      },
      context: {
        stack: error.stack,
    ...
    if (this.config!.monitoring.features.rum && typeof window !== 'undefined') {
  ...
  async log(level: string, message: string, metadata?: any): Promise<void> {
    if (!this.isEnabled() || !this.config!.monitoring.features.logs) return;
  ...
  async trackMetric(metric: Omit<GrafanaMetric, 'timestamp'>): Promise<void> {
    if (!this.isEnabled() || !this.config!.monitoring.features.metrics) return;
  ...
  async trackBusinessMetric(
    ...
    if (!this.isEnabled() || !this.config!.monitoring.customMetrics?.enabled) return;
  ...
  async startSpan(name: string, parentSpan?: any): Promise<any> {
    if (!this.isEnabled() || !this.config!.monitoring.features.traces) return null;
  ...
  async trackRUMEvent(event: Omit<GrafanaRUMEvent, 'timestamp'>): Promise<void> {
    if (!this.isEnabled() || !this.config!.monitoring.features.rum || typeof window === 'undefined')
      return;
  ...
  async reportHealthCheck(health: Omit<GrafanaHealthCheck, 'timestamp'>): Promise<void> {
    if (!this.isEnabled() || !this.config!.monitoring.features.healthChecks) return;
  ...
  private async sendMetric(metric: GrafanaMetric): Promise<void> {
    try {
      const response = await fetch(`${this.config!.endpoints.prometheus.url}/api/v1/write`, {
        method: 'POST',
        headers: {
      ...
      console.error('[GrafanaMonitoring] Failed to send metric: ', error);
    }
  }

  private async sendLog(log: GrafanaLogEntry): Promise<void> {
    try {
      const response = await fetch(`${this.config!.endpoints.loki.url}/loki/api/v1/push`, {
        method: 'POST',
        headers: {
      ...
      console.error('[GrafanaMonitoring] Failed to send log: ', error);
    }
  }

  private async sendTrace(trace: GrafanaTrace): Promise<void> {
    try {
      const response = await fetch(`${this.config!.endpoints.otel.url}/v1/traces`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
          ...
          resourceSpans: [
            {
              resource: {
                attributes: [
                  {
                    key: 'service.name',
                    value: { stringValue: this.config!.monitoring.service.name },
                  },
                  {
                    key: 'service.version',
                    value: { stringValue: this.config!.monitoring.service.version },
                  },
                ],
              },
      ...
      console.error('[GrafanaMonitoring] Failed to send trace: ', error);
    }
  }

  private async sendRUMEvent(event: GrafanaRUMEvent): Promise<void> {
    try {
      const response = await fetch(`${this.config!.endpoints.rum.url}/collect`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      console.error('[GrafanaMonitoring] Failed to send RUM event: ', error);
    }
  }
  ...
  private async sendLogsBatch(logs: GrafanaLogEntry[]): Promise<void> {
    try {
      ...
      console.error('[GrafanaMonitoring] Failed to send logs batch: ', error);
    }
  }
  ...
  private async sendRUMEventsBatch(events: GrafanaRUMEvent[]): Promise<void> {
    try {
      const response = await fetch(`${this.config!.endpoints.rum.url}/collect/batch`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      console.error('[GrafanaMonitoring] Failed to send RUM events batch: ', error);
    }
  }
