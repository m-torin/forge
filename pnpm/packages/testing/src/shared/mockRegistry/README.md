...
The Mock Registry provides a centralized system for managing mock values in tests across the monorepo. It allows tests to override default values temporarily and ensures consistent mock values across the test suite.

## Features

- Global registry for mock values with default values from example environment variables
...
The Mock Registry is designed to work with the `keys.ts` pattern used across the monorepo. Here's an example of how to integrate it:
...
describe('Your Service', () => {
  ...
  it('should allow overriding mock values for specific tests', () => {
    ...
    mockStripeKeys.override({
      ...
      const keysObj = keys();
      expect(keysObj.STRIPE_SECRET_KEY).toBe('sk_test_custom_key_for_this_test');
    });
  });
});
```
...
1. Create a new file in `packages/testing/src/shared/mockRegistry/services/your-service.ts`
2. Implement service-specific helpers similar to the Stripe example
3. Export the helpers from `packages/testing/src/shared/mockRegistry/services/index.ts`
4. Update the default values in `packages/testing/src/shared/mockRegistry/registry.ts` if needed
