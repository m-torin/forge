...
describe('Server ESLint Configuration', () => {
  ...
  it('includes stricter TypeScript rules for server code', () => {
    ...
    expect(
      configWithStrictTSRules.rules['@typescript-eslint/no-floating-promises'],
    ).toBe('error');
    expect(
  ...
  it('includes Node.js specific rules', () => {
    ...
    expect(configWithNodeRules.rules['node/no-deprecated-api']).toBe('error');
  ...
  it('includes additional security rules for server code', () => {
    ...
    expect(configWithSecurityRules.rules['security/detect-unsafe-regex']).toBe(
      'error',
    );
    expect(
  ...
  it('includes relaxed rules for test files', () => {
    ...
    expect(
      testFilesConfig.rules['@typescript-eslint/explicit-function-return-type'],
    ).toBe('off');
