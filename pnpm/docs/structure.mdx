...
next-forge is a monorepo, which means it contains multiple packages in a single
...
next-forge contains a number of [apps](/apps) that make up your project. Each
...
next-forge contains a number of shared packages that are used across the
